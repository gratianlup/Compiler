...
void FastInlineHeuristic::Invalidate(Function* function) {
    ...
    if(functionInlineInfo_.ContainsKey(function->Id())) {
        functionInlineInfo_.Remove(function->Id());
    }
}
...
void FastInlineHeuristic::AnalyzeFunction(Function* function, InlineInfo& info) {
    ...
    for(auto block = function->FirstBlock(); block; block = block->NextBlock()) {
        ...
        average = UpdateAverageInstrsPerBlock((float)block->InstructionCount(), 
                                              average, blockCount++);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void FastInlineHeuristic::GetConditionParameters(Operand* op, ParameterList& list,
                                                 int level) {
