...
int CallGraph::DiscoverComponent(CallNode* node, int& preorderNumber,
    ...
    for(int i = 0; i < node->CallSiteCount(); i++) {
        ...
        for(int j = 0; j < callSite->CalledFunctionsCount(); j++) {
            ...
            if(calledNode->Weight() == 0) {
                ...
                int previousLowLink = lowLink;
                lowLink = std::min(lowLink, calledNode->Weight());
                hasSelfLoop |= lowLink == previousLowLink;
            }
        }
    }
    ...
    if(node->Weight() == lowLink) {
        ...
        if((componentNodes > 1) || hasSelfLoop) {
            ...
            while(true) {
                ...
                if(lastNode == node) {
                    // Stop because we reached the root node.
                    break;
                }
            }
        }
...
void CallGraph::RedirectCallingSites(CallNodeGroup* component) {
    ...
    for(int i = 0; i < component->NodeCount(); i++) {
        ...
        // Mark the fact that the node is now
        // part of a node group. Can be used by optimizations
        // if they want to be conservative with theses kind of nodes.
        MarkNodeInGroup(node);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
int CallGraph::FindRoots(bool onlyCalledByExternal, bool recompute) {
    if(rootNodesComputed_ && (recompute == false)) {
        return rootNodes_.Count();
    }

    // A root node is considered a function that is not
    // called by any other functions. Note that node groups
    // and functions that are not definitions are ignored.
    rootNodes_.Clear();
    ...
    for(int i = 0; i < nodes_.Count(); i++) {
        ...
        if(callNode->GetFunction()->IsDefinition() == false) {
            continue;
        }

        if(onlyCalledByExternal) {
            if(callNode->IsCalledOnlyByExternalFunctions()) {
                rootNodes_.Add(callNode);
            }
        }
        else if(callNode->IsNotCalledByFunctions()) {
            rootNodes_.Add(callNode);
        }
    }
...
void CallGraph::DepthFirstSearchImpl(CallNodeBase* node, CallNodeVisitor* visitor,
    ...
    if(VisitNode(node, visitor) == false) {
        return;
    }
...
void CallGraph::PostorderTraversalImpl(CallNodeList& startNodes, 
    ...
    while(stack.IsNotEmpty()) {
        ...
        if(pushed == false) {
            if(VisitNode(lastNode, visitor) == false) {
                return;
            }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void CallGraph::ReversePostorderTraversal(CallNodeBase* startNode, 
                                          CallNodeVisitor* visitor) {
    local<PostorderVisitor> postorderVisitor = new PostorderVisitor();
    PostorderTraversal(startNode, postorderVisitor);
    auto& nodes = postorderVisitor->GetNodeList();

    for(int i = nodes.Count() - 1; i >= 0; i--) {
        if(VisitNode(nodes[i], visitor) == false) {
            return;
        }
    }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool CallGraph::ReversePostorderTraversalFromRoots(CallNodeVisitor* visitor) {
    local<PostorderVisitor> postorderVisitor = new PostorderVisitor();
    bool result = PostorderTraversalFromRoots(postorderVisitor);
    auto& nodes = postorderVisitor->GetNodeList();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool CallGraph::ReversePostorderTraversalFromUniqueRoot(CallNodeVisitor* visitor) {
    local<PostorderVisitor> postorderVisitor = new PostorderVisitor();
    bool result = PostorderTraversalFromUniqueRoot(postorderVisitor);
    auto& nodes = postorderVisitor->GetNodeList();
