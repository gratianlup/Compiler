...
private:
    ...
    typedef StaticList<Parameter*, 2> ParameterList;
    ...
    static const int ARRAY_PARAMETER_BENEFIT = 3;
    ...
    void GetConditionParameters(Operand* op, ParameterList& list, int level = 0);
...
public:
    ...
    virtual int GetInliningCost(Function* function) override;

    virtual int GetInliningBenefit(Function* function,
                                   ArgumentList* specializationArguments) override;

    virtual int GetAverageInstructionsPerBlock(Function* function) override;

    virtual int GetCallCount(Function* function, bool ignoreLightweight = false) override;

    virtual void Invalidate(Function* function) override;
};
