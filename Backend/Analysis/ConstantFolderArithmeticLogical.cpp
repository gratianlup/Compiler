...
Operand* ConstantFolder::HandleArithmetic(Opcode opcode, Operand* opA, 
    ...
    if(Instruction::IsFloatArithmetic(opcode) == false) {
        ...
        if(intConstA && intConstB) {
		    ...
		    switch(opcode) {
			    case Opcode::Add:  { result = IA::Add(intConstA, intConstB);  break; }
			    case Opcode::Sub:  { result = IA::Sub(intConstA, intConstB);  break; }
			    case Opcode::Mul:  { result = IA::Mul(intConstA, intConstB);  break; }
			    case Opcode::Div:  {
		...
		if(auto floatConstB = opB->As<FloatConstant>()) {
			...
			double result;
			switch(opcode) {
				case Opcode::Fadd: { result = FA::Fadd(floatConstA, floatConstB); break; }
				case Opcode::Fsub: { result = FA::Fsub(floatConstA, floatConstB); break; }
				case Opcode::Fmul: { result = FA::Fmul(floatConstA, floatConstB); break; }
				case Opcode::Fdiv: { 
					// Don't fold if we divide by 0.
...
Operand* ConstantFolder::HandleArithmeticOneConst(Opcode opcode, Operand* opA, 
    ...
    if(intConstB && intConstB->IsZero()) {
		...
		switch(opcode) {
			...
			case Opcode::Fmul: {
				...
				// 'FloatMode::Exact' flag is not set on the operation.
				auto floatA = opA->As<FloatConstant>();

				if((FA::IsNaN(floatA) || FA::IsInfinity(floatA)) &&
				   (mode == FloatMode::Exact)) {
					return nullptr;
				}
    ...
    if(intConstA && intConstA->IsZero()) {
		...
		switch(opcode) {
			...
			case Opcode::Udiv:
			case Opcode::Mod: 
			case Opcode::Umod: return GetZeroInt(opA);
		}
	}
...
Operand* ConstantFolder::HandlePointerSub(Opcode opcode, Operand* opA, Operand* opB) {
	...
	// sub (ptoi (ptop (field a, INDEX_A, int8*), int32), 
    // and similar for the right side.
	else if(MatchConversion<PtoiInstr>(Match<FieldInstr>(
		    MatchAny(&baseA), MatchIC(&indexOpA)))(opA) &&
		    MatchConversion<PtoiInstr>(Match<FieldInstr>(
		    MatchAny(&baseB), MatchIC(&indexOpB)))(opB)) {
		isElem = true;
	}
	...
	if(auto variableRefA = baseA->As<VariableReference>()) {
		baseType = variableRefA->GetLocalVariable()->GetType();
	}
	...
	if(isElem) {
		...
		__int64 offsetA = recordType->Fields()[(int)indexA].FieldOffset;
		__int64 offsetB = recordType->Fields()[(int)indexB].FieldOffset;
...
Operand* ConstantFolder::HandleLogical(Opcode opcode, Operand* opA, Operand* opB,
	...
	if(intConstA && intConstB) {
		...
		// Perform the operation; note that we don't care about overflow here.
		switch(opcode) {
			case Opcode::And: { result = IA::And(intConstA, intConstB); break; }
			case Opcode::Or:  { result = IA::Or(intConstA, intConstB);  break; }
			case Opcode::Xor: { result = IA::Xor(intConstA, intConstB); break; }
			case Opcode::Shl: {
...
Operand* ConstantFolder::HandleAndOnInstr(Operand* opA, IntConstant* intConst) {
	...
	if(auto variableRef = ptoiInstr->TargetOp()->As<VariableReference>()) {
		if(variableRef->IsGlobal() && variableRef->GetLocalVariable()->HasAlignment()) {
			__int64 alignment = variableRef->GetLocalVariable()->Alignment();
