...
void EdgeEquivalenceInfo::ProcessFunction(Function* function) {
    ...
    CreateParameterRanges(function);
    ProcessBlock(function->FirstBlock(), nullptr);
    delete domTree_;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void EdgeEquivalenceInfo::CreateParameterRanges(Function* function) {
    auto entryBlock = function->FirstBlock();
    auto tag = GetOrCreateTag(entryBlock, nullptr);

    for(int i = 0; i < function->ParameterCount(); i++) {
        auto parameterVariable = function->GetParameterVariable(i);
        auto constantsTag = parameterVariable->GetTag<ParameterConstantsTag>();
        
        if(parameterVariable->IsAddressTaken() || (constantsTag == nullptr)) {
            continue;
        }

        if(constantsTag->IsSafeToUse()) {
            if(constantsTag->IsRange()) {
                PushRangeToSuccessors(constantsTag->GetRange(), 
                                      function->GetParameter(i), entryBlock, tag);
            }
            else if(parameterVariable->IsPointer() && 
                    constantsTag->IsNotZero()) {
                PushNotZeroToSuccessors(function->GetParameter(i), entryBlock, tag);
            }
            else if(parameterVariable->IsInteger()) {
                CreateRangeFromConstants(constantsTag, function, i, tag);
            }
        }
    }
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void EdgeEquivalenceInfo::CreateRangeFromConstants(ParameterConstantsTag* constantsTag, 
                                                   Function* function, int parameterIndex, 
                                                   RangeTag* tag) {
    // Find the range of the parameter by computing the minimum
    // and maximum possible constants.
    ...
    for(int i = 0; i < constantsTag->ConstantCount(); i++) {
        if(auto intConstant = constantsTag->GetConstant(i)->As<IntConstant>()) {
    ...
    PushRangeToSuccessors(Range(lowConstant, highConstant), 
                          function->GetParameter(parameterIndex),
                          function->FirstBlock(), tag);
}
    ...
    for(auto instr = block->FirstInstruction(); instr; 
        ...
        switch(instr->GetOpcode()) {
            ...
            case Opcode::Mul: {
                ...
                if(EvaluateWithRange(instr->As<ArithmeticInstr>(), tag, result)) {
                    PushRangeToSuccessors(result, instr->GetDestinationOp(), block, tag);
                }
                break;
            }
            case Opcode::Div:
            ...
            case Opcode::Question: {
                ...
                if(ExtractRangeFromQuestion(instr->As<QuestionInstr>(), tag, result)) {
                    PushRangeToSuccessors(result, instr->GetDestinationOp(), block, tag);
                }
                break;
            }
            case Opcode::Phi: {
                ...
                if(ExtractRangeFromPhi(instr->As<PhiInstr>(), tag, result)) {
                    PushRangeToSuccessors(result, instr->GetDestinationOp(), block, tag);
                }
                break;
            }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool EdgeEquivalenceInfo::EvaluateWithRange(ArithmeticInstr* instr,
                                            RangeTag* tag, Range& result) {
    // For some instructions the resulting range can be estimated
    // if at least one of the operands has a known range. If only
    // one range is known the other operand must be a constant. 
    // For example, 't2 = add t1, 5', where 't1 = {2, 9}', 
    // results in computing the range 't2 = {7, 14}'.
    ...
    bool hasLeftRange = tag->GetRange(instr->LeftOp(), leftRange);
    bool hasRightRange = tag->GetRange(instr->RightOp(), rightRange);

    if(hasLeftRange && hasRightRange) {
        // Don't try to evaluate anti-ranges.
        if(leftRange.IsRange && rightRange.IsRange) {
            return EvaluateWithRange(instr, leftRange, rightRange, tag, result);
        }
        else return false;
    }
    else if((hasLeftRange == false) && (hasRightRange == false)) {
        // No operand has the range known.
        return false;
    }

    // If the range is an anti-range don't try to evaluate it.
    // For subtraction accept a range only on the left side.
    if((hasLeftRange && (leftRange.IsRange == false))   ||
       (hasRightRange && (rightRange.IsRange == false)) ||
       (instr->IsSub() && hasRightRange)) {
        return false;
    }
    
    // Check if the other operand is a constant.
    auto intConstant = hasLeftRange? instr->RightOp()->As<IntConstant>() :
                                     instr->LeftOp()->As<IntConstant>();
    if(intConstant == nullptr) {
        return false;
    }
    
    return EvaluateWithRange(instr, hasLeftRange ? leftRange : rightRange,
                             intConstant, tag, result);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool EdgeEquivalenceInfo::EvaluateWithRange(ArithmeticInstr* instr, Range range, 
                                            IntConstant* intConstant, 
                                            RangeTag* tag, Range& result) {
    DebugValidator::IsNotNull(intConstant);
    DebugValidator::IsTrue(range.IsRange);
    DebugValidator::IsTrue(instr->IsAdd() || instr->IsSub() || instr->IsMul());

    // Some examples: [1, 4] * 2 -> [2, 8], [a, 3] + 5 -> [a + 5, 8]
    __int64 lowConstant = range.Low.Constant;
    __int64 highConstant = range.High.Constant;
    __int64 otherConstant = intConstant->Value();
    auto intKind = intConstant->GetType()->GetSubtype();
    bool valid = false;

    if(instr->IsAdd()) {
        if((IA::AddOverflows(lowConstant, otherConstant, intKind) == false) &&
           (IA::AddOverflows(highConstant, otherConstant, intKind) == false)) {
            lowConstant = IA::Add(lowConstant, otherConstant, intKind);
            highConstant = IA::Add(highConstant, otherConstant, intKind);
            valid = true;
        }
    }
    else if(instr->IsSub()) {
        if((IA::SubOverflows(lowConstant, otherConstant, intKind) == false) &&
           (IA::SubOverflows(highConstant, otherConstant, intKind) == false)) {
            lowConstant = IA::Sub(lowConstant, otherConstant, intKind);
            highConstant = IA::Sub(highConstant, otherConstant, intKind);
            valid = true;
        }
    }
    else if(instr->IsMul() && (range.HasBaseOperand() == false)) {
        // Note that a range of the form [a, b] cannot be multiplied
        // because there is no way to express the resulting range.
        if((IA::MulOverflows(lowConstant, otherConstant, intKind) == false) &&
           (IA::MulOverflows(highConstant, otherConstant, intKind) == false)) {
            lowConstant = IA::Mul(lowConstant, otherConstant, intKind);
            highConstant = IA::Mul(highConstant, otherConstant, intKind);
            valid = true;
        }
    }

    // If the bounds could be computed create the final range.
    if(valid) {
        result = Range(range.Low.Base, lowConstant,
                       range.High.Base, highConstant);
    }
...
bool EdgeEquivalenceInfo::EvaluateWithRange(ArithmeticInstr* instr, 
                                            ...
                                            RangeTag* tag, Range& result) {
    DebugValidator::IsTrue(instr->IsAdd() || instr->IsSub() || instr->IsMul());
    DebugValidator::IsTrue(leftRange.IsRange);
    DebugValidator::IsTrue(rightRange.IsRange);
    
    // Some examples (note that symbolic ranges are not accepted):
    // [2, 4] + [1, 5] -> [3, 9]  (lowLeft + lowRight,  highLeft + highRight)
    // [2, 4] - [1, 5] -> [-3, 3] (lowLeft - highRight, highLeft - lowRight)            
    // [2, 4] * [1, 5] -> [2, 20] (lowLeft * lowRight,  highLeft * highRight)
    if(leftRange.HasBaseOperand() || rightRange.HasBaseOperand()) {
        return false;
    }

    auto intKind = instr->LeftOp()->GetType()->As<IntegerType>()->GetSubtype();
    __int64 lowLeft = leftRange.Low.Constant;
    __int64 highLeft = leftRange.High.Constant;
    __int64 lowRight = rightRange.Low.Constant;
    __int64 highRight = rightRange.High.Constant;
    __int64 lowResult;
    __int64 highResult;
    bool valid = false;

    if(instr->IsAdd()) {
        if((IA::AddOverflows(lowLeft, lowRight, intKind) == false) &&
           (IA::AddOverflows(highLeft, highRight, intKind) == false)) {
            lowResult = IA::Add(lowLeft, lowRight, intKind);
            highResult = IA::Add(highLeft, highRight, intKind);
            valid = true;
        }
    }
    else if(instr->IsSub()) {
        if((IA::SubOverflows(lowLeft, highRight, intKind) == false) &&
           (IA::SubOverflows(highLeft, lowRight, intKind) == false)) {
            lowResult = IA::Sub(lowLeft, highRight, intKind);
            highResult = IA::Sub(highLeft, lowRight, intKind);
            valid = true;
        }
    }
    else if(instr->IsMul()) {
        if((IA::MulOverflows(lowLeft, lowRight, intKind) == false) &&
           (IA::MulOverflows(highLeft, highRight, intKind) == false)) {
            lowResult = IA::Mul(lowLeft, lowRight, intKind);
            highResult = IA::Mul(highLeft, highRight, intKind);
            valid = true;
        }
    }

    if(valid) {
        result = Range(lowResult, highResult);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool EdgeEquivalenceInfo::ExtractRangeFromPhi(PhiInstr* instr, 
                                              RangeTag* tag, Range& result) {
    if(instr->HasDestinationOp() == false) {
        return false;
    }

    // Try to determine the minimum and maximum values by looking
    // at the ranges of the incoming operands.
    ...
    for(int i = 0; i < instr->OperandCount(); i++) {
        ...
        if(auto intConstant = incomingOp->As<IntConstant>()) {
            minValue = std::min(minValue, intConstant->Value());
            maxValue = std::max(maxValue, intConstant->Value());
        }
        else if(tag->GetRange(incomingOp, incomingRange)) {
            // Accept only ranges that are not symbolic.
            if((incomingRange.IsRange == false) ||
                incomingRange.HasBaseOperand()) {
                return false;
            }

            minValue = std::min(minValue, incomingRange.Low.Constant);
            maxValue = std::max(maxValue, incomingRange.High.Constant);
        }
        else return false;
    }

    result = Range(minValue, maxValue);
    return true;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool EdgeEquivalenceInfo::ExtractRangeFromQuestion(QuestionInstr* instr, 
                                                   RangeTag* tag, Range& result) {
    // Try to determine the minimum and maximum values by looking
    // at the ranges of the 'true' and 'false' operands.
    __int64 minValue = std::numeric_limits<__int64>::max();
    __int64 maxValue = std::numeric_limits<__int64>::min();
    Range trueRange;
    Range falseRange;

    if(auto intConstant = instr->TrueOp()->As<IntConstant>()) {
        minValue = std::min(minValue, intConstant->Value());
        maxValue = std::max(maxValue, intConstant->Value());
    }
    else if(tag->GetRange(instr->TrueOp(), trueRange)) {
        // Accept only ranges that are not symbolic.
        if((trueRange.IsRange == false) || trueRange.HasBaseOperand()) {
            return false;
        }

        minValue = std::min(minValue, trueRange.Low.Constant);
        maxValue = std::max(maxValue, trueRange.High.Constant);
    }
    else return false;

    if(auto intConstant = instr->FalseOp()->As<IntConstant>()) {
        minValue = std::min(minValue, intConstant->Value());
        maxValue = std::max(maxValue, intConstant->Value());
    }
    else if(tag->GetRange(instr->FalseOp(), falseRange)) {
        // Accept only ranges that are not symbolic.
        if((falseRange.IsRange == false) || falseRange.HasBaseOperand()) {
            return false;
        }

        minValue = std::min(minValue, falseRange.Low.Constant);
        maxValue = std::max(maxValue, falseRange.High.Constant);
    }
    else return false;
...
bool EdgeEquivalenceInfo::ExtractRangeFromGlobalConstant(Operand* op, Operand* resultOp,
    ...
    auto globalVar = baseOp->As<VariableReference>()->GetGlobalVariable();

    if((globalVar->IsConstant() == false) ||
       (globalVar->HasInitializerList() == false)) {
        return false;
    }
    else if(auto constantsTag = globalVar->GetTag<GlobalConstantsTag>()) {
        // All possible constants might be known, try to extract a range.
        return ExtractRangeFromPossibleConstants(constantsTag, op, block, tag);
    }
        ...
        if(initializer->IsInitializerList()) {
            ...
            if((initializer->Value()->IsFunctionReference() == false) &&
               (initializer->Value()->IsVariableReference() == false)) {
                ...
                if((initializer->Conversion() != InitConversion::IntToPointer) ||
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool EdgeEquivalenceInfo::ExtractRangeFromPossibleConstants(GlobalConstantsTag* constantsTag, 
                                                            Operand* op, Block* block, 
                                                            RangeTag* tag) {
    if(constantsTag->HasOnlyConstants()) {
        ...
        PushRangeToSuccessors(Range(lowConstant, highConstant), op, block, tag);
        return true;
    }
...
void EdgeEquivalenceInfo::PushNotZeroToSuccessors(Operand* op, Block* block,
    ...
    if(tag->GetRange(op, existingRange) &&
       (tag->IsNotZero(op) != RangeResult::Yes)) {
...
bool EdgeEquivalenceInfo::IsLoopHeader(Block* block, Block* immDomBlock) {
    ...
    else if(auto loopTag = block->ParentFunction()->GetTag<LoopTag>()) {
        if(loopTag->IsSafeToUse()) {
            return loopTag->IsLoopHeader(block);
        }
    }

    // Do a check if no loop information is available.
    // 'block' should represent a single-entry loop header 
    // with control flowing from 'immDomBlock'.
...
bool EdgeEquivalenceInfo::IsFunctionExitBlock(Block* block) {
    ...
    if(block->BranchInstruction()->IsReturn()) {
        return true;
    }

    bool callsExit = false;

    block->ForEachInstructionOfType<CallInstr>([&](CallInstr* instr) -> bool {
        auto type = StdlibRecognizer::Recognize(instr);

        callsExit = (type == StdlibType::exit)  ||
                    (type == StdlibType::abort) ||
                    (type == StdlibType::longjmp);
        return callsExit == false;
    });
...
void EdgeEquivalenceInfo::AddIfRange(IfInstr* ifInstr, Operand* op, 
    ...
    if(oldRangeUsed == false) {
        tag->AddRangeForSuccessor(ifInstr->TrueTargetBlock(), 
                                  op, range);
    }
    ...
    if(op->GetType()->IsPointer()) {
        if(IsNullPointer(range)) {
            // [0, 0] -> (-INF, -1] U [1, +INF)
            tag->AddRangeForSuccessor(ifInstr->FalseTargetBlock(), 
...
void EdgeEquivalenceInfo::PropagateCaseValues(BlockReference* defaultTarget,
    ...
    targetValues.ForEach([tag, defaultTarget, conditionOp, &valueList]
        ...
        if(pair.Value.Count() == 1) {
            ...
            Range range(pair.Value[0], pair.Value[pair.Value.Count() - 1]);
            tag->AddRangeForSuccessor(pair.Key->Target(), conditionOp, range);
        }
...
bool EdgeEquivalenceInfo::IsOperandAddSub(Operand* opB, bool &isOpBAdd, Operand* &opBTemp, 
    ...
    if(auto addInstr = opB->DefiningInstrAs<AddInstr>()) {
        ...
        if(auto intConst = subInstr->RightOp()->As<IntConstant>()) {
            // We can use this information only if overflow
            // is undefined (which is the case for C).
            if(subInstr->HasUndefinedOverflow()) {
                isOpBSub = true;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Range EdgeEquivalenceInfo::CreateRangeForConstCmp(IntConstant* intConst, OrderType order, 
...
Range EdgeEquivalenceInfo::CreateRangeForLessCmp(Range &opRange, __int64 constValue, 
    ...
    __int64 adjust = order == OrderType::Less ? 1 : 0;
...
Range EdgeEquivalenceInfo::CreateRangeForGreaterCmp(Range &opRange, __int64 constValue, 
    ...
    // 'a > [x, +INF) + C' -> [x + C + 1, +INF)
    // For >= 1 is not added.
    __int64 adjust = order == OrderType::Greater ? 1 : 0;
...
Range EdgeEquivalenceInfo::CreateRangeFromPointerCmp(Operand* opA, Operand* opB,
    ...
    if((tag->IsNotZero(opA) == RangeResult::Yes) &&
    ...
    if(result == RangeResult::Yes) {
        ...
        if((order == OrderType::Equal) || 
           (order == OrderType::Greater)) {
            return Range(-1, 1, false /* isRange */);
        }
    }
    else if(result == RangeResult::No) {
        // 'a == b' -> a == NULL, if 'b == NULL'
        // 'a != b' -> a != NULL, if 'b == NULL'
        // 'a > b' -> a != NULL, if 'b == NULL'
        // 'a < b' -> a != NULL, if 'b == NULL'
...
void EdgeEquivalenceInfo::CollectAndRanges(AndInstr* instr, RangeList& list,
    ...
    // 'a = [4, +INF]' and 'a = (-INF, 8]', combined 'a = [4, 8]'. 
...
void EdgeEquivalenceInfo::CollectOrRanges(OrInstr* instr, RangeList& list,
    ...
    // comparisons, like in 'a == 0 || a == 1 || a == 2' -> [0, 3].
    ...
    for(int i = 0; i < list.Count(); i++) {
        if(requiredOp) {
            ...
            if((requiredOp && requiredOp->IsInteger()) == false) {
                valid = false;
                break;
            }
        }
        ...
        if(range.HasBaseOperand() == false) {
			...
			allPositive = allPositive && (tag->IsPositive(op) == RangeResult::Yes);
            allNotZero  = allNotZero  && (tag->IsNotZero(op)  == RangeResult::Yes);
            allNegative = allNegative && (tag->IsNegative(op) == RangeResult::Yes);
        }
        else {
            valid = false;
            return;
