...
AliasLocation SymbolicAliasAnalysis::ComputeOffset(AliasLocation& location) {
	...
	while(base->HasDefiningInstruction()) {
		...
		else if(auto fieldInstr = base->DefiningInstrAs<FieldInstr>()) {
...
bool SymbolicAliasAnalysis::CanResultBeValid(AliasLocation& locationA,
	...
	// the size of the accessed regions, or if the base
	// objects are completely different variables.
    ...
    return baseOpA && baseOpB &&
           baseOpA->IsVariableReference() &&
           baseOpB->IsVariableReference() &&
           (baseOpA != baseOpB);
}
...
AliasResult SymbolicAliasAnalysis::ComputeArrayAlias(AliasLocation& locationA,
	...
	if(instrAIsAddr && instrBIsAddr &&
	   (instrA->IsSameKind(instrB) == false)) {
		return AliasResult::May;
	}
...
bool SymbolicAliasAnalysis::IsNoArrayElementOverlap(__int64 constIndexA, 
	...
	// If both locations represent local/global variables
	...
	if(IsAddressNotTaken(locationA) && 
	   IsAddressNotTaken(locationB)) {
		...
		if(newLocationA.Base() != newLocationB.Base()) {
			auto result = Parent()->ComputeAliasWithUnknownSize(newLocationA.Base(),
																newLocationB.Base());
			return result == AliasResult::None;;
		}
...
AliasResult SymbolicAliasAnalysis::ComputeVariableArrayAlias(AddressInstr* instrA, 
	...
	if((indexInstrA && ((indexInstrA->IsAdd() || indexInstrA->IsSub()) == false)) ||
	   (indexInstrB && ((indexInstrB->IsAdd() || indexInstrB->IsSub()) == false))) {
		return AliasResult::May; // Only 'add' and 'sub' supported.
	}
	...
	if((indexInstrA && indexInstrB) &&
	   (indexInstrA->GetSourceOp(0) != indexInstrB->GetSourceOp(0))) {
		if(Parent()->ComputeAliasWithUnknownSize(instrA->BaseOp(), 
                                                 instrB->BaseOp()) == AliasResult::None) {
            return AliasResult::None;
        }
        else return AliasResult::May;
	}
...
AliasResult SymbolicAliasAnalysis::ComputeVariableArrayAlias(AddressInstr* instrA,
	...
	if(indexInstrA->IsSameKind(indexInstrB) == false) {
...
AliasResult SymbolicAliasAnalysis::ComputeRecordAlias(AliasLocation& locationA,
													  AliasLocation& locationB) {
	auto fieldInstrA = locationA.Base()->DefiningInstrAs<FieldInstr>();
	auto fieldInstrB = locationB.Base()->DefiningInstrAs<FieldInstr>();

	if(fieldInstrA && fieldInstrB) {
		// Determine the alias between two record accesses
		// like 'a.x' and 'a.y.z'.
		return ComputeRecordAlias(fieldInstrA, fieldInstrB,
								  locationA, locationB);
	}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
AliasResult SymbolicAliasAnalysis::ComputeRecordAlias(FieldInstr* instrA, 
													  FieldInstr* instrB,
													  AliasLocation& locationA,
													  AliasLocation& locationB) {
	if(instrA->GetRecordType() == instrB->GetRecordType()) {
		...
		auto fieldA = instrA->GetSelectedField();
		auto fieldB = instrB->GetSelectedField();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
FieldInstr* SymbolicAliasAnalysis::GetAncestor(FieldInstr* fieldInstrA, 
	...
	while(fieldInstrA) {
		if(fieldInstrA->BaseOp()->GetType() == fieldInstrB->BaseOp()->GetType()) {
			return fieldInstrA;
		}
		else {
			// Walk up the record tree. 
			offsetDifference += fieldInstrA->GetSelectedField().FieldOffset;
			fieldInstrA = fieldInstrA->BaseOp()->DefiningInstrAs<FieldInstr>();
		}
	}
...
bool SymbolicAliasAnalysis::IdentifyIV(Operand* op, SimpleIV& ivInfo) {
	...
	// variable have been already folded into a single one.
	// 'i++; i++;' -> 'i += 2;'
	...
	if(step) {
        ...
        ivInfo.HasNotEqualOrder = comparisonOrder == OrderType::NotEqual;
...
Operand* SymbolicAliasAnalysis::IdentifyAddrIVIncrement(Operand* op, Operand* requiredIV,
    ...
    // or by multiple one, with the condition that are indices are constants:
    // t1 = addr iv, 1
    // t2 = addr t1, 2  ->  step = 3
...
bool SymbolicAliasAnalysis::DecidesLoopExecution(CmpInstrBase* cmpInstr, 
    ...
    if(IsSingleBlockLoop(ifInstr->TrueTargetBlock(), loopHeader)) {
        return true;
    }
...
bool SymbolicAliasAnalysis::IsSingleBlockLoop(Block* body, Block* loopHeader) {
    ...
    return gotoInstr && gotoInstr->TargetBlock() == loopHeader;
}
...
bool SymbolicAliasAnalysis::IsInvalidEndValue(OrderType comparisonOrder, 
    ...
    if(comparisonOrder == OrderType::NotEqual) {
        return false;
    }
    // for(i = n - 1; i >= 0; i++) invalid
    // for(i = n - 1; i < 0; i--) invalid
    if(isNegativeStep) {
        if((comparisonOrder != OrderType::Greater) &&
           (comparisonOrder != OrderType::GreaterOrEqual)) {
            return true;
        }
    }
    // for(i = 0; i < n; i--) invalid
    // for(i = 0; i > n; i++) invalid
    else if((comparisonOrder != OrderType::Less) &&
            (comparisonOrder != OrderType::LessOrEqual)) {
        return true;
    }
...
bool SymbolicAliasAnalysis::AreIndicesIndependent(SimpleIndex& indexA, 
   ...
   if(result == AliasResult::Must) {
	   // Definitely independent.
	   return true;
   }
   else if(result == AliasResult::None) {
	   // Definitely not independent.
	   return false;
   }

   // Check if the indices are relative to the addresses
   ...
   // into the indices (this can be done only if the offset
   ...
   // If the indices are equal we use the GCD test,
   // otherwise we try to answer using certain relations
   // that appear between the different indices.
...
AliasResult SymbolicAliasAnalysis::AreIndicesIndependentSimple(SimpleIndex& indexA, 
															   ...
															   AliasLocation& locationB,
															   Block* testBlock) {
	// Here we handle only indices that have the same
	...
	if(indexA.Factor == indexB.Factor) {
		// Either both factors are equal or both indices don't have factors.
		if(indexA.Adjustment == indexB.Adjustment) {
			if(indexA.HasNegativeAdjustment != indexB.HasNegativeAdjustment) {
				return AliasResult::None; // &a[i * F + A] != &a[i *F - A]
			}
			else return AliasResult::Must; // &a[i * F + A] == &a[i * F + A]
		}
			...
			return AreNotEqual(indexA.Adjustment, indexB.Adjustment) ? 
				   AliasResult::None : AliasResult::Must;
		}
	}
	...
	return AliasResult::May; // Unknown.
}
...
bool SymbolicAliasAnalysis::AreIndicesIndependent(SimpleIndex& indexA, 
	...
	// we can express one of the indices in terms of the other
...
bool SymbolicAliasAnalysis::AreIndicesIndependentSameIndex(SimpleIndex& indexA, 
	...
	// between two array indices if the GCD of the factors
...
bool SymbolicAliasAnalysis::IsGreaterThanZero(Operand* op, Block* testBlock) {
	...
	return opInfo.IsGreater(op, GetZeroInt(testBlock), 
							true, testBlock) == RangeResult::Yes;
}
...
bool SymbolicAliasAnalysis::AreNotEqual(Operand* opA, Operand* opB, Block* testBlock) {
	...
	// As a last resort ask we ask Operand Info.
	OperandInfo opInfo(nullptr, Parent()->GetTarget());
	return opInfo.AreNotEqual(opA, opB, testBlock) == RangeResult::Yes;
}
...
bool SymbolicAliasAnalysis::AreEqual(Operand* opA, Operand* opB, Block* testBlock) {
	...
	// As a last resort ask we ask Operand Info.
	OperandInfo opInfo(nullptr, Parent()->GetTarget());
	return opInfo.AreEqual(opA, opB, testBlock) == RangeResult::Yes;
}
...
bool SymbolicAliasAnalysis::IsGreater(Operand* opA, Operand* opB, Block* testBlock) {
	...
	// As a last resort ask we ask Operand Info.
	OperandInfo opInfo(nullptr, Parent()->GetTarget());
	return opInfo.IsGreater(opA, opB, true, testBlock) == RangeResult::Yes;
}
...
bool SymbolicAliasAnalysis::IsSmaller(Operand* opA, Operand* opB, Block* testBlock) {
	...
	// As a last resort ask we ask Operand Info.
	OperandInfo opInfo(nullptr, Parent()->GetTarget());
	return opInfo.IsSmaller(opA, opB, true, testBlock) == RangeResult::Yes;
}
...
bool SymbolicAliasAnalysis::IsGreaterOrEqual(Operand* opA, Operand* opB, Block* testBlock) {
	...
	// As a last resort ask we ask Operand Info.
	OperandInfo opInfo(nullptr, Parent()->GetTarget());
	return opInfo.IsGreaterOrEqual(opA, opB, true, testBlock) == RangeResult::Yes;
}
