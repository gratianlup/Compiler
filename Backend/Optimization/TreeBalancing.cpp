...
void TreeBalancing::Execute(Function* function) {
    funct_ = function;
    ...
    for(int i = postorderList.Count() - 1; i >= 0; i--) {
        ...
        block->ForEachInstruction([rank, this](Instruction* instr) -> bool {
            if(instr->HasDestinationOp()) {
                int actualRank = rank * ComputeRankFactor(instr);
                operandRanks_.Add(instr->GetDestinationOp(), actualRank);                
            }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
int TreeBalancing::ComputeRankFactor(Instruction* instr) {
    // We want to be sure that instructions that are highly probable
    // NOT to be loop-invariant to get grouped last (at least they
    // should not prevent other instructions from moving outside).
    if(instr->IsLoad()) {
        return LOAD_RANK_FACTOR;
    }
    else if(auto callInstr = instr->As<CallInstr>()) {
        if(auto intrinsic = callInstr->GetIntrinsic()) {
            // Calls to math intrinsics are loop-invariant if
            // all arguments are also loop-invariant.
            if(intrinsic->IsMathIntrinsic() || 
               intrinsic->IsBitwiseIntrinsic()) {
                return INTRINSIC_CALL_RANK_FACTOR;
            }
        }
        else if(auto function = callInstr->GetCalledFunction()) {
            if(function->IsNoState()) {
                // Calls to functions that don't depend on the global
                // state of the program are more likely to be loop-invariant,
                // especially if the arguments are not pointers.
                bool allParameters;
                bool hasPointerArgs = HasPointerArguments(callInstr, allParameters);

                if(hasPointerArgs) {
                    if(allParameters) {
                        // If the pointers are parameters they might have been
                        // marked as having no alias by interprocedural analysis.
                        return NOSTATE_CALL_WITH_PARAMETER_POINTERS_RANK_FACTOR;
                    }
                    else return NOSTATE_CALL_WITH_POINTERS_RANK_FACTOR;
                }

                return NOSTATE_CALL_RANK_FACTOR;
            }
        }

        // Any other call type is unlikely to be loop-invariant.
        return CALL_RANK_FACTOR;
    }

    // No rank adjustment for other instructions.
    return 1;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool TreeBalancing::HasPointerArguments(CallInstr* instr, bool& allParameters) {
    bool hasPointers = false;
    allParameters = true;

    instr->ForEachArgumentOfType<PointerType>([&](Operand* argument, int i) -> bool {
        hasPointers = true;
        allParameters &= argument->IsParameter();
        return true;
    });
...
void TreeBalancing::OptimizeExpression(Instruction* instr) {
    ...
    if(auto result = ConvertToMultiplication(root)) {
...
void TreeBalancing::ConvertSubtractionToAddition(Expression* expr) {
    ...
    if(operationExpr->IsSubtraction()) {
        ...
        operationExpr->Operation = Opcode::Add;
...
TreeBalancing::Expression* TreeBalancing::NegateValue(ValueExpression* expr) {
    ...
    // 'param' -> '0 - param'.
    return new OperationExpression(Opcode::Sub, GetZeroValue(expr->ResultType),
        ...
        case Opcode::Mul: {
            ...
            if(operationExpr->Left->IsOperation() &&
               operationExpr->Right->IsConstant()) {
                ...
                if((otherExpr->Operation != Opcode::Mul) ||
...
void TreeBalancing::DistributeFactors(ExpressionList& expressionRoots) {
    ...
    for(int i = 0; i < count; i++) {
        if(expressionRoots[i]->IsOperation() == false) {
            continue;
        }

        auto operationExpr = static_cast<OperationExpression*>(expressionRoots[i]);
        ...
        if((operationExpr->IsMultiplication()) &&
            ...
            if(times == std::abs(value)) {
                ...
                __int64 remaining = negated ? std::min(0LL, value + MAX_DISTRIBUTE_FACTOR) :
                                              std::max(0LL, value - MAX_DISTRIBUTE_FACTOR);
                auto remainingExpr = GetIntConstantValue(remaining, operationExpr->ResultType);
...
void TreeBalancing::UnifyIdentical(ExpressionList& expressionRoots) {
    ...
    for(int i = 0; i < expressionRoots.Count() - 1; i++) {
        ...
        expressionRoots[i] = new OperationExpression(Opcode::Mul, factorExpr,
...
void TreeBalancing::ExtractFactors(ExpressionList& expressionRoots) {
    ...
    while(changed && (expressionRoots.Count() > 1)) {
        ...
        for(int i = 0; i < expressionRoots.Count(); i++) {
            ...
            ExtractFactors(expr, Opcode::Mul, false /* onlyValues */, foundExprs);
        ...
        for(int i = 0; i < expressionRoots.Count(); i++) {
            ...
            auto candidateExprA = exprA->IsValue() ? 
                                  exprA : GetOperationAs(exprA, Opcode::Mul);
            ...
            for(int j = i + 1; j < expressionRoots.Count(); j++) {
                ...
                auto candidateExprB = exprB->IsValue() ? 
                                      exprB : GetOperationAs(exprB, Opcode::Mul);
...
TreeBalancing::Expression* TreeBalancing::UnifyWithFactors(Expression* a, Expression* b, 
    ...
    auto sumExpr = new OperationExpression(Opcode::Add, a, b, a->ResultType);
    auto result = new OperationExpression(Opcode::Mul, sumExpr, 
...
TreeBalancing::Expression* TreeBalancing::GenerateFactor(OccurrenceList& factors) {
    ...
    for(int i = 1; i < factors.Count(); i++) {
        ...
        result = new OperationExpression(Opcode::Mul, result, newFactor, 
                                         result->ResultType);
    }
...
TreeBalancing::Expression* TreeBalancing::GenerateFactor(Expression* term, int times) {
    ...
    while(times-- > 1) {
        result = new OperationExpression(Opcode::Mul, result, 
...
TreeBalancing::Expression* TreeBalancing::RemoveFactors(Expression* expr, 
    ...
    // For example, if 'factors = {a,c}' and the expression is '(a * b) * c'
    // the result would be '(1 * b) * 1' (this will be simplified later).
    if(auto mulExpr = GetOperationAs(expr, Opcode::Mul)) {
...
Operand* TreeBalancing::EmitRankedExpression(OperationExpression* expr, 
    ...
    if(expr->Left->IsOperation(expr->Operation)) {
        ...
        if(HasLowerRank(otherExpr->Left, expr->Right)) {
            // '(a * b) * c' -> 'b * (a * c)'
        ...
        else if(HasLowerRank(otherExpr->Right, expr->Right)) {
            // '(a * b) * c' -> 'a * (b * c)'
            auto opA = EmitExpression(otherExpr->Left, positionInstr);
            auto opB = EmitExpression(otherExpr->Right, positionInstr);
            auto opC = EmitExpression(expr->Right, positionInstr);
    ...
    if(expr->Right->IsOperation(expr->Operation)) {
        ...
        if(HasLowerRank(otherExpr->Left, expr->Left)) {
            // 'a * (b * c)' -> '(a * b) * c'
        ...
        else if(HasLowerRank(otherExpr->Right, expr->Left)) {
            // 'a * (b * c)' -> '(a * c) * b'
            auto opA = EmitExpression(expr->Left, positionInstr);
            auto opB = EmitExpression(otherExpr->Left, positionInstr);
            auto opC = EmitExpression(otherExpr->Right, positionInstr);
...
Operand* TreeBalancing::CreateInstruction(Opcode opcode, Operand* leftOp,
    ...
    // Don't emit an instruction if it would definitely
    // turn into a constant after folding.
    if(leftOp->IsConstant() && rightOp->IsConstant()) {
        return FoldConstants(opcode, leftOp, rightOp);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* TreeBalancing::FoldConstants(Opcode opcode, Operand* leftOp, Operand* rightOp) {
    DebugValidator::IsTrue(Instruction::IsArithmetic(opcode) ||
                           Instruction::IsLogical(opcode));
    DebugValidator::IsTrue(leftOp->IsConstant());
    DebugValidator::IsTrue(rightOp->IsConstant());

    IRGenerator irGen(funct_->ParentUnit());
    ConstantFolder folder(&irGen, GetTarget());
    return folder.FoldBinary(opcode, leftOp, rightOp);
}
...
void TreeBalancing::SortByRank(ExpressionList& roots) {
    for(int i = 0; i < roots.Count(); i++) {
        for(int j = i + 1; j < roots.Count(); j++) {
            if(HasLowerRank(roots[i], roots[j])) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool TreeBalancing::HasLowerRank(Expression* a, Expression* b) {
    if(a->Rank < b->Rank) {
        return true;
    }
    else if(a->Rank == b->Rank) {
        // If we have two addressing instructions that end having the same rank
        // favor the one that uses a index operand with lower rank. For example,
        // in 'a[i] * b[j] * 5' give 'a[i]' a lower rank so that it is grouped with 5
        // (loop-invariant code motion can hoist the group then).
        if(a->IsValue() && b->IsValue()) {
            auto valueA = static_cast<ValueExpression*>(a);
            auto valueB = static_cast<ValueExpression*>(b);
            auto instrA = valueA->Value->DefiningInstruction();
            auto instrB = valueB->Value->DefiningInstruction();

            if(instrA && instrB && instrA->IsLoad() && instrB->IsLoad()) {
                auto loadedInstrA = instrA->GetSourceOp(0)->DefiningInstruction();
                auto loadedInstrB = instrB->GetSourceOp(0)->DefiningInstruction();

                if(loadedInstrA && loadedInstrB &&
                   loadedInstrA->IsAddressing() && loadedInstrB->IsAddressing()) {
                    // Favor the index with the smaller rank.
                    return GetRank(loadedInstrA->GetSourceOp(1)) <
                           GetRank(loadedInstrB->GetSourceOp(1));
                }
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
int TreeBalancing::GetRank(Operand* op) {
    if(op->IsConstant()) {
        return 0;
    }
...
TreeBalancing::OperationExpression* TreeBalancing::CreateMultiply(Expression* left,
                                                                  Expression* right) {
    return new OperationExpression(Opcode::Mul, left, 
