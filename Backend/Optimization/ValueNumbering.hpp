...
private:
    ...
    // Returns 'true' if it is unsafe to replace to replace the current call
    // with the previous one because the locations pointed by the pointer
    // arguments (may) have changed in between.
    bool HasGlobalStateChanged(CallInstr* previousCall, CallInstr* currentCall);
