...
private:
    ...
    struct AccessPath {
        StaticList<__int64, 3> Indices; // The accessed components.
    ...
    struct Access {
        ...
        AccessPath Path;     // The sequence of access indices.
    ...
    struct CopyAccess {
        ...
        __int64 Size;                  // The amount of bytes copied/set.
    ...
    struct TreeNode {
        ...
        __int64 Offset;            // The offset, in bytes.
        __int64 Size;              // The size, in bytes.
	...
	typedef StaticList<Instruction*, 16> WorkList;
	...
	static const int CALL_PENALIZATION = 4;
    ...
    int replacementsCreated_; // For debugging only.
	...
	// Returns 'true' if the address of the specified variable
	// is taken only be 'call' instructions and doesn't escape.
	bool IsTakenOnlyByCalls(Variable* variable);

	bool BuildInitialWorklist(VariableReference* variableRef, WorkList& worklist);

	bool IsEligibleUser(Instruction* user, Instruction* source, 
						VariableReference* variableRef);

	bool ComputesZeroOffset(Instruction* instr, VariableReference* variableRef);

	bool ShouldAddUserToWorklist(Instruction* instr);
	...
	// and collects the indices used by the addressing instructions
    ...
    bool CreateAccessPathForField(FieldInstr* fieldInstr, AccessPath& path, 
                                  Variable*& variable, __int64& offset, bool usedByAddr);
    ...
    bool ShouldDoPartialReplacement(Candidate& candidate, PathDict& replaced);
    ...
    void CopyCommonComponents(NodeList& destList, NodeList& sourceList,
                              __int64 startDestOffset, __int64 startSourceOffset,
    ...
    void CopyRemainingComponents(NodeList& sourceList, __int64 startDestOffset, 
                                 __int64 startSourceOffset, __int64 finalSourceOffset,
                                 CopyAccess& copyAccess);
    ...
    TreeNode* CreateTree(const Type* type, __int64 offset, 
    ...
    bool HasCommonRegion(TreeNode* node, __int64 offset, __int64 size);
    ...
    void AddInRegionToList(TreeNode* node, __int64 offset, 
	...
	// by the access path (creates 'index' and 'field' instructions).
    ...
    void GenerateMemorySet(Operand* destAddress, Operand* valueOp,
                           __int64 toOffset, int size, Instruction* beforeInstr);
    ...
    void WriteReplacementToMemory(Variable* replacement, CopyAccess& access, 
                                  __int64 toOffset, Instruction* beforeInstr);
