...
bool BlockUtils::ReplacePhiOperandsBlock(Block* phiBlock, Block* previousBlock,
    ...
    for(auto instr = phiBlock->FirstInstruction(); instr;
        ...
        for(int i = 0; i < phiInstr->OperandCount(); i++) {
            if(phiInstr->GetOperandBlock(i) == previousBlock) {
                previousIndex = i;
            }
            else if(phiInstr->GetOperandBlock(i) == newBlock) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BlockUtils::ReplacePhisWithIncomingFrom(Block* block, Block* incomingBlock) {
	DebugValidator::IsNotNull(block);
	DebugValidator::IsNotNull(incomingBlock);

	while(auto phiInstr = block->FirstInstruction()->As<PhiInstr>()) {
		if(phiInstr->HasDestinationOp()) {
			auto incomingOp = phiInstr->GetOperandFromBlock(incomingBlock);
			phiInstr->ResultOp()->ReplaceWith(incomingOp);
		}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BlockUtils::RemoveIncomingFrom(Block* block, Block* incomingBlock) {
	DebugValidator::IsNotNull(block);
	DebugValidator::IsNotNull(incomingBlock);

	block->ForEachPhiInstruction([&](PhiInstr* phiInstr) -> bool {
		if(phiInstr->HasOperandFromBlock(incomingBlock)) {
			phiInstr->RemoveOperand(incomingBlock);
		}
