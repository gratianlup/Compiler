    ...
    if(Instruction::IsAssociative(opcode) &&
       ((Instruction::IsFloatArithmetic(opcode) == false) || 
        ...
        if(arithInstr1) {
			...
			if(arithInstr1->IsFloatArithmetic() && (mode != FloatMode::Fast)) {
        ...
        if(arithInstr2) {
			...
			if(arithInstr2->IsFloatArithmetic() && (mode != FloatMode::Fast)) {
				// The second instruction must also be marked as "fast".
				return nullptr;
			}
...
Operand* Peephole::HandleDistributive(Opcode opcode, Operand* opA, Operand* opB, 
    ...
    if(instrA && instrB && 
       Instruction::IsFloatArithmetic(opcode)) {
        ...
        if((arithInstrA->GetFPMode() != FloatMode::Fast) ||
           (arithInstrB->GetFPMode() != FloatMode::Fast)) {
            return nullptr;
        }
    }
    ...
    if(((opcode == Opcode::Fadd) || (opcode == Opcode::Fsub)) &&
       (instrA && instrB) && instrA->IsSameKind(instrB) &&
       (instrA->GetOpcode() == Opcode::Fdiv)) {
        ...
        if(C1 && C2 && (instrA->GetSourceOp(0) == instrB->GetSourceOp(0))) {
            ...
            double R  = opcode == Opcode::Fadd ? FA::Fadd(R1, R2, floatKind) :
    ...
    if(instrB && Instruction::IsDistributive(opcode, instrB->GetOpcode())) {
        // Don't do it for integer division.
        if((opcode == Opcode::Div) || (opcode == Opcode::Udiv)) {
            return nullptr;
        }
...
Operand* Peephole::HandleSubAssociative(Operand* opA, Operand* opB, Block* block) {
	...
	if(Match<AddInstr>(MatchAny(&a), MatchAny(&b))(opA) && 
       MatchAny(&c)(opB)) {
		// Try to simplify 'b - c'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, b, c, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Add, a, result1, block)) {
		...
		// Try to simplify 'a - c'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, a, c, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Add, b, result1, block)) {
				// The operands can be simplified.
				return LOG(result2);
			}
	...
	if(Match<AddInstr>(MatchAny(&b), MatchAny(&c))(opB) && 
       MatchAny(&a)(opA)) {
		// Try to simplify 'a - b'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, a, b, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Sub, result1, c, block)) {
				// The operands can be simplified.
				return LOG(result2);
			}
		...
		// Try to simplify 'a - c'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, a, c, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Sub, result1, b, block)) {
				// The operands can be simplified.
				return LOG(result2);
			}
	...
	if(Match<SubInstr>(MatchAny(&a), MatchAny(&b))(opB) &&
       MatchAny(&c)(opA)) {
		// Try to simplify 'c - a'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, c, a, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Add, result1, b, block)) {
				// The operands can be simplified.
				return LOG(result2);
			}
		...
		// Try to simplify 'b - a'.
		if(auto result1 = SimplifyBinary(Opcode::Sub, b, a, block)) {
			if(auto result2 = SimplifyBinary(Opcode::Add, result1, c, block)) {
				// The operands can be simplified.
				return result2;
			}
