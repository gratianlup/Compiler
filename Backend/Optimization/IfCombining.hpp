...
// by hoisting the operands in the same block. It also tries to promote 
// operations to 'quest' instructions.
// It can be thought of as a scope-restricted variant of 'if-conversion'.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
private:
    ...
    typedef Dictionary<Instruction*, bool> VisitedDictionary;
    typedef StaticList<Instruction*, 32> CandidateList;
    typedef StaticList<IfInstr*, 32> CandidateIfList;
    typedef Dictionary<Operand*, Instruction*> TrappingDictionary;
    typedef IRDominatorTree::TDominatorNode DominatorNode;
    
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    static const int QUESTION_PHI_LIMIT = 2;
    static const int HOISTING_LIMIT = 5;
    static const int HOISTING_LIMIT_IF_THEN_ELSE = 4;
    
    ProfileInfo* profile_;
    IRDominatorTree* domTree_;
    Dictionary<Block*, TrappingDictionary> trappingOps_;
    IntProfileAdjustment hoistingLimitAdjustment_;
    ...
    bool IsUsedOnlyByHoisted(Instruction* instr, VisitedDictionary& visited,
                             Block* sourceBlock);
    ...
    void ComputeTrappingOperands(Block* block, TrappingDictionary& dict);
...
public:
    IfCombining(ProfileInfo* profile = nullptr) : profile_(profile) {
        hoistingLimitAdjustment_ = IntProfileAdjustment(profile);
    }
