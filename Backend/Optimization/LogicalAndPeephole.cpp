...
Operand* Peephole::HandleAnd(Operand* opA, Operand* opB, Block* block,
	...
	if(auto result = folder_.FoldBinary(Opcode::And, opA, opB, block)) {
		return LOG(result);
	}
    ...
    if(auto result = HandleAssociative(Opcode::And, opA, opB, block)) {
        return result;
    }

    // Try to simplify using distributivity laws.
    if(auto result = HandleDistributive(Opcode::And, opA, opB, block)) {
        return result;
    }
	...
	if(Match<XorInstr>(MatchAny(&a), MatchIC(&C1, block))(opA) && 
       ...
       (C1 == C2) && IA::IsPowerOfTwo(C1)) {
		...
		return LOG(CreateCompare(andOp, C1, OrderType::Equal, 
                                 opA->GetType(), true));
	}
	...
	if(IsLogicalNot(opA, &a) && MatchIC(&C, block)(opB) && 
       IA::IsPowerOfTwo(C)) {
		...
		return LOG(CreateCompare(andOp, C, OrderType::Equal, 
                                 opA->GetType(), true));
	}
    ...
    if(auto result = HandleOperationOnQuest(Opcode::And, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::And, opA, opB)) {
        return result;
    }
    ...
    // This optimization applies to code that involves bitfield copying.
    // ((a << C) >> C) & 2^C+1 -> a & 2^C+1
    // // ((a << 31) >> 31) & 1 -> 1
    // The bits produced by the sign extension are newer used.
    if(MatchIC(&C2)(opB) && IA::IsPowerOfTwo(C2) &&
       Match<ShrInstr>(Match<ShlInstr>(MatchAny(&a), MatchIC(&C1)), MatchIC(&C))(opA)) {
        __int64 typeBits = TI::GetSizeBits(a->GetType(), GetTarget());

        if((C == C1) && (C1->Value() < typeBits) &&
           ((IA::Log2(C2) + 1) == (typeBits - C1->Value()))) {
            auto andOp = GetTemporary(a);
            irGen_.GetAnd(a, C2, andOp);
            return LOG(andOp);
        }
    }
...
Operand* Peephole::HandleAndCmpCmp(Operand* opA, Operand* opB, Block* block) {
    ...
    // ((a & b) == 0) & ((a & d) == 0) -> (a & (b | d)) == 0
    if(MatchCmpCmpOperands(cmpInstrA, cmpInstrB, CmpCmpComparand::Zero, 
        ...
        return LOG(CreateCompare(andOp, GetZeroInt(a), OrderType::Equal, 
                                 cmpInstrA->ResultOp()->GetType(), 
                                 cmpInstrA->IsUcmp()));
    }

	// ((a & b) == b) & ((a & d) == d) -> (a & (b | d)) == (b | d)
    if(MatchCmpCmpOperands(cmpInstrA, cmpInstrB, CmpCmpComparand::Right, 
                           true /* expectAnd */, a, b, d)) {
        auto orOp = GetTemporary(a);
        irGen_.GetOr(b, d, orOp);
        ...
        return LOG(CreateCompare(andOp, orOp, OrderType::Equal, 
                                 cmpInstrA->ResultOp()->GetType(), 
                                 cmpInstrA->IsUcmp()));
    }

	// ((a & b) == a) & ((a & d) == a) -> (a & (b & d)) == a
    if(MatchCmpCmpOperands(cmpInstrA, cmpInstrB, CmpCmpComparand::Left, 
                           true /* expectAnd */, a, b, d)) {
        auto andOp1 = GetTemporary(a);
        ...
        return LOG(CreateCompare(andOp2, a, OrderType::Equal, 
                                 cmpInstrA->ResultOp()->GetType(), 
                                 cmpInstrA->IsUcmp()));
    }
    ...
    if(MatchCmpCmpOperands(cmpInstrA, cmpInstrB, CmpCmpComparand::Right, 
        ...
        return LOG(CreateCompare(orOp, andOp, OrderType::Equal, 
                                 cmpInstrA->ResultOp()->GetType(), 
                                 cmpInstrA->IsUcmp()));
    }
    ...
    if(MatchCmpCmpOperands(cmpInstrA, cmpInstrB, CmpCmpComparand::Left, 
        ...
        return LOG(CreateCompare(orOp2, a, OrderType::Equal, 
                                 cmpInstrA->ResultOp()->GetType(), 
                                 cmpInstrA->IsUcmp()));
    }
...
Operand* Peephole::HandleAndTwoConst(Operand* opA, Operand* opB, Block* block) {
	...
	if(Match<OrInstr>(MatchAny(&a), MatchIC(&C1, block))(opA) &&
	   opA->HasSingleUser()) {
		...
		if(C1C2 != C1->Value()) {
			auto constantOp = folder_.FoldBinary(Opcode::And, C1, C2, block);
			auto orOp = GetTemporary(opA);
	...
	if(Match<XorInstr>(MatchAny(&a), MatchIC(&C1, block))(opA) &&
	   opA->HasSingleUser()) {
		...
		auto andOp2 = folder_.FoldBinary(Opcode::And, C1, C2, block);
...
Operand* Peephole::HandleAndOneConst(Operand* opA, Operand* opB, Block* block) {
	...
	if((Match<ShlInstr>(MatchInt(1, block), MatchAny(&a))(opA) ||
		...
		irGen_.GetUcmp(OrderType::Equal, a, GetZeroInt(opA), cmpOp);
		return LOG(cmpOp);
	}
	...
	if(auto zextInstr = opA->DefiningInstrAs<ZextInstr>()) {
		...
		__int64 fromBitsValue = IA::ValueFromBitCount(fromBits);
...
Operand* Peephole::HandleAndIntCmp(Operand* opA, Operand* opB, Block* block) {
    ...
    // Test for comparisons that can be replaced by an
    // equality comparison like in the following example:
    // (a > 19) & (a < 21) -> a == 20
    // This can be done only for integers.
    auto greaterCmp = leftCmp->IsGreater() ? leftCmp :
                      rightCmp->IsGreater() ? rightCmp : nullptr;
    auto lessCmp = rightCmp->IsLess() ? rightCmp :
                   leftCmp->IsLess() ? leftCmp : nullptr;

    if((greaterCmp && lessCmp)                      && 
       (greaterCmp->IsFcmp() == false)              && // No floating comparison.
        greaterCmp->IsSameKind(lessCmp)             && // Same type of comparison.
        (greaterCmp->LeftOp() == lessCmp->LeftOp()) && // Same operand compared.
        greaterCmp->RightOp()->IsIntConstant()      && // Other operand should be constat.
        lessCmp->RightOp()->IsIntConstant()) {         
        auto greaterConst = greaterCmp->RightOp()->As<IntConstant>();
        auto lessConst = lessCmp->RightOp()->As<IntConstant>();

        if((lessConst->Value() - greaterConst->Value()) == 2) {
            __int64 equalValue = greaterConst->Value() + 1;
            auto equalConst = irGen_.GetIntConst(greaterConst->GetType(), equalValue);

            return LOG(CreateCompare(greaterCmp->LeftOp(), equalConst, 
                                     OrderType::Equal, opA->GetType(), greaterCmp->IsUcmp()));
        }
    }
...
Operand* Peephole::HandleAndIntCmpSameConst(CmpInstrBase* leftCmp, 
    ...
    DebugValidator::IsTrue(leftCmp->IsSameKind(rightCmp));
    DebugValidator::AreEqual(leftCmp->RightOp(), rightCmp->RightOp());
    IntConstant* C;
    ...
    if(leftCmp->IsEqual() && 
       leftCmp->RightOp()->IsZeroInt()) {
        ...
        return LOG(CreateCompare(orOp, leftCmp->RightOp(), OrderType::Equal,
							     leftCmp->ResultOp()->GetType(), true /* isUcmp */));
    }
    else if(leftCmp->IsCmp() && leftCmp->IsGreaterOrEqual() &&
            leftCmp->RightOp()->IsZeroInt()) {
        // (a >= 0) & (b >= 0) -> (a | b) >= 0
        auto orOp = GetTemporary(leftCmp->LeftOp());
		irGen_.GetOr(leftCmp->LeftOp(), rightCmp->LeftOp(), orOp);

        return LOG(CreateCompare(orOp, leftCmp->RightOp(), OrderType::GreaterOrEqual,
							     leftCmp->ResultOp()->GetType(), false /* isUcmp */));
    }
...
Operand* Peephole::HandleAndIntCmpConst(CmpInstrBase* leftCmp, CmpInstrBase* rightCmp,
	...
	case OrderType::Equal: {
		...
		case OrderType::NotEqual: // (a == 8) & (a != 9) -> a == 8
		case OrderType::Less: {   // (a == 8) & (a < 9)  -> a == 8
			return LOG(leftCmp->ResultOp());
		}
		case OrderType::Equal:     // (a == 8) & (a == 9) -> false
        case OrderType::Greater: { // (a == 8) & (a  > 9) -> false
	...
	case OrderType::NotEqual: {
		...
		case OrderType::Less: { // (a != 8) & (a < 9) -> a < 8
			...
			if(IsLeftRightMinusOne(leftConst, rightConst)) {
				return LOG(CreateCompare(leftCmp->LeftOp(), leftConst, 
                                         OrderType::Less, leftCmp->ResultOp()->GetType(), 
									     leftCmp->IsUcmp()));
			}
            break;
		}
		case OrderType::Equal:     // (a != 8) & (a == 10) -> a == 10
		case OrderType::Greater: { // (a != 8) & (a  > 10) -> a > 10
		...
		case OrderType::NotEqual: {
			...
			return LOG(CreateCompare(subOp, GetOneInt(leftOp), OrderType::Greater,
	...
	case OrderType::Less: {
		...
		case OrderType::NotEqual: { // (a < 8) & (a != 10) -> a < 8
			return LOG(leftCmp->ResultOp());
		}
		case OrderType::Less: { // (a < 8) & (a < 10) -> a < 8
			// This is true only if both comparisons are of the same type.
			if(leftCmp->IsUcmp() == leftCmp->IsUcmp()) {
				return LOG(leftCmp->ResultOp());
			}
	...
	case OrderType::Greater: {
		switch(rightCmp->Order()) {
		case OrderType::Equal: { // (a > 8) & (a == 9) -> a == 9
			return LOG(rightCmp->ResultOp());
		}
		case OrderType::Greater: { // (a > 8) & (a > 9)  -> a > 9
			// This is true only if both comparisons are of the same type.
			if(leftCmp->IsUcmp() == leftCmp->IsUcmp()) {
				return LOG(rightCmp->ResultOp());
			}
		...
		case OrderType::Less: { // (a > 8) & (a < 10) -> (a - 8) < 2
			// This is true only if both comparisons are of the same type.
			if(leftCmp->IsUcmp() == leftCmp->IsUcmp()) {
		...
		case OrderType::NotEqual: { // (a > 8) & (a != 9) -> a > 9
			if(IsLeftRightMinusOne(leftConst, rightConst)) {
				return LOG(CreateCompare(leftCmp->LeftOp(), rightConst, 
										 OrderType::Greater, leftCmp->ResultOp()->GetType(), 
										 leftCmp->IsUcmp()));
			}
            break;
		}
