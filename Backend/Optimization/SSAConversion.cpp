...
void SSAConversion::RenameVariables(Block* block) {
    ...
    while(instr) {
        ...
        if(auto phiInstr = instr->As<PhiInstr>()) {
            ...
            auto activeOp = GetActiveOperand(variableRef->GetLocalVariable());
			PreserveTags(loadInstr->ResultOp(), activeOp);
            ...
            PushOnStack(variableRef->GetLocalVariable(), 
                        storeInstr->SourceOp(), assignedVars);
    ...
    for(int i = 0; i < children.Count(); i++) {
        ...
        if(replacedPhis_.Count() != oldReplacedPhiCount) {
            for(int j = oldReplacedPhiCount; j < replacedPhis_.Count(); j++) {
                if(replacedPhis_[j].Parent == block) {
                    // All children should use the new operand
                    // instead of the 'phi' result.
                    PushOnStack(replacedPhis_[j].PhiVariable,
                                replacedPhis_[j].Replacement, assignedVars);
                }
            }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void SSAConversion::PreserveTags(Operand* sourceOp, Operand* destOp) {
	auto temp = sourceOp->As<Temporary>();
	auto parameter = destOp->As<Parameter>();

	if((temp == nullptr) || (parameter == nullptr)) {
		return;
	}

	if(auto classTag = temp->GetTag<TypeClassTag>()) {
		if(parameter->HasTag<TypeClassTag>() == false) {
			parameter->AddTag(classTag);
		}
	}
}
...
void SSAConversion::CheckConditionalPropagation(Block* block, ConditionalInfo& info){
    ...
    if(auto loadInstr = conditionOp->DefiningInstrAs<LoadInstr>()) {
        info.Order = OrderType::NotEqual; // For the 'true' branch.
    ...
    if((info.LeftComparisonVariable && 
        info.LeftComparisonVariable->IsAddressTaken()) ||
       (info.RightComparisonVariable && 
        info.RightComparisonVariable->IsAddressTaken())) {
        info.Type = Condition_None;
    }
}
...
void SSAConversion::PerformConditionalPropagation(Block* block, Block* childBlock,
    ...
    if((childBlock == ifInstr->TrueTargetBlock() ||
        childBlock == ifInstr->FalseTargetBlock()) == false) {
        return;
    }
    ...
    if(info.Type == Condition_Single) {
        ...
        variable = info.LeftComparisonVariable;
        auto rightConstOp = info.ConstantOperand ? 
                            info.ConstantOperand->As<IntConstant>() : nullptr;
...
Variable* SSAConversion::GetLoadedLocal(LoadInstr* instr) {
    if(auto variableRef = instr->SourceOp()->As<VariableReference>()) {
        ...
        auto variable = variableRef->GetLocalVariable();

        if(variable->IsAddressNotTaken()) {
            return variable;
        }
    }
...
bool SSAConversion::IsCandidate(Operand* op) {
    ...
    return variableRef->GetLocalVariable()->IsAddressNotTaken();
}
...
void SSAConversion::ReplaceResult(Temporary* resultOp, Operand* newOp) {
    ...
    if(simplify) {
		AddUsersToWorklist(resultOp, worklist);
	}
