...
bool ScalarReplacementOfAggregates::IsEligibleCandidate(Variable* variable) {
	...
	// Even if it is taken, if it is used only be 'call' instructions 
	// valid code can still be generated around the call sites.
    if(variable->IsAddressTaken() &&
	   (IsTakenOnlyByCalls(variable) == false)) {
        return false;
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::IsTakenOnlyByCalls(Variable* variable) {
	// Check if the address is taken only by 'call' instructions.
	// If it escapes it is difficult to track it, so give up early.
	if(variable->IsEscape()) {
		return false;
	}

	// Use a worklist algorithm that processes all instructions that use
	// the variable directly or indirectly through a series of
	// addressing instructions or pointer casts.
	auto variableRef = variable->GetReference();
	WorkList worklist;

	if(BuildInitialWorklist(variableRef, worklist) == false) {
		return false;
	}
	...
	while(worklist.IsNotEmpty()) {
		...
		if(instr->HasDestinationOp()) {
			...
			destOp->ForEachUser([&, this](Instruction* user, int index) -> bool {
				if(IsEligibleUser(user, instr, variableRef)) {
					if(ShouldAddUserToWorklist(user)) {
						worklist.Add(user);
					}
				}
				else valid = false;
			...
			// Stop as soon as a invalid use is found.
			if(valid == false) {
				return false;
			}
		}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::BuildInitialWorklist(VariableReference* variableRef,
														 WorkList& worklist) {
	bool valid = true;

	funct_->ForEachInstruction([&, this](Instruction* instr) -> bool {
		if(instr->HasSourceOp(variableRef)) {
			if(IsEligibleUser(instr, nullptr /* source */, variableRef)) {
				if(ShouldAddUserToWorklist(instr)) {
					worklist.Add(instr);
				}
			}
			else valid = false;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::IsEligibleUser(Instruction* user, Instruction* source,
												   VariableReference* variableRef) {
	if(auto callInstr = user->As<CallInstr>()) {
		// The address of the first byte of the variable should be passed
		// to the function, other offsets are not handled properly.
		if(source) {
			return ComputesZeroOffset(source, variableRef);
		}
		else return true;
	}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::ComputesZeroOffset(Instruction* instr, 
													   VariableReference* variableRef) {
	if(auto ptopInstr = instr->As<PtopInstr>()) {
		if(ptopInstr->TargetOp() == variableRef) {
			return true;
		}
		else if(auto definingInstr = ptopInstr->TargetOp()->DefiningInstruction()) {
			return ComputesZeroOffset(definingInstr, variableRef);
		}
	}
	else if(instr->IsAddressing()) {
		auto addrInstr = static_cast<AddressInstr*>(instr);

		if(addrInstr->IndexOp()->IsZeroInt()) {
			if(addrInstr->BaseOp() == variableRef) {
				return true;
			}
			else if(auto definingInstr = ptopInstr->TargetOp()->DefiningInstruction()) {
				return ComputesZeroOffset(definingInstr, variableRef);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::ShouldAddUserToWorklist(Instruction* instr) {
	return (instr->IsLoad() == false)  &&
		   (instr->IsStore() == false) &&
		   (instr->IsCall() == false);
}
...
int ScalarReplacementOfAggregates::ComputeRequiredVariables(const Type* type) {
    if(auto recordType = type->As<RecordType>()) {
        ...
        return (int)arrayType->Size() * elemVars;
    }
    else return 1;
}
...
void ScalarReplacementOfAggregates::ProcessCall(CallInstr* instr) {
    ...
    // Check if any argument represents a record that is passed
    // by value (i.e it doesn't escape the called function).
...
void ScalarReplacementOfAggregates::ProcessCopyMemory(CallInstr* instr) {
	...
	bool valid = (intConst != nullptr) && 
				 ((int)intConst->Value() == intConst->Value());
    ...
    bool hasDestPath = CreateAccessPath(CopyMemoryIntr::GetDestination(instr),
                                        destPath, destVariable, destOffset);
    bool hasSourcePath = CreateAccessPath(CopyMemoryIntr::GetSource(instr),
    ...
    for(int i = 0; i < 2; i++) {
        ...
        __int64 offset = i == 0 ? destOffset : sourceOffset;
        ...
        if(variable) {
            ...
            if(valid) {
                ...
                valid = (offset + (int)intConst->Value()) <= varSize;
            }
    ...
    if(valid && hasDestPath) {
        ...
        access.Size = (int)intConst->Value();
...
void ScalarReplacementOfAggregates::ProcessSetMemory(CallInstr* instr) {
    ...
    if(valid) {
        ...
        valid = (offset + (int)intConst->Value()) <= destVariableSize;
    }
...
bool ScalarReplacementOfAggregates::CreateAccessPath(Operand* op, AccessPath& path,
                                                     Variable*& variable, __int64& offset,
                                                     bool usedByAddr) {
    // An access path is composed from a series of 'index' and 'field'
    ...
    if(auto variableRef = op->As<VariableReference>()) {
        ...
        if(variable = variableRef->GetLocalVariable()) {
        ...
        return CreateAccessPathForField(fieldInstr, path, variable, 
                                        offset, usedByAddr);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ScalarReplacementOfAggregates::CreateAccessPathForField(FieldInstr* fieldInstr, 
                                                             AccessPath& path, 
                                                             Variable*& variable, 
                                                             __int64& offset,
															 bool usedByAddr) {
    ...
    if(CreateAccessPath(fieldInstr->BaseOp(), path, 
                        variable, offset, usedByAddr)) {
        auto intConst = fieldInstr->IndexOp()->As<IntConstant>();
...
void ScalarReplacementOfAggregates::ReplaceAggregates() {
    ...
    for(int i = 0; i < candidates_.Count(); i++) {
        ...
        for(int j = 0; j < candidate.Accesses.Count(); j++) {
            auto& access = candidate.Accesses[j];
    ...
    for(int i = 0; i < accesses.Count(); i++) {
        ...
        if((last >= 0) && 
           (accesses[i].Path == hottnessList[last].Path)) {
            ...
            // because the scalarized components must probably be written back.
...
int ScalarReplacementOfAggregates::EstimateHottness(Access& access) {
    int hotness = access.Type == Access_Load ? LOAD_ACCESS_HOTTNESS :
    ...
    //! TODO: PROFILE INFO
    ProfileInfo profile;
    auto frequencyInfo = profile.GetDynamicExecutionFrequency(access.Source);

    if(frequencyInfo.IsValid()) {
        // Use the profile info, it's the most precise.
        return hotness * frequencyInfo.Frequency();
    }
...
void ScalarReplacementOfAggregates::ReplaceCopyAccess(Candidate& candidate) {
    ...
    for(int i = 0; i < copies.Count(); i++) {
        ...
        // Obtain the list of scalarized compnoents
        // that are affected by the copy.
        NodeList destList;
        NodeList sourceList;
        __int64 startDestOffset = 0;
        __int64 startSourceOffset = 0;
        ...
        __int64 finalSourceOffset = startSourceOffset + copyAccess.Size;
        ...
        for(int j = 0; j < destList.Count(); j++) {
            if(destList[j]->Processed == false) {
                ReloadReplacement(candidate.Base, 
                                  destList[j]->Path,
                                  destList[j]->Replacement,
                                  copyAccess.IntrinsicCall);
            }
        }

        // The original copy access can now be removes.
        CopyAccessReplaced(copyAccess);
        copyAccess.IntrinsicCall->RemoveFromBlock(true /* free */);
    }
}
...
void ScalarReplacementOfAggregates::CopyCommonComponents(NodeList& destList, 
                                                         NodeList& sourceList,
                                                         __int64 startDestOffset, 
                                                         __int64 startSourceOffset,
...
void ScalarReplacementOfAggregates::CopyRemainingComponents(NodeList& sourceList, 
    ...
    __int64 sourceOffset = startSourceOffset;
    __int64 destOffset = startDestOffset;
    ...
    for(int i = 0; i < sourceList.Count(); i++) {
        ...
        if(sourceNode->Offset > sourceOffset) {
            int copyAmount = int(sourceNode->Offset - sourceOffset);
        ...
        __int64 difference = (sourceNode->Offset - sourceOffset) +
                              sourceNode->Size;
    ...
    if(sourceOffset != finalSourceOffset) {
        int copyAmount = (int)(finalSourceOffset - sourceOffset);
        GenerateMemoryCopy(copyAccess, sourceOffset, 
                           destOffset, copyAmount,
                           copyAccess.IntrinsicCall);
    }
...
void ScalarReplacementOfAggregates::ReplaceSetAccess(Candidate& candidate, 
    ...
    __int64 destOffset = startNode->Offset;
    __int64 finalDestOffset = destOffset + setAccess.Size;

    for(int i = 0; i < destList.Count(); i++) {
        if(destList[i]->Offset > destOffset) {
            GenerateMemorySet(irGen.GetVariableRef(candidate.Base),
                              setConst, destOffset,
                              (int)(destList[i]->Offset - destOffset),
    ...
    if(destOffset != finalDestOffset) {
        GenerateMemorySet(irGen.GetVariableRef(candidate.Base),
                          setConst, destOffset,
                          (int)(finalDestOffset - destOffset),
                          setAccess.IntrinsicCall);
    }

    // The original set access can now be removed.
    CopyAccessReplaced(setAccess);
    setAccess.IntrinsicCall->RemoveFromBlock(true /* free */);
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ScalarReplacementOfAggregates::TreeNode* 
ScalarReplacementOfAggregates::CreateTree(const Type* type, __int64 offset, 
    ...
    if(auto recordType = type->As<RecordType>()) {
        ...
        for(int i = 0; i < recordType->FieldCount(); i++) {
            path.Indices[last] = i;
            __int64 childOffset = offset + recordType->GetFieldOffset(i);
        ...
        for(int i = 0; i < arrayType->Size(); i++) {
            path.Indices[last] = i;
            __int64 childOffset = offset + (elemSize * i);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void ScalarReplacementOfAggregates::AddInRegionToList(TreeNode* node, __int64 offset, 
                                                      __int64 size, NodeList& list) {
...
ScalarReplacementOfAggregates::GetNodeForAccessPath(Candidate& candidate, 
    ...
    for(int i = 0; i < path.Indices.Count(); i++) {
        node = node->Children[(int)path.Indices[i]];
    }
...
Operand* ScalarReplacementOfAggregates::GetAddressForPath(AccessPath& path, 
    ...
    for(int i = 0; i < indices.Count(); i++) {
        ...
        if(auto recordType = lastType->As<RecordType>()) {
            ...
            auto fieldType = recordType->GetFieldType(indices[i]);
            auto addressType = irGen.GetPointer(fieldType);
            auto newAddressOp = irGen.GetTemporary(addressType);

            auto instr = irGen.GetField(addressOp, indexConst, newAddressOp);
            block->InsertInstructionBefore(instr, beforeInstr);
...
void ScalarReplacementOfAggregates::GenerateMemoryCopy(CopyAccess& access, 
                                                       __int64 fromOffset, __int64 toOffset, 
...
bool ScalarReplacementOfAggregates::ImproveLoadFromPhi(LoadInstr* loadInstr, 
    ...
    for(int i = 0; i < phiInstr->OperandCount(); i++) {
        auto op = phiInstr->GetOperand(i);
        auto block = phiInstr->GetOperandBlock(i);

        // Load the incoming operand and add it to the new 'phi'.
        auto loadedOp = irGen.GetTemporary(loadedType);
        auto otherLoadInstr = irGen.GetLoad(op, loadedOp);
        block->InsertInstructionBefore(otherLoadInstr, block->LastInstruction());
        newPhiInstr->AddOperand(loadedOp, phiInstr->GetOperandBlockReference(i));
    }
