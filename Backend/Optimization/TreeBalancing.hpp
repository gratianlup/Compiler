...
private:
    ...
    public:
        ...
        bool IsSubtraction() const {
            return Operation == Opcode::Sub;
        }

        bool IsAddition() const {
            return Operation == Opcode::Add;
        }

        bool IsMultiplication() const {
            return Operation == Opcode::Mul;
        }

        virtual bool IsNegation() const override {
            return (Operation == Opcode::Sub) && Left->IsZero();
        }

        virtual bool IsOperation(Opcode operation) const override {
    ...
    public:
        ...
        virtual Constant* AsConstant() override {
            return Value->As<Constant>();
        }

        virtual IntConstant* AsIntConstant() override {
            return Value->As<IntConstant>();
        }

        virtual Parameter* AsParameter() override {
    ...
    typedef Dictionary<Instruction*, bool> ProcessedInstructionsDictionary;
    typedef Dictionary<IntConstant*, ValueExpression*> ConstantExpressionsDictionary;
    typedef Dictionary<Operand*, int> OperandRankDictionary;
    typedef IntArithmetic IA;

    //! TODO: Use controls!
    static const __int64 MAX_DISTRIBUTE_FACTOR = 8;
    static const __int64 LOAD_RANK_FACTOR = 5;
    static const __int64 CALL_RANK_FACTOR = 50;
    static const __int64 INTRINSIC_CALL_RANK_FACTOR = 2;
    static const __int64 NOSTATE_CALL_RANK_FACTOR = 2;
    static const __int64 NOSTATE_CALL_WITH_POINTERS_RANK_FACTOR = 5;
    static const __int64 NOSTATE_CALL_WITH_PARAMETER_POINTERS_RANK_FACTOR = 3;

    Function* funct_;
    ProcessedInstructionsDictionary processedInstrs_;
    ConstantExpressionsDictionary constValueExprs_;
    OperandRankDictionary operandRanks_;
    ...
    // Computes a factor used to multiply the instruction rank.
    // 'load' and 'call' instructions get a higher rank.
    int ComputeRankFactor(Instruction* instr);

    // Returns 'true' if any of the arguments are pointers.
    // 'allParameters' is set if all pointer arguments are parameters.
    bool HasPointerArguments(CallInstr* instr, bool& allParameters);
    ...
    // Constant-folds the specified operands using the opcode.
    Operand* FoldConstants(Opcode opcode, Operand* leftOp, Operand* rightOp);
    ...
    bool HasLowerRank(Expression* a, Expression* b);

    int GetRank(Operand* op);
...
public:
    ~TreeBalancing() {
        // Delete all constant expressions.
