...
Operand* Peephole::HandleTrunc(TruncInstr* instr) {
	...
	if(auto result = folder_.FoldConversion(Opcode::Trunc, op, castType, 
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Trunc, op, castType)) {
		return result;
	}
...
#endif
		return LOG(ChangeExpressionType(op, castType, Opcode::Trunc));
	}
	...
	switch(instr->GetOpcode()) {
		...
		case Opcode::Ushr: {
            ...
            if(auto intConst = instr->GetSourceOp(1)->As<IntConstant>()) {
                ...
                int lastBit = (int)(op->GetType()->As<IntegerType>()->SizeInBits() -
									intConst->Value());
                if(lastBit >= firstBit) {
...
Operand* Peephole::ChangeExpressionType(Operand* op, const Type* newType, 
                                        Opcode opcode) {
	bool isSigned = opcode == Opcode::Sext;
...
Operand* Peephole::HandleZext(ZextInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Zext, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Zext, op, castType)) {
		return result;
	}
	...
	if(CanBeZeroExtended(op, castType, maskedBits)) {
		...
		auto resultOp = ChangeExpressionType(op, castType, Opcode::Zext);
...
Operand* Peephole::HandleZextOnCmp(CmpInstrBase* instr, const IntegerType* castType) {
	...
	if(auto intConst = instr->RightOp()->As<IntConstant>()) {
		...
		if(((instr->Order() == OrderType::Less) && intConst->IsZero()) ||
			// zext (a > -1), int32 -> (a >> 31) ^ 1, for signed comparison.
			// As above, it only tests the sign bit.
		   ((instr->Order() == OrderType::Greater) && intConst->IsMinusOne()) ||
...
bool Peephole::CanBeZeroExtendedInstructionUshr(Instruction* instr, const Type* zextType, 
    ...
    if(auto intConst = instr->GetSourceOp(1)->As<IntConstant>()) {
        maskedBits = masked + (int)intConst->Value();
...
Operand* Peephole::HandleSext(SextInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Sext, op, castType, 
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Sext, op, castType)) {
		return result;
	}
...
#endif
		auto resultOp = ChangeExpressionType(op, castType, Opcode::Sext);
	...
	if(Match<ShrInstr>(Match<ShlInstr>(MatchConversion<TruncInstr>(MatchAny(&a)),
	   MatchIC(&C1)), MatchIC(&C2))(op)) {
		...
		if((C1 == C2) && (a->GetType() == castType)) {
			...
			int bits = endBits - startBits + (int)C1->Value();
...
Operand* Peephole::HandleFtrunc(FtruncInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Ftrunc, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Ftrunc, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleFext(FextInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Fext, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Fext, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleFtoi(FtoiInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Ftoi, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Ftoi, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleItof(ItofInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Itof, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Itof, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleUitof(UitofInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Uitof, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Uitof, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleFtoui(FtouiInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Ftoui, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Ftoui, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandleItop(ItopInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Itop, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}
	...
	if(auto result = HandleCastCast(Opcode::Itop, op, castType)) {
		return result;
	}
...
Operand* Peephole::HandlePtoi(PtoiInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Ptoi, op, castType,
                                            instr->ParentBlock())) {
		return LOG(result);
	}
	...
	if(auto result = HandleCastCast(Opcode::Ptoi, op, castType)) {
		return result;
	}
	...
	else if(Match<FieldInstr>(MatchAny(&base), MatchInt(0))(op)) {
	...
	// This also works with 'index' and 'field' instructions.
	const Type* ptopType;
	...
	if(Match<AddressInstr>(MatchConversion<PtopInstr>(
	   ...
	   MatchAny(&base), nullptr, &ptopType), MatchIC(&C))(op) ||
	   Match<FieldInstr>(MatchConversion<PtopInstr>(
	   MatchAny(&base), nullptr, &ptopType), MatchIC(&C))(op)) {
        // We have a match.
		auto baseType = base->GetType()->As<PointerType>();
		bool isElement = op->DefiningInstrIs<FieldInstr>();
		__int64 offset;

		if(op->DefiningInstrIs<FieldInstr>()) {
			// The base is a record, use the offset of the the field.
			auto recordType = baseType->PointeeType()->As<RecordType>();
			DebugValidator::IsSmaller(C->Value(), recordType->FieldCount());
			offset = recordType->Fields()[(int)C->Value()].FieldOffset;
		}
...
Operand* Peephole::HandlePtop(PtopInstr* instr) {
	...
	// Try to fold it to a constant.
	if(auto result = folder_.FoldConversion(Opcode::Ptop, op, instr->CastType(),
                                            instr->ParentBlock())) {
		return LOG(result);
	}

	// Check for the cast-cast case, it's pretty common.
	if(auto result = HandleCastCast(Opcode::Ptop, op, instr->CastType())) {
		return result;
	}
...
Operand* Peephole::HandleCastCastTrunc(ConversionInstr* innerCast, const Type* castType) {
    ...
    if(innerCast->Is<TruncInstr>()) {
        ...
        // 'trunc a, intX', if 'X == Y'
        innerType = innerCast->CastType()->As<IntegerType>();
        outerType = castType->As<IntegerType>();
