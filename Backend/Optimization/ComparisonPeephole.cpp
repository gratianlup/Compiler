...
Operand* Peephole::HandleCmp(CmpInstr* instr) {
	...
	if(auto result = folder_.FoldCompare(Opcode::Cmp, instr->LeftOp(), 
    ...
    if(auto tag = instr->ParentBlock()->GetTag<RangeTag>()) {
        if(instr->IsLess()) {
            ...
            if(result != RangeResult::Maybe) {
                return LOG(GetBool(result == RangeResult::Yes, instr->ResultOp()));
            }
        }
...
Operand* Peephole::HandleUcmp(UcmpInstr* instr) {
	// We use the constant evaluator even if not both operands are constant,
	// because it also eliminates very simple cases, like 'a & 0 -> 0'.
	if(auto result = folder_.FoldCompare(Opcode::Ucmp, instr->LeftOp(), 
										 instr->RightOp(), instr->Order(),
                                         instr->ParentBlock())) {
		return LOG(result);
	}
...
Operand* Peephole::HandleFcmp(FcmpInstr* instr) {
	// We use the constant evaluator even if not both operands are constant,
	// because it also eliminates very simple cases, like 'a & 0 -> 0'.
	if(auto result = folder_.FoldCompare(Opcode::Fcmp, instr->LeftOp(), 
										 instr->RightOp(), instr->Order(),
                                         instr->ParentBlock())) {
		return LOG(result);
	}
	...
	if(isSigned) {
		...
		if(FA::IsSmaller((double)max, floatConst->Value(), floatKind) ||
		   FA::IsLarger((double)min, floatConst->Value(), floatKind)) {
			...
			if((order == OrderType::Equal) || (order == OrderType::Greater) ||
			   (order == OrderType::GreaterOrEqual)) {
	...
	switch(order) {
		...
		case OrderType::LessOrEqual: {
			...
			if((isSigned == false) && FA::IsNegative(floatConst)) {
				...
				auto constantOp = irGen_.GetIntConst(intType, floatToInt);
				OrderType otherOrder = FA::IsNegative(floatConst) ?
                                       OrderType::Less : OrderType::LessOrEqual;

				return LOG(CreateCompare(itofInstr->TargetOp(), constantOp, otherOrder,
									     instr->ResultOp()->GetType(), 
                                         isSigned == false));
			}
		}
		...
		case OrderType::GreaterOrEqual: {
			...
			if((isSigned == false) && FA::IsNegative(floatConst)) {
				...
				auto constantOp = irGen_.GetIntConst(intType, floatToInt);
				OrderType otherOrder = FA::IsNegative(floatConst) ?
                                       OrderType::GreaterOrEqual : OrderType::Greater;

				return LOG(CreateCompare(itofInstr->TargetOp(), constantOp, 
                                         otherOrder, instr->ResultOp()->GetType(), 
                                         isSigned == false));
			}
		}
    ...
    if(auto C = AsIntConstant(opA, instr->ParentBlock())) {
        if(instr->IsLessOrEqual()) {
            ...
            auto constantOp = irGen_.GetIntConst(C->GetType(), 
                                                 IA::Sub(C, GetOneInt(C)));
            instr->SetLeftOp(constantOp);
            instr->SetOrder(OrderType::LessOrEqual);
        }
            ...
            auto constantOp = irGen_.GetIntConst(C->GetType(), 
                                                 IA::Sub(C, GetOneInt(C)));
            instr->SetLeftOp(constantOp);
            instr->SetOrder(OrderType::GreaterOrEqual);
        }
            ...
            auto constantOp = irGen_.GetIntConst(C->GetType(), 
                                                 IA::Sub(C, GetOneInt(C)));
            instr->SetLeftOp(constantOp);
            instr->SetOrder(OrderType::Greater);
        }
    }
        ...
        auto subC = AsIntConstant(subInstr->RightOp(), instr->ParentBlock());
        ...
        if(instr->IsLess()) {
            // a - C < b -> a - (C-1) <= b  
            auto constantOp = irGen_.GetIntConst(subC->GetType(), 
                                                 IA::Sub(subC, GetOneInt(subC)));
        ...
        auto addC = AsIntConstant(addInstr->RightOp(), instr->ParentBlock());
        ...
        if(instr->IsGreater()) {
            // a + C > b -> a + (C-1) >= b  
            auto constantOp = irGen_.GetIntConst(addC->GetType(), 
                                                 IA::Sub(addC, GetOneInt(addC)));
...
Operand* Peephole::HandleIntegerCmp(CmpInstrBase* instr) {
    ...
    if(instr->IsNotEqual() && IsZeroInt(opB, block) && 
       opA->HasDefiningInstruction() && opA->DefiningInstruction()->IsComparison()) {
	...
	// ((a & 1) << 31) != 0 -> a & 1 != 0
	// ((a & 1) << 31) == 0 -> a & 1 == 0
	// This kind of code appears when bitfields are used like this:
	// if(a.b) ..., where b is a bitfield.
	if((order == OrderType::Equal) || 
	   (order == OrderType::NotEqual)) {
		IntConstant* andConst;
		IntConstant* shlConst;

		if(Match<ShlInstr>(Match<AndInstr>(MatchAny(&a), MatchIC(&andConst)),
						   MatchIC(&shlConst))(opA) && IsZeroInt(opB, block)) {
			int typeBits = TI::GetSizeBits(a->GetType());
			int rightmostBit = IA::RightmostSetBit(andConst);

			if((rightmostBit + shlConst->Value()) < typeBits) {
				// None of the bits are shifted out.
				auto andOp = opA->DefiningInstruction()->GetSourceOp(0);
				return LOG(CreateCompare(andOp, opB, order, 
	...
	if(opA->IsVariableReference() &&
       opB->IsVariableReference() && (opA != opB)) {
		switch(order) {
			case OrderType::Equal:
			case OrderType::LessOrEqual:
			case OrderType::GreaterOrEqual: {
...
Operand* Peephole::HandleIntegerCmpAddSubConst(Operand* opA, Operand* opB, 
	...
	if(arithInstrB == nullptr) {
        if(auto C2 = AsIntConstant(opB, block)) {
            return HandleIntegerCmpAddSubConst(arithInstrA->LeftOp(), C1, C2, 
                                               resultOp, order, arithInstrA->IsAdd());
        }
        else return nullptr;
    }
    else if(arithInstrB->HasUndefinedOverflow() == false) {
        return nullptr;
    }
    ...
    bool sameOpcode = arithInstrA->GetOpcode() == arithInstrB->GetOpcode();
    ...
    if(overflow == false) {
        ...
        if(std::abs(value) < std::abs(IA::LimitToType(C2))) {
            // We can perform the simplification.
            auto addSubOp = GetTemporary(C1);
            auto constantOp = irGen_.GetIntConst(C1->GetType(), value);

            if(rightAdd) irGen_.GetAdd(b, constantOp, addSubOp);
            else irGen_.GetSub(b, constantOp, addSubOp);
            
            return LOG(CreateCompare(a, addSubOp, order, 
                                     addSubOp->GetType(), false));
        }
    }
    ...
    if(overflow == false) {
        ...
        if(std::abs(value) < std::abs(IA::LimitToType(C1))) {
            // We can perform the simplification.
            auto addSubOp = GetTemporary(C1);
            auto constantOp = irGen_.GetIntConst(C1->GetType(), value);

            if(leftAdd) irGen_.GetAdd(a, constantOp, addSubOp);
            else irGen_.GetSub(a, constantOp, addSubOp);
           
            return LOG(CreateCompare(addSubOp, b, order, 
                                     addSubOp->GetType(), false));
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::HandleIntegerCmpAddSubConst(Operand* nonConstOp, IntConstant* intConst,
                                               IntConstant* otherIntConst, Operand* resultOp, 
                                               OrderType order, bool isAdd) {
    // NCO + IC ORDER OIC -> NCO ORDER OIC - IC
    // NCO - IC ORDER OIC -> NCO ORDER OIC + IC
    bool hasOverflow = isAdd ? IA::SubOverflows(otherIntConst, intConst) :
                               IA::AddOverflows(otherIntConst, intConst);

    if(hasOverflow) {
        return nullptr;
    }

    __int64 value = isAdd ? IA::Sub(otherIntConst, intConst) :
                            IA::Add(otherIntConst, intConst);
    auto constantOp = irGen_.GetIntConst(intConst->GetType(), value);

    return LOG(CreateCompare(nonConstOp, constantOp, order,
                             resultOp->GetType(), false));
}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::HandleIntegerCmpPtr(Operand* opA, Operand* opB, 
									   Operand* resultOp, OrderType order) {
		...
		if(auto ptoiInstrB = opB->DefiningInstrAs<PtoiInstr>()) {
			...
			if(ptrBits == intBits) {
				...
				if(targetTypeA != targetTypeB) {
					// Convert 'A' to 'B'.
					targetA = irGen_.GetTemporary(targetTypeB);
	...
	// Test for the comparison of two pointers that are converted 
	// to another pointer. If the original types are the same we can
	// eliminate the cast. Actually we could always eliminate the casts 
	// (because we compare the address of the pointers, which doesn't depend 
	// on their type) but don't do it because it interferes with the rules 
	// of the IR language, and these casts don't generate code anyway.
...
Operand* Peephole::HandleIntegerCmpBin(Operand* opA, Operand* opB, Operand* resultOp, 
	...
	if(Match<AddInstr>(MatchAny(&a), MatchAny(&b))(opA)) {
		if(((order == OrderType::Equal) || 
            (order == OrderType::NotEqual)) == false) {
			return nullptr;
		}
       ...
       (b == opB)) {
		...
		switch(order) {
			...
			case OrderType::LessOrEqual: {
				...
				// (a % -5) < -5 -> false
				return LOG(CreateCompare(b, GetZeroInt(b), OrderType::GreaterOrEqual,
									     resultOp->GetType(), false));
			}
			case OrderType::Greater:
			case OrderType::GreaterOrEqual: {
				...
				return LOG(CreateCompare(b, GetZeroInt(b), OrderType::Less,
									     resultOp->GetType(), false));
			}
		}
	}
...
Operand* Peephole::HandleIntegerCmpMul(MulInstr* mulInstrA, MulInstr* mulInstrB, 
									   Operand* resultOp, OrderType order, 
                                       bool isSigned, Block* block) {
	if(((order == OrderType::Equal) || (order == OrderType::NotEqual)) == false) {
		return nullptr;
	}
...
Operand* Peephole::HandleIntegerCmpEquality(Instruction* instr, IntConstant* intConst, 
											Operand* resultOp, OrderType order, 
											bool isSigned, Block* block) {
	DebugValidator::IsTrue((order == OrderType::Equal) ||
                           (order == OrderType::NotEqual));
	Operand* a;
	Operand* b;
	...
	switch(instr->GetOpcode()) {
		case Opcode::Add: {
			...
			if(auto addConst = AsIntConstant(addInstr->RightOp(), block)) {
				...
				auto constantOp = folder_.FoldBinary(Opcode::Sub, intConst, 
                                                     addConst, block);
        ...
        case Opcode::Sub: {
            ...
            if(auto subConst = AsIntConstant(subInstr->RightOp(), block)) {
				// We can fold the constants.
				auto constantOp = folder_.FoldBinary(Opcode::Add, intConst, 
                                                     subConst, block);
...
Operand* Peephole::HandleIntegerCmpSelf(Operand* opA, Operand* opB, Operand* resultOp, 
	...
	if(isSigned == false) {
		...
		if((order == OrderType::Less) || 
           (order == OrderType::LessOrEqual)) {
			return LOG(CreateCompare(a, constantOp, OrderType::Greater,
                                     resultOp->GetType(), true));
		}
		else {
			// 'greater' and 'greater or equal'.
			// For example, 'a + 2 > a' as long as 'a <= 255 - 2'.
			return LOG(CreateCompare(a, constantOp, OrderType::LessOrEqual, 
                                     resultOp->GetType(), true));
		}
	}
	...
	// For a language like C it is specified in the standard that
	// integers cannot overflow, so we are allowed to replace something
	// like 'a + 5 < a' with 'false' (this is done by GCC too).
	if((opA->DefiningInstrIs<AddInstr>() && 
		opA->DefiningInstrAs<AddInstr>()->HasUndefinedOverflow()) ||
	   (opB->DefiningInstrIs<AddInstr>() && 
		opB->DefiningInstrAs<AddInstr>()->HasUndefinedOverflow())) {
		if((order == OrderType::Less) || 
		   (order == OrderType::LessOrEqual)) {
			return GetBool(false);
		}
		else return GetBool(true); 
	}
	...
	switch(order) {
		case OrderType::Equal: {
			...
			if(lowOverflow && highOverflow) {
				...
				return LOG(CreateCompare(divInstr->LeftOp(), highConst, 
                                         OrderType::Less, resultOp->GetType(), 
                                         isSigned == false));
			}
				...
				return LOG(CreateCompare(divInstr->LeftOp(), lowConst, 
                                         OrderType::GreaterOrEqual, resultOp->GetType(),
                                         isSigned == false));
			}
			else {
		...
		case OrderType::LessOrEqual: {
			if(lowOverflow) {
				...
				auto lowConst = order == OrderType::LessOrEqual ?
		...
		case OrderType::GreaterOrEqual: {
			if(highOverflow) {
				...
				auto lowConst = order == OrderType::GreaterOrEqual ?
								irGen_.GetIntConst(intConst->GetType(), low) :
								irGen_.GetIntConst(intConst->GetType(), high);
...
__int64 Peephole::ComputeIntegerCmpDivBounds(ArithmeticInstr* divInstr, 
	...
	switch(order) {
		case OrderType::Less:           { order = OrderType::Greater;        break; }
		case OrderType::LessOrEqual:    { order = OrderType::GreaterOrEqual; break; }
		case OrderType::Greater:        { order = OrderType::Less;           break; }
		case OrderType::GreaterOrEqual: { order = OrderType::LessOrEqual;    break; }
	}
