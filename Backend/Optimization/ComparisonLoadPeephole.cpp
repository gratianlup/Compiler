...
//                          -> if((j == 1) && (i != 0)) -> if only one element remains
...
Operand* Peephole::HandleIntegerCmpLoad(LoadInstr* instr, IntConstant* intConst, 
	...
	bool isNotEqual = order == OrderType::NotEqual;
	...
	for(int i = 0; i < ranges.Count(); i++) {
		...
		if(isNotEquality) {
			...
			for(int j = 0; j < range.Count; j++) {
				auto constantOp = irGen_.GetIntConst(resultType, range.StartIndex + j);
				rangeOps.Add(CreateCompare(indexOp, constantOp, OrderType::Equal,
										   resultType, false));
			}
		}
		else { 
...
bool Peephole::IsLoadFromConstArray(LoadInstr* instr, GlobalVariable*& globalVar,
	...
	if((instr->SourceOp()->DefiningInstrIs<IndexInstr>()   ||
		...
		instr->SourceOp()->DefiningInstrIs<FieldInstr>()) == false) {
	    return false;
	}

	// The base operand should be a constant global variable with an initializer,
	// or, if allowed, a series of 'index'/'field' instructions that lead to a global
	...
	if(ShouldLoadFromConstStruct() && (addrInstr->IsAddress() == false)) {
		...
		while(true) {
			...
			if(auto fieldInstr = lastBaseOp->DefiningInstrAs<FieldInstr>()) {
				subindexList.Add(fieldInstr->GetFieldIndex());
				lastBaseOp = fieldInstr->BaseOp();
				lastWasIndex = false;
				level++;
			}
				...
				else if((int)intConst->Value() != intConst->Value()) {
					return false;
				}

				if(intConst) {
                    subindexList.Add((int)intConst->Value());
                }
	...
	globalVar = variableRef->GetGlobalVariable();
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void Peephole::AddToCurrentRange(__int64 value, __int64 requiredValue, int index,
	...
	if((order == OrderType::Equal) || (order == OrderType::NotEqual)) {
		...
		// a[i] > 8
		switch(order) {
			case OrderType::Less:           { match = value <  requiredValue; break; }
			case OrderType::LessOrEqual:    { match = value <= requiredValue; break; }
			case OrderType::Greater:        { match = value >  requiredValue; break; }
			case OrderType::GreaterOrEqual: { match = value >= requiredValue; break; }
...
bool Peephole::MatchesRequiredValue(__int64 value, __int64 requiredValue,
	...
	switch(order) {
		case OrderType::Equal:          { return resultValue <  requiredValue; break; }
		case OrderType::Less:           { return resultValue <  requiredValue; break; }
		case OrderType::LessOrEqual:    { return resultValue <= requiredValue; break; }
		case OrderType::Greater:        { return resultValue >  requiredValue; break; }
		case OrderType::GreaterOrEqual: { return resultValue >= requiredValue; break; }
		default: DebugValidator::Unreachable();
	}
...
Operand* Peephole::CreateRangeTest(ConstantRange& range, Operand* indexOp, bool isEquality,
	...
	if(isEquality) {
		if(range.Count == 1) {
			...
			return CreateCompare(indexOp, constantOp, OrderType::Equal, 
                                 resultType, false);
		}
		else {
			// i >= a && i <= b
            ...
            auto leftResult = CreateCompare(indexOp, leftConstOp, OrderType::GreaterOrEqual,
											resultType, false);
			auto rightResult = CreateCompare(indexOp, rightConstOp, OrderType::Less,
											 resultType, false);
		...
		if(range.Count == 1) {
			// i != a
			auto constantOp = irGen_.GetIntConst(type, range.StartIndex);
			return CreateCompare(indexOp, constantOp, OrderType::NotEqual, 
                                 resultType, false);
		}
		else {
			// i < a || i > b (>=)
            ...
            auto leftResult = CreateCompare(indexOp, leftConstOp, OrderType::Less,
											resultType, false);
			auto rightResult = CreateCompare(indexOp, rightConstOp, OrderType::GreaterOrEqual,
											 resultType, false);
