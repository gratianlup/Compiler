...
bool Peephole::HandleIf(IfInstr* instr) {
    ...
    Operand* a = nullptr;
    OrderType order = OrderType::Less;
    ...
    if(instr->ConditionOp()->HasDefiningInstruction() &&
       instr->ConditionOp()->DefiningInstruction()->IsComparison()) {
        ...
        // if (~a), T, F -> if a, F, T
        order = OrderType::Equal;
    }

    if((a && (order == OrderType::NotEqual)) ||
       (a && boolCmpWithOne && (order == OrderType::Equal))) {
        // Skip over the comparison, it's unnecessary.
        instr->SetConditionOp(a);
        return LOG(true);
    }
    else if((a && (order == OrderType::Equal)) || 
            (a && boolCmpWithOne && (order == OrderType::NotEqual))) {
    ...
    if(instr->ConditionOp()->HasDefiningInstruction() &&
       instr->ConditionOp()->DefiningInstruction()->IsComparison()) {
        ...
        if(cmpInstr->IsNotEqual() && cmpInstr->ResultOp()->HasSingleUser()) {
            cmpInstr->SetOrder(OrderType::Equal);
            InvertIfTargets(instr);
            return LOG(true);
        }
    }
    ...
    if(auto zextInstr = instr->ConditionOp()->DefiningInstrAs<ZextInstr>()) {
        ...
        instr->SetConditionOp(sextInstr->TargetOp());
        return LOG(true);
    }
...
void Peephole::InvertIfTargets(IfInstr* instr) {
    auto trueBlock = instr->TrueTargetBlock();
    auto falseBlock = instr->FalseTargetBlock();
...
bool Peephole::HandleSwitch(SwitchInstr* instr) {
    ...
    // Try to remove 'case's if the condition operand is a global variable
    // for which the possible constant values are known.
    if(HandleSwitchGlobalConstants(instr)) {
        return true;
    }

    // Try to remove 'case's if the condition operand is a parameter
    // for which the possible constant values are known.
    if(HandleSwitchParameterConstants(instr)) {
        return true;
    }
...
bool Peephole::HandleSwitchOnMod(SwitchInstr* switchInstr) {
    ...
    if(auto modInstr = conditionOp->DefiningInstrAs<ModInstr>()) {
        ...
        if(auto constantOp = umodInstr->RightOp()->As<IntConstant>()) {
            minValue = 0;
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool Peephole::HandleSwitchGlobalConstants(SwitchInstr* switchInstr) {
    // If the switch condition is a value loaded from a
    // global variable for which interprocedural analysis
    // determined that only some certain known constants are possible
    // we can eliminate all cases that cannot appear.
    auto loadInstr = switchInstr->ConditionOp()->DefiningInstrAs<LoadInstr>(); 
    bool changed = false;

    if((loadInstr == nullptr) ||
       (loadInstr->SourceOp()->IsGlobalVariableRef() == false)) {
        return changed;
    }
    ...
    // Remove any case that is not in the list of possible values.
    for(int i = 0; i < switchInstr->CaseCount(); i++) {
        __int64 caseValue = switchInstr->GetCase(i).Value;

        if(constantsTag->HasIntegerConstant(caseValue) == false) {
            switchInstr->RemoveCase(i);
            changed = true;
            i--;
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool Peephole::HandleSwitchParameterConstants(SwitchInstr* switchInstr) {
    // If the switch condition is a value loaded from a
    // parameter for which interprocedural analysis determined 
    // that only some certain known constants are possible
    // we can eliminate all cases that cannot appear.
    auto parameter = switchInstr->ConditionOp()->As<Parameter>();
    ...
    // It can be either a range or a list of constants.
    // A list of constants is more precise sometimes.
    bool changed = false;

    if(constantsTag->IsRange()) {
        __int64 low = constantsTag->GetRange().Low.Constant;
        __int64 high = constantsTag->GetRange().High.Constant;

        for(int i = 0; i < switchInstr->CaseCount(); i++) {
            __int64 caseValue = switchInstr->GetCase(i).Value;

            if((caseValue < low) || (caseValue > high)) {
                switchInstr->RemoveCase(i);
                changed = true;
                i--;
            }
...
bool Peephole::HandleSwitchOperandInfo(SwitchInstr* switchInstr) {
    ...
    __int64 minValue = opInfo_.GetMinimumValue(conditionOp, block);
    __int64 maxValue = opInfo_.GetMaximumValue(conditionOp, false, block);
