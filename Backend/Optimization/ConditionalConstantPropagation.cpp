...
ConstantLatticeCellCache::ConstantLatticeCellCache() {
    ...
    topCell_ = new ConstantLatticeCell(CellType::Top);
    bottomCell_ = new ConstantLatticeCell(CellType::Bottom);
}
...
bool ConstantLattice::TransferArithmeticLogical(Instruction* instr) {
    ...
    if(instr->IsArithmetic()) {
		return TransferArithmetic(instr, constCellA, constCellB);
	}
	else if (instr->IsLogical()) {
		return TransferLogical(instr, constCellA, constCellB)              ||
...
bool ConstantLattice::TransferArithmeticLogicalPhi(Instruction* instr,
    ...
    for(int i = 0; i < phiInstrA->OperandCount(); i++) {
        FlowEdge edge(phiInstrA->GetOperandBlock(i), block);
...
bool ConstantLattice::TransferLogical(Instruction* instr,
    ...
    if(instr->IsShift() && constCellB->IsConstant()) {
        ...
        // For 'shr' we need to know if the shifted operand
...
VariableReference* ConstantLattice::GetBaseGlobal(Operand* op, __int64& offset) {
    ...
    while(true) {
        if(auto variableRef = op->As<VariableReference>()) {
            ...
            int field = fieldInstr->GetFieldIndex();
            offset += fieldInstr->GetRecordType()->GetFieldOffset(field);
...
bool ConstantLattice::TransferCall(CallInstr* instr) {
    ...
    // because they appear pretty frequently.
    ...
    if(mathIntrinsic->IsAbs()) {
        ...
        __int64 abs = std::abs(intConst->Value());
...
#if 0
	//! TODO: use controls to activate region specialization.
