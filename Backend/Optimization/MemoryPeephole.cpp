...
//
// Implements peephole optimizations for 'index', 'field', 'addr'
...
Operand* Peephole::HandleAddress(AddressInstr* instr) {
    ...
    if(auto ptopInstr = baseOp->DefiningInstrAs<PtopInstr>()) {
        ...
        else if(auto fieldInstr = targetOp->DefiningInstrAs<FieldInstr>()) {
            return HandleAddrPtopField(ptopInstr, fieldInstr, 
                                       baseOp, indexOp);
        }
    ...
    else if(auto fieldInstr = baseOp->DefiningInstrAs<FieldInstr>()){
        return HandleAddrField(fieldInstr, baseOp, indexOp);
    }
...
Operand* Peephole::HandleAddrPtopRecord(PtopInstr* ptopInstr, 
                                        ...
                                        Operand* baseOp, Operand* indexOp) {
    // addr (ptop p {}*, X*), C1 -> field p, C2
    // struct ABC { int a,b,c; }; ABC *p; (char*)p + 4, (char*)p + 2
    // addr (ptop p, int8*), 4 -> ptop (field p, 1), int32 (&p->b)
    ...
    // Create the 'field' instruction with the new field index.
...
Operand* Peephole::HandleAddrIndex(IndexInstr* indexInstr, Operand* baseOp, 
    ...
    return LOG(CreateAddressOfElement(indexInstr->BaseOp(), IA::Add(C1, C2), 
                                      indexInstr->GetArrayType()));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::HandleAddrPtopField(PtopInstr* ptopInstr, FieldInstr* fieldInstr, 
                                       Operand* baseOp, Operand* indexOp) {
    // addr (ptop (field p, C1), X*), C2 -> ptop (field p, C3), X*
    // struct ABC { int a,b,c } p; (char*)&p.b + 4
    // addr (ptop (field p, 1), int8*), 4 -> field p, 2 ((char*)&p.c)
    auto block = ptopInstr->ParentBlock();
    auto C1 = AsIntConstant(fieldInstr->IndexOp(), block);
    auto C2 = AsIntConstant(indexOp, block);
    ...
    // Compute the offset relative to the start of the record.
    auto castType = ptopInstr->CastType()->As<PointerType>();
    auto recordType = GetPointedRecord(fieldInstr->BaseOp());
    auto field = recordType->Fields()[(int)C1->Value()];

    __int64 pointeeSize = TI::GetSize(castType->PointeeType(), GetTarget());
    __int64 offset = field.FieldOffset + (C2->Value() * pointeeSize);

    // See if there is a field at that offset. If there is, we can use
    // just the 'field' instruction with an updated field index.
    int newFieldIndex = FieldAtOffset(recordType, offset);
    ...
    // Create the 'field' instruction with the new field index.
    // Depending on which field was selected, the 'ptop' may be needed or not.
    auto elemOp = CreateAddressOfField(fieldInstr->BaseOp(),
                                       newFieldIndex, recordType);
    return LOG(InsertPtopIfRequired(elemOp, baseOp->GetType()));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::HandleAddrField(FieldInstr* fieldInstr, Operand* baseOp,
                                   Operand* indexOp) {
    // addr (field p, C1), C2 -> field p, C3, if 'C3' is a valid field index
    // struct ABC { int a,b,c; }; ABC a; &a.b + 1
    // addr (field p, 1), 1 -> field p, 2 (&a.c)
    ...
    auto block = fieldInstr->ParentBlock();
    auto C1 = AsIntConstant(fieldInstr->IndexOp(), block);
    auto C2 = AsIntConstant(indexOp, block);
    ...
    // addr (field p, 0), 2 -> field p, 3 (&p.d)
...
Operand* Peephole::CreateAddressOfField(Operand* baseOp, int fieldIndex, 
                                        const RecordType* recordType) {
    // Create the 'field' instruction with the specified field index.
    ...
    irGen_.GetField(baseOp, constantOp, elemOp);
    return elemOp;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::CreateAddressOfElement(Operand* baseOp, __int64 index, 
...
Operand* Peephole::HandleIndex(IndexInstr* instr) {
    ...
    // int (*p)[4] = &b.a; *p[2]
    // index (ptop (field a, 0), [4 int32]), 2 -> index (field a, 0), 2
    if(auto ptopInstr = baseOp->DefiningInstrAs<PtopInstr>()) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::HandleElement(FieldInstr* instr) {
    auto baseOp = instr->BaseOp();
    auto indexOp = instr->IndexOp();
    
    // field (ptop (addr p, C1), X{}), C2 -> field (ptop p, X{}), C3
    // This can be done only if we find an element at the computed offset.
    // struct ABC { int a,b }; char* p; ((ABC*)(p + 4))->a -> (ABC*)p->b
    // field (ptop (addr p, 4), <ABC>), 1 -> field (ptop p, <ABC>), 2
    if(auto ptopInstr = baseOp->DefiningInstrAs<PtopInstr>()) {
        ...
        __int64 offset = (C1->Value() * pointeeSize) + 
                         recordType->Fields()[(int)C2->Value()].FieldOffset;
        ...
        if(fieldIndex != -1) {
            ...
            // The result should have 'int32*' type, but the 'field' will have 'int16*'.
...
Operand* Peephole::HandleLoad(LoadInstr* instr) {
    ...
    if(GetSafetyInfo()->IsSafeToReorderLoad(instr) == false) {
        return nullptr;
    }
...
Operand* Peephole::HandleStore(StoreInstr* instr) {
    ...
    // Don't touch volatile operations.
    if(GetSafetyInfo()->IsSafeToReorderStore(instr) == false) {
        return nullptr;
    }
	...
	auto successorBlock = gotoInstr->TargetBlock();
