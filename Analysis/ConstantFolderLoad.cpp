...
Operand* ConstantFolder::HandleLoad(Operand* sourceOp) {
	...
	// Try to load from an 'addr', 'index' or 'field' instruction that index into
...
Operand* ConstantFolder::LoadFromAddress(Operand* op, const Type* loadType, 
	...
	switch(instr->GetOpcode()) {
		...
		case Opcode::Field: {
			auto fieldInstr = instr->As<FieldInstr>();
			__int64 index = fieldInstr->GetFieldIndex();

			if((int)index != index) {
				// The index is to large to process.
				return nullptr;
			}
			...
			__int64 fieldOffset = recordType->Fields()[(int)index].FieldOffset;
			__int64 newOffset = offset + fieldOffset;
			return LoadFromAddress(fieldInstr->BaseOp(), loadType, newOffset);
		}
		case Opcode::Address: {
			// If the index operand is not a constant give up.
...
bool ConstantFolder::IsValidOffset(GlobalVariable* globalVar, const Type* loadType,
	...
	if((offset + loadSize) > initSize) {
		...
		if((initializer->Conversion() == InitConversion::PointerToPointer) &&
...
Operand* ConstantFolder::LoadFromOffset(Initializer* initializer, const Type* sourceType,
	...
	if(initializer->IsInitializerList()) {
		...
		if(auto arrayType = sourceType->As<ArrayType>()) {
			...
			return LoadFromOffset((*initList)[(int)childIndex], arrayType->ElementType(),
...
Operand* ConstantFolder::LoadFromMultipleArray(InitializerList* initList, 
	...
	while(requestedBytes > 0) {
		...
		__int64 bytePos = firstIteration ? childOffset : 0LL;
		__int64 bytes;
		firstIteration = false;

		if(GetOperandData((*initList)[(int)childIndex]->Value(), temp, bytes) == false) {
...
Operand* ConstantFolder::LoadFromInitializer(Initializer* initializer, const Type* loadType, 
	...
	if(initializer->Conversion() == InitConversion::PointerToInt) {
		if(initializer->Value()->IsNullConstant()) {
			...
			(initializer->Conversion() == InitConversion::IntToPointer)) {
...
Operand* ConstantFolder::ExtractFromOperand(Operand* op, const Type* loadType, 
	...
	int loadSize = TypeInfo::GetSize(loadType, target_);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* ConstantFolder::LoadFromString(StringConstant* stringConst,
                                        const Type* loadType, __int64 offset) {
	...
	while(loadSize > 0) {
		data[dataPos] = (unsigned char)stringConst->Value()[(int)offset];
		dataPos++;
		offset++;
		loadSize--;
	}
