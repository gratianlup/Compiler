...
private:
    ...
    // Tries to evaluate an instruction that has 
    // at least one operand with a known range. 
    bool EvaluateWithRange(ArithmeticInstr* instr, RangeTag* tag, Range& result);

    bool EvaluateWithRange(ArithmeticInstr* instr, Range range, 
                           IntConstant* intConstant, RangeTag* tag, Range& result);

    bool EvaluateWithRange(ArithmeticInstr* instr, Range leftRange, Range rightRange,
                           RangeTag* tag, Range& result);

    // Tries to form a range from the range of the incoming operands.
    bool ExtractRangeFromPhi(PhiInstr* instr, RangeTag* tag, Range& result);

    // Tries to form a range from the ranges of the true/false operands.
    bool ExtractRangeFromQuestion(QuestionInstr* instr, RangeTag* tag, Range& result);
    ...
    bool ExtractRangeFromPossibleConstants(GlobalConstantsTag* constantsTag, Operand* op,
                                           Block* block, RangeTag* tag);
    ...
    // Tries to create ranges for all parameters using
    // information gathered during interprocedural analysis.
    void CreateParameterRanges(Function* function);

    void CreateRangeFromConstants(ParameterConstantsTag* constantsTag, 
                                  Function* function, int parameterIndex, RangeTag* tag);
