...
class Matcher {
protected:
	// Used by the derived classes to handle 
	// the instruction/operand that must be matched.
...
public:
	...
	// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	virtual bool IsInstructionMatcher() const override {
		return true;
	}

	virtual bool MatchImpl(Instruction* instr) override {
		...
		if(matcherA_.IsInstructionMatcher()) {
			if(opA->HasDefiningInstruction() == false) {
                return false;
            }
		...
		if(matcherB_.IsInstructionMatcher()) {
			if(opB->HasDefiningInstruction() == false) {
                return false;
            }
	...
	virtual bool MatchImpl(Operand* op) override {
...
public:
	MatchAny(Operand** matchedOp = nullptr) :
            matchedOp_(matchedOp) {}
...
public:
	MatchInt(__int64 value,  Block* testBlock = nullptr, Type* type = nullptr) : 
	...
	virtual bool MatchImpl(Operand* op) override {
		...
		if(intConst) {
			if(intConst->Value() != value_) {
                return false;
            }

			if(type_ && (intConst->GetType() != type_)) {
                return false;
            }
...
public:
	MatchFloat(double value,  Type* type = nullptr) : 
	...
	virtual bool MatchImpl(Operand* op) override {
		...
		if(auto floatConst = op->As<FloatConstant>()) {
			if(floatConst->Value() != value_) {
                return false;
            }

			if(type_ && (floatConst->GetType() != type_)) {
                return false;
            }
...
public:
	MatchIC(IntConstant** matchedOp = nullptr,
            Block* testBlock = nullptr, Type* type = nullptr) : 
...
public:
	MatchNullptr(NullConstant** matchedOp = nullptr) : 
            matchedOp_(matchedOp) {}
...
public:
	MatchUndef(UndefinedConstant** matchedOp = nullptr) : 
            matchedOp_(matchedOp) {}
