...
//
// Defines a tag that can be used to store information about the bits 
// of an operand that are known to be definitely set to one or zero.
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
public:
    ...
    static KnownBitsTag* unknownBitsTag_;
    ...
    virtual void Free() override {
        if(this != unknownBitsTag_) {
            delete this;
        }
    }

    // Creates a new tag having the known bits according to the specified masks.
    ...
    static KnownBitsTag* GetAllUnknownBits() {
        return unknownBitsTag_;
    }

    // Returns 'true' if none of the one/zero bits are known.
    bool IsAllUnknownBits() const {
        return this == unknownBitsTag_;
    }
    ...
    bool IsZeroBit(int index) {
        ...
        DebugValidator::IsSmaller(index, 64);
