...
// Defines the tag used to represent the fact that all bits are unknown.
KnownBitsTag* KnownBitsTag::unknownBitsTag_ = KnownBitsTag::GetKnownBits(0, 0);
...
int OperandInfo::GetVariableSize(const Operand* op, const TargetInfo* target) {
	...
	if(auto variableRef = op->As<VariableReference>()) {
        const Variable* variable = variableRef->IsLocalVariableRef() ? 
                                   variableRef->GetLocalVariable() : 
                                   variableRef->GetGlobalVariable();
...
int OperandInfo::GetStringLength(const Operand* op) {
	...
	// Make sure the array is not to large for a 32 bit integer.
	if((int)arrayType->Size() == arrayType->Size()) {
		return (int)arrayType->Size();
	}
	else return -1;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
__int64 OperandInfo::GetMaximumValue(Operand* op, bool isSigned, Block* testBlock) {
    DebugValidator::IsNotNull(op);
    ...
    else if(auto variableRef = GetBaseOperand(op)->As<VariableReference>()) {
        // For initialized constant variables we know precisely the maximum.
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            __int64 maxValue = std::numeric_limits<__int64>::min();

            if(GetMaximumGlobalValue(globalVariable, maxValue)) {
                return maxValue;
            }
        }
    }
    ...
    if(auto callInstr = op->DefiningInstrAs<CallInstr>()) {
        ...
        // Try to handle load from constant global variable.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::GetMaximumGlobalValue(GlobalVariable* globalVariable, 
                                        __int64& maxValue) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        ...
        return GetMaximumInitializer(globalVariable->GetInitializer(), maxValue, 
                                     MAXIMUM_INITIALIZER_DEPTH);
    }
        ...
        if(constantsTag->HasOnlyConstants() == false) {
            return false;
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::GetMaximumInitializer(Initializer* initializer, 
                                        __int64& maxValue, int depth) {
    ...
    if(initializer->IsInitializerList()) {
        ...
        for(int i = 0; i < initializerList->Count(); i++) {
            if(GetMaximumInitializer((*initializerList)[i],
                                      maxValue, depth - 1) == false) {
                return false;
            }
        }
    ...
    else if(auto intConstant = initializer->Value()->As<IntConstant>()) {
        maxValue = std::max(maxValue, intConstant->Value());
        return true;
    }
...
__int64 OperandInfo::GetMinimumValue(Operand* op, bool isSigned, Block* testBlock) {
    ...
    if(auto intConst = op->As<IntConstant>()) {
        ...
        // For initialized constant variables we know precisely the minimum.
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            __int64 minValue = std::numeric_limits<__int64>::max();

            if(GetMinimumGlobalValue(globalVariable, minValue)) {
                return minValue;
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::GetMinimumGlobalValue(GlobalVariable* globalVariable, 
                                        __int64& minValue) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        ...
        return GetMinimumInitializer(globalVariable->GetInitializer(), minValue, 
                                     MAXIMUM_INITIALIZER_DEPTH);
    }
    else if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
        // Some constant values of the variable are known.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::GetMinimumInitializer(Initializer* initializer, 
                                        __int64& minValue, int depth) {
    // Stop if the initializer has too many dimensions.
    if(depth == 0) {
        return false;
    }
    ...
    if(initializer->IsInitializerList()) {
        ...
        // Process each child initializer and stop as soon as one fails.
        for(int i = 0; i < initializerList->Count(); i++) {
            if(GetMinimumInitializer((*initializerList)[i],
                                      minValue, depth - 1) == false) {
                return false;
            }
        }
...
bool OperandInfo::IsPositive(Operand* op, Block* testBlock, int level) {
    ...
    if(op->IsFloating()) {
        ...
        // For initialized constant variables we know precisely the result.
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            if(IsPositiveGlobalValue(globalVariable)) {
                return true;
            }
        }
    }
    ...
    // 'a = [2, +INF)' -> true
    if(auto rangeTag = GetRangeTag(op, testBlock)) {
        if(rangeTag->IsPositive(op) == RangeResult::Yes) {
            return true;
        }
    }
    ...
    if(zeroBits & mask) {
        ...
        if(auto variableRef = loadInstr->SourceOp()->As<VariableReference>()) {
            if(auto globalVariable = variableRef->GetGlobalVariable()) {
                return IsPositiveGlobalValue(globalVariable);
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsPositiveGlobalValue(GlobalVariable* globalVariable) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        if(globalVariable->HasZeroInitializer()) {
        ...
        return IsPositiveInitializer(globalVariable->GetInitializer(), 
                                     MAXIMUM_INITIALIZER_DEPTH);
    }
    else if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
        // Some constant values of the variable are known.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsPositiveInitializer(Initializer* initializer, int depth) {
    // Stop if the initializer has too many dimensions.
    if(depth == 0) {
        return false;
    }
    ...
    if(initializer->IsInitializerList()) {
        ...
        // Process each child initializer and stop as soon as one fails.
        for(int i = 0; i < initializerList->Count(); i++) {
            if(IsPositiveInitializer((*initializerList)[i], depth - 1) == false) {
                return false;
            }
        }
...
bool OperandInfo::IsNegative(Operand* op, Block* testBlock, int level) {
    ...
    if(op->IsFloating()) {
        ...
        // For initialized constant variables we know precisely the result.
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            if(IsNegativeGlobalValue(globalVariable)) {
                return true;
            }
        }
    }
    ...
    // 'a = (-INF, -3]' -> true
    if(auto rangeTag = GetRangeTag(op, testBlock)) {
        if(rangeTag->IsNegative(op) == RangeResult::Yes) {
            return true;
        }
    }
    ...
    if(oneBits & mask) {
        ...
        if(auto variableRef = loadInstr->SourceOp()->As<VariableReference>()) {
            if(auto globalVariable = variableRef->GetGlobalVariable()) {
                return IsNegativeGlobalValue(globalVariable);
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsNegativeGlobalValue(GlobalVariable* globalVariable) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        if(globalVariable->HasZeroInitializer()) {
        ...
        return IsNegativeInitializer(globalVariable->GetInitializer(), 
                                     MAXIMUM_INITIALIZER_DEPTH);
    }
    else if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
        // Some constant values of the variable are known.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsNegativeInitializer(Initializer* initializer, int depth) {
    // Stop if the initializer has too many dimensions.
    if(depth == 0) {
        return false;
    }
    ...
    if(initializer->IsInitializerList()) {
        ...
        // Process each child initializer and stop as soon as one fails.
        for(int i = 0; i < initializerList->Count(); i++) {
            if(IsNegativeInitializer((*initializerList)[i], depth - 1) == false) {
                return false;
            }
        }
...
bool OperandInfo::IsZero(Operand* op, Block* testBlock, int level) {
    ...
    if(op->IsFloating()) {
        ...
        // For initialized constant variables we know precisely the result.
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            if(IsZeroGlobalValue(globalVariable)) {
                return true;
            }
        }
    }
    ...
    // 'a = [0, 0]' -> true
    if(auto rangeTag = GetRangeTag(op, testBlock)) {
        if(rangeTag->IsZero(op) == RangeResult::Yes) {
            return true;
        }
    }
    ...
    // Otherwise handle some instructions like 'phi' and 'quest'.
    ...
    if(auto callInstr = op->DefiningInstrAs<CallInstr>()) {
        // If we have a call to 'abs' we know that the result is zero
        // if the argument is zero (this should have been folded anyway).
        if(auto intrinsic = callInstr->GetIntrinsicAs<MathIntrinsic>()) {
            if(intrinsic->IsAbs()) {
        ...
        if(auto variableRef = loadInstr->SourceOp()->As<VariableReference>()) {
            if(auto globalVariable = variableRef->GetGlobalVariable()) {
                return IsZeroGlobalValue(globalVariable);
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsZeroGlobalValue(GlobalVariable* globalVariable) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        if(globalVariable->HasZeroInitializer()) {
        ...
        return IsZeroInitializer(globalVariable->GetInitializer(), 
                                 MAXIMUM_INITIALIZER_DEPTH);
    }
    else if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
        // Some constant values of the variable are known.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsZeroInitializer(Initializer* initializer, int depth) {
    // Stop if the initializer has too many dimensions.
    if(depth == 0) {
        return false;
    }
    ...
    if(initializer->IsInitializerList()) {
        ...
        // Process each child initializer and stop as soon as one fails.
        for(int i = 0; i < initializerList->Count(); i++) {
            if(IsZeroInitializer((*initializerList)[i], depth - 1) == false) {
                return false;
            }
        }
...
bool OperandInfo::IsNotZero(Operand* op, Block* testBlock, int level) {
    ...
    if(op->IsFloating()) {
        ...
        // Interprocedural analysis might have discovered
        // that the parameter can never be zero.
        auto constantsTag = parameter->GetVariable()->GetTag<ParameterConstantsTag>();

        if(constantsTag && 
           constantsTag->IsSafeToUse() && 
    ...
    if(auto rangeTag = GetRangeTag(op, testBlock)) {
        if(rangeTag->IsNotZero(op) == RangeResult::Yes) {
            return true;
        }
    }
    ...
    if(oneBits) {
        ...
        // both operands are definitely zero. Note that this can be done
        // only if overflow is undefined.
        ...
        if(auto variableRef = loadInstr->SourceOp()->As<VariableReference>()) {
            if(auto globalVariable = variableRef->GetGlobalVariable()) {
                return IsNotZeroGlobalValue(globalVariable);
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsNotZeroGlobalValue(GlobalVariable* globalVariable) {
    // If the variable is an initialized constant
    // the maximum can be computed exactly.
    if(globalVariable->IsConstant() && globalVariable->HasInitializer()) {
        if(globalVariable->HasZeroInitializer()) {
        ...
        return IsNotZeroInitializer(globalVariable->GetInitializer(), 
                                    MAXIMUM_INITIALIZER_DEPTH);
    }
    else if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
        // Some constant values of the variable are known.
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool OperandInfo::IsNotZeroInitializer(Initializer* initializer, int depth) {
    // Stop if the initializer has too many dimensions.
    if(depth == 0) {
        return false;
    }
    ...
    if(initializer->IsInitializerList()) {
        ...
        // Process each child initializer and stop as soon as one fails.
        for(int i = 0; i < initializerList->Count(); i++) {
            if(IsNotZeroInitializer((*initializerList)[i], depth - 1) == false) {
                return false;
            }
        }
...
int OperandInfo::GetProbableIntConstant(Operand* op, ValueList& valueList,
    ...
    if(auto intConst = GetIntConstant(op, testBlock)) {
        valueList.Add(ValueProbability(intConst, ProbabilityType::Always));
        return 1;
    }
...
int OperandInfo::ComputeProbableValues(Operand* op, ValueList& valueList,
    ...
    if(auto intConst = GetIntConstant(op, testBlock)) {
        ...
        // If the possible constants are known we can use them
        // to estimate the probable values.
        auto constantsTag = parameter->GetVariable()->GetTag<ParameterConstantsTag>();

        if(constantsTag && constantsTag->IsSafeToUse()) {
            float probability = 1.0f / constantsTag->UniqueConstantCount();

            constantsTag->ForEachUniqueConstant([&](Constant* constant) -> bool {
                valueList.Add(ValueProbability(constant, ProbabilityType::Percent, probability));
                return true;
            });
        }
    }
        ...
        if(auto globalVariable = variableRef->GetGlobalVariable()) {
            if(auto constantsTag = globalVariable->GetTag<GlobalConstantsTag>()) {
                // Create an object for each known constant,
                // whit its 
                for(int i = 0; i < constantsTag->ConstantCount(); i++) {
                    auto pair = constantsTag->GetConstantProbabilityPair(i);
                    valueList.Add(ValueProbability(pair.Value, ProbabilityType::Percent,
                                                   pair.Probability));
                }
            }
        }
...
int OperandInfo::ComputeProbableValuesRange(Operand* op, ValueList& valueList, 
    ...
    if((range.HasBaseOperand() == false) && 
        ...
        if(inRange <= limit) {
            ...
            float percent = 1.0f / (float)inRange;
            ...
            for(int i = 0; i < inRange; i++) {
                auto constant = unit_->Constants().GetInt(op->GetType(),
                                                          startValue + i);
                ValueProbability result(constant, ProbabilityType::Percent, percent);
...
void OperandInfo::ComputeProbability(ValueProbability& a, ValueProbability& b, 
    ...
    // P(result) = P(a) * P(b).
    if((a.Type == ProbabilityType::Always) &&
       (b.Type == ProbabilityType::Always)) {
        result.Type = ProbabilityType::Always;
    }
    else {
        float probabilityA = a.Type == ProbabilityType::Always ?
                             1.0f : a.Probability;
        float probabilityB = b.Type == ProbabilityType::Always ?
                             1.0f : b.Probability;
        result.Type = ProbabilityType::Percent;
...
int OperandInfo::ComputeProbableValuesQuestion(QuestionInstr* instr, ValueList& valueList,
    ...
    if(conditionList[0].Value->IsZeroInt()) {
        ...
        falseValues = std::min(falseValues, MAXIMUM_PROBABLE_VALUES);
        ...
        trueValues = std::min(trueValues, MAXIMUM_PROBABLE_VALUES);
...
float OperandInfo::EdgeProbability(Block* fromBlock, Block* toBlock) {
    ...
    else return 1.0f / (float)toBlock->PredecessorCount();
}
...
RangeResult OperandInfo::AreEqual(Operand* a, Operand* b, Block* testBlock) {
    ...
    if(auto intConstA = a->As<IntConstant>()) {
        if(auto intConstB = b->As<IntConstant>()) {
            return IA::AreEqual(intConstA, intConstB) ?
                   RangeResult::Yes : RangeResult::No;
        }
    }
    ...
    if(a->IsPointer()) {
        ...
        if(aIsNull && bIsNull) {
            ...
            return RangeResult::No;
        }
    }

    return RangeResult::Maybe;
}
...
RangeResult OperandInfo::AreNotEqual(Operand* a, Operand* b, Block* testBlock) {
    ...
    // Check if at least one of the operand is a parameter.
    // The possible values of the parameters might be known.
    if(a->IsParameter() || b->IsParameter()) {
        auto result = AreNotEqualParameters(a, b);
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
RangeResult OperandInfo::AreNotEqualParameters(Operand* a, Operand* b) {
    // If the possible values of the parameters are known
    // we can do a more precise 'not equal' test than using just the range.
    auto parameterA = a->As<Parameter>();
    auto parameterB = b->As<Parameter>();

    if(parameterA && parameterB) {
        return AreNotEqualParameters(parameterA, parameterB);
    }
    else if(b->IsIntConstant() && a->IsParameter()) {
        // Check if the order holds for each parameter constant/constant pair.
        auto parameter = a->As<Parameter>();
        auto intConstant = b->As<IntConstant>();

        if(auto tag = GetParameterConstants(parameter)) {
            bool orderHolds = true;

            tag->ForEachConstant([&](Constant* constant, FunctionReference* f) -> bool {
                if(auto otherIntConstant = constant->As<IntConstant>()) {
                    if(IntArithmetic::AreNotEqual(intConstant, otherIntConstant)) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
RangeResult OperandInfo::AreNotEqualParameters(Parameter* parameterA, 
                                               Parameter* parameterB) {
    auto tagA = GetParameterConstants(parameterA);
    auto tagB = GetParameterConstants(parameterB);

    // Both parameters should have a valid constants tag
    // and it shouldn't be too expensive to compute the result.
    if(((tagA && tagB) == false) ||
       ((tagA->ConstantCount() * tagB->ConstantCount()) > 32)) {
        return RangeResult::Maybe;
    }

    for(int i = 0; i < tagA->ConstantCount(); i++) {
        for(int j = 0; j < tagB->ConstantCount(); j++) {
            auto intConstantA = tagA->GetConstant(i)->As<IntConstant>();
            auto intConstantB = tagB->GetConstant(i)->As<IntConstant>();

            if((intConstantA == false) || (intConstantB == false) ||
                IntArithmetic::AreEqual(intConstantA, intConstantB)) {
                // The order doesn't hold for all pairs.
                return RangeResult::Maybe;
            }
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ParameterConstantsTag* OperandInfo::GetParameterConstants(Parameter* parameter) {
    DebugValidator::IsNotNull(parameter);

    auto variable = parameter->GetVariable();
    auto tag = variable->GetTag<ParameterConstantsTag>();

    if(tag && tag->IsSafeToUse() && tag->IsConstantList()) {
        return tag;
    }
    else return nullptr;
}
...
bool OperandInfo::IsPointerNotNull(Operand* op, Block* testBlock, bool handlePhi) {
    ...
    // any 'addr', 'index', 'field' and 'ptop' instructions).
