...
public:
	...
	static double LimitToType(double value, IRFloatingKind kind) {
		if(kind == IRFloatingKind::Float) return (float)value;
		else return value;
	}

	static double LimitToType(FloatConstant* value, const FloatingType* type) {
		return LimitToType(value->Value(), type->GetSubtype());
	}

	static double LimitToType(FloatConstant* value) {
	...
	static double Fadd(double a, double b, IRFloatingKind kind) {
		double result = kind == IRFloatingKind::Float ? (double)((float)a + float(b)) : a + b;
		return result;
	}
	...
	static double Ftrunc(double value, IRFloatingKind fromKind, IRFloatingKind toKind) {
		if(toKind == IRFloatingKind::Float) return (double)((float)value);
		else return value;
	}
	...
	static __int64 Ftoi(double value, IRFloatingKind fromKind, IRIntegerKind toKind) {
		...
		__int64 result = fromKind == IRFloatingKind::Float ? (__int64)((float)value) : (__int64)value;
	...
	static __int64 Ftoui(double value, IRFloatingKind fromKind, IRIntegerKind toKind) {
		// Round to single value if necessary.
		if(fromKind == IRFloatingKind::Float) {
			value = (float)value;
		}
		...
		return (__int64)IntArithmetic::LimitToType(result + (__int64)temp, toKind);
	}
	...
	static bool AreEqual(double a, double b, IRFloatingKind kind) {
		if(kind == IRFloatingKind::Float) return (float)a == (float)b;
		else return a == b;
	}
	...
	static int FractionSizeInBits(IRFloatingKind kind) {
		switch(kind) {
			case IRFloatingKind::Float:  return 23; // single precision
			case IRFloatingKind::Double: return 52; // double precision
	...
	static int ExponentSizeInBits(IRFloatingKind kind) {
		switch(kind) {
			case IRFloatingKind::Float: return 8;	// single precision
			case IRFloatingKind::Double: return 11;	// double precision
			default: DebugValidator::Unreachable();
		}
    ...
    static bool AreClose(double a, double b, IRFloatingKind kind) {
        if(kind == IRFloatingKind::Float) return std::fabs((float)a - float(b)) < 0.000001;
