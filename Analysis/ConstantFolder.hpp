...
private:
    ...
    typedef StaticList<IntConstant*, 4> IntConstantList;
	...
	Operand* GetBool(bool state, const Type* type = nullptr) {
		...
		return state ? GetOneInt(type) : GetZeroInt(type);
	}
	...
	Operand* HandleArithmetic(Opcode opcode, Operand* opA, 
							  Operand* opB, FloatMode mode = FloatMode::Exact,
                              Block* block = nullptr);
	...
	Operand* HandleCompareNullAddr(Opcode opcode, Operand* opA, 
                                   Operand* opB, OrderType order);
	
	Operand* HandleCompareAddrAddr(Opcode opcode, Operand* opA, 
                                   Operand* opB, OrderType order);
	
	Operand* HandleCompareFieldField(Opcode opcode, Operand* opA,
                                     Operand* opB, OrderType order);

    Operand* HandleCompareLoadLoad(Opcode opcode, Operand* opA, 
                                   Operand* opB, OrderType order);

    bool IsStoreCallBetweenLoads(LoadInstr* loadA, LoadInstr* loadB);

    Operand* HandleCompareGlobalLoad(Opcode opcode, Operand* opA, 
                                     Operand* opB, OrderType order);

    Operand* HandleCompareGlobalLoads(Opcode opcode, Operand* sourceA, 
                                      Operand* sourceB, OrderType order);

    GlobalConstantsTag* GetGlobalConstants(Operand* op);

    ParameterConstantsTag* GetParameterConstants(Operand* op);

    Operand* HandleCompareParameters(Opcode opcode, Operand* opA, 
                                     Operand* opB, OrderType order);

    Operand* HandleCompareParameters(Opcode opcode, Parameter* parameterA, 
                                     Parameter* parameterB, OrderType order);

    Operand* ComputeResultForAllPairs(IntConstantList& listA, 
                                      IntConstantList& listB, 
                                      Opcode opcode, OrderType order);

    bool OrderHoldsForAllPairs(IntConstantList& listA, 
                               IntConstantList& listB, 
                               OrderType order, bool isSigned);

    Operand* HandleComparePhi(Opcode opcode, Operand* opA,  
                              Operand* opB, OrderType order);
	...
	Operand* HandleAddress(Opcode opcode, Operand* baseOp,
	...
	Operand* HandleCall(CallInstr* callInstr, ArgumentList* replacementArgs);
    Operand* HandleIntrinsic(CallInstr* callInstr, Intrinsic* intrinsic, 
                             ArgumentList* replacementArgs);
    Operand* EvaluateAbs(CallInstr* callInstr, ArgumentList* replacementArgs);
    Operand* EvaluateMin(CallInstr* callInstr, ArgumentList* replacementArgs);
    Operand* EvaluateMax(CallInstr* callInstr, ArgumentList* replacementArgs);

    Operand* GetArgument(int index, CallInstr* callInstr,
                         ArgumentList* replacementArgs) {
        if(replacementArgs) {
            return (*replacementArgs)[index];
        }
        else return callInstr->GetArgument(index);
    }
    ...
    Block* SelectBlock(Block* testBlock, Instruction* instr) {
        return testBlock ? testBlock : instr->ParentBlock();
    }
...
public:
    ...
    // If 'testBlock' is specified the instruction is evaluated
    // is if it was defined in the block.
	Operand* Fold(Instruction* instr, Block* testBlock = nullptr);
	...
	Operand* FoldBinary(Opcode opcode, Operand* opA, Operand* opB, 
						Block* block = nullptr, FloatMode mode = FloatMode::Exact);
    ...
    // Tries to fold the specified 'call' instructions using
    // the arguments from the replacement list instead of the original ones.
    Operand* FoldCall(CallInstr* instr, ArgumentList* replacementArgs);
