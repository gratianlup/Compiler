...
void RangeTag::InstructionRemoved(Instruction* instr, Block* parent) {
    ...
    RemoveOperandRanges(instr->GetDestinationOp());
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RangeTag::ParameterRemoved(Variable* parameter, Function* parent) {
    RemoveOperandRanges(parent->GetParameter(parameter));
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void RangeTag::RemoveOperandRanges(Operand* op) {
    // Remove the ranges that apply to the block.
    for(int i = 0; i < ranges_.Count(); i++) {
        if(ReferesOperand(ranges_[i], op)) {
            ranges_.RemoveAt(i);
            i--;
        }
    }
    ...
    for(int i = 0; i < children_.Count(); i++) {
        ...
        children_[i]->RemoveOperandRanges(op);
    }
}
...
bool RangeTag::RangeQueryResult::GetResult(RangeQueryType queryType, 
                                           RangeResult& result) {
    switch(queryType) {
        case RangeQueryType::AreEqual:         return ResultFromValue(AreEqual, result);
        case RangeQueryType::AreNotEqual:      return ResultFromValue(AreNotEqual, result);
        case RangeQueryType::IsSmaller:        return ResultFromValue(IsSmaller, result);
        case RangeQueryType::IsSmallerOrEqual: return ResultFromValue(IsSmallerOrEqual,result);
        case RangeQueryType::IsGreater:        return ResultFromValue(IsGreater, result);
        case RangeQueryType::IsGreaterOrEqual: return ResultFromValue(IsGreaterOrEqual,result);
    }
...
void RangeTag::RangeQueryResult::SetResult(RangeQueryType queryType, 
                                           RangeResult result) {
    switch(queryType) {
        case RangeQueryType::AreEqual:         { AreEqual         = (unsigned short)result; break; }       
        case RangeQueryType::AreNotEqual:      { AreNotEqual      = (unsigned short)result; break; }    
        case RangeQueryType::IsSmaller:        { IsSmaller        = (unsigned short)result; break; }      
        case RangeQueryType::IsSmallerOrEqual: { IsSmallerOrEqual = (unsigned short)result; break; }
        case RangeQueryType::IsGreater:        { IsGreater        = (unsigned short)result; break; }      
        case RangeQueryType::IsGreaterOrEqual: { IsGreaterOrEqual = (unsigned short)result; break; }
    }
}
...
Range RangeTag::IntersectRangeWithAntiRange(Range range, Range antiRange, 
    ...
    if((IsGreater(antiRange.Low, range.Low) == RangeResult::Yes) &&
       (IsSmaller(antiRange.High, range.High) == RangeResult::Yes)) {
    ...
    if((IsSmaller(antiRange.Low, range.Low) == RangeResult::Yes) &&
       (IsGreater(antiRange.High, range.High) == RangeResult::Yes)) {
        return range;
    }
    ...
    if(IsGreater(antiRange.High, range.Low) == RangeResult::Yes) {
    ...
    if(IsSmaller(antiRange.Low, range.High) == RangeResult::Yes) {
...
bool RangeTag::HaveCommonPoint(Range a, Range b) {
    ...
    return (IsSmaller(a.Low, b.High) == RangeResult::Yes) &&
           (IsGreater(a.High, b.Low) == RangeResult::Yes);
}
...
RangeResult RangeTag::AreEqual(Operand* a, Operand* b, bool checkReverse, int level) {
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::Equal);
    }

    // Try to retrieve the result from the cache.
    RangeResult result;

    if(ResultFromCache(a, b, RangeQueryType::AreEqual, result)) {
        return result;
    }
    ...
    AddResultToCache(a, b, RangeQueryType::AreEqual, result);
    return result;
}
        ...
        if((range.Low.Base == other) &&
           (range.High.Base == other)) {
            ...
            if(checkReverse &&
               ...
               (range.High.Constant != 0) &&
               (IA::GetSignBit(range.Low.Constant, IRIntegerKind::Int64) ==
                IA::GetSignBit(range.High.Constant, IRIntegerKind::Int64))) {
                return RangeResult::No;
            }
        }
...
RangeResult RangeTag::AreNotEqual(Operand* a, Operand* b, bool checkReverse, int level) {
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::NotEqual);
    }

    // Try to retrieve the result from the cache.
    RangeResult result;

    if(ResultFromCache(a, b, RangeQueryType::AreNotEqual, result)) {
        return result;
    }
    ...
    AddResultToCache(a, b, RangeQueryType::AreNotEqual, result);
    return result;
}
...
RangeResult RangeTag::IsSmaller(Operand* a, Operand* b, bool isSigned,
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::Less, isSigned);
    }
    ...
    if(ResultFromCache(a, b, RangeQueryType::IsSmaller, result)) {
        return result;
    }
    ...
    if(hasResult) {
        AddResultToCache(a, b, RangeQueryType::IsSmaller, result);
        return result;
    }
...
RangeResult RangeTag::IsSmallerOrEqual(Operand* a, Operand* b, bool isSigned, 
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::LessOrEqual, isSigned);
    }
    ...
    if(ResultFromCache(a, b, RangeQueryType::IsSmallerOrEqual, result)) {
        return result;
    }
    ...
    if(hasResult) {
        AddResultToCache(a, b, RangeQueryType::IsSmallerOrEqual, result);
        return result;
    }
    ...
    else if(IsSmallerRangeWithRange(a, b, isSigned) == RangeResult::Yes) {
        // This response is conservative, but a 'No' response
        // is not because it doesn't include the <= case.
        result = RangeResult::Yes;
    }
    else result = RangeResult::Maybe;
...
RangeResult RangeTag::IsGreater(Operand* a, Operand* b, bool isSigned, 
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::Greater, isSigned);
    }
    ...
    if(ResultFromCache(a, b, RangeQueryType::IsGreater, result)) {
        return result;
    }
    ...
    if(hasResult) {
        AddResultToCache(a, b, RangeQueryType::IsGreater, result);
        return result;
    }
        ...
        if(range.Low.Constant != 0) {
            ...
            return IsGreaterRangeWithConst(range.Low.Base, intConst, isSigned,
                ...
                (IsSmallerRangeWithConst(range.Low.Base, intConst, 
                                         isSigned, false, level + 1) == RangeResult::Yes)) {
            return RangeResult::No;
        }
    }
...
RangeResult RangeTag::IsGreaterOrEqual(Operand* a, Operand* b, bool isSigned, 
    ...
    if(a->IsIntConstant() && b->IsIntConstant()) {
        return CompareConstants(a, b, OrderType::GreaterOrEqual, isSigned);
    }
    ...
    if(ResultFromCache(a, b, RangeQueryType::IsGreaterOrEqual, result)) {
        return result;
    }
    ...
    if(hasResult) {
        AddResultToCache(a, b, RangeQueryType::IsGreaterOrEqual, result);
        return result;
    }
    ...
    if(auto intConstB = b->As<IntConstant>()) {
        ...
        if(IA::AddOverflows(intConstA, oneConst) == false) {
            ...
            result = IsSmallerRangeWithConst(a, newConst, isSigned, 
                                    ...
                                    checkReverse, level) == RangeResult::Yes) {
        // This response is conservative, but a 'No' response
        // is not because it doesn't include the <= case.
        result = RangeResult::Yes;
    }
...
RangeResult RangeTag::IsNotZero(Operand* op, bool checkReverse) {
    ...
    if(AreNotEqual(op, zeroConst, false) == RangeResult::Yes) {
        return RangeResult::Yes;
    }
    else if(checkReverse &&
            AreEqual(op, zeroConst, false) == RangeResult::Yes) {
        return RangeResult::No;
    }
    else return RangeResult::Maybe;
}
...
RangeResult RangeTag::IsZero(Operand* op, bool checkReverse) {
    ...
    if(AreEqual(op, zeroConst, false) == RangeResult::Yes) {
        return RangeResult::Yes;
    }
    else if(checkReverse &&
            AreNotEqual(op, zeroConst, false) == RangeResult::Yes) {
        return RangeResult::No;
    }
    else return RangeResult::Maybe;
}
...
RangeResult RangeTag::IsPositive(Operand* op, bool checkReverse) {
    ...
    if(IsGreaterOrEqual(op, zeroConst, true, false) == RangeResult::Yes) {
        return RangeResult::Yes;
    }
    else if(checkReverse &&
            IsSmaller(op, zeroConst) == RangeResult::Yes) {
        return RangeResult::No;
    }
    else return RangeResult::Maybe;
}
...
RangeResult RangeTag::IsNegative(Operand* op, bool checkReverse) {
    ...
    if(IsSmaller(op, zeroConst, true, false) == RangeResult::Yes) {
        return RangeResult::Yes;
    }
    else if(checkReverse &&
            IsGreaterOrEqual(op, zeroConst) == RangeResult::Yes) {
        return RangeResult::No;
    }
    else return RangeResult::Maybe;
}
...
RangeResult RangeTag::ComparePhiRanges(PhiInstr* phiInstr, Operand* otherOp, 
    ...
    for(int i = 0; i < phiInstr->OperandCount(); i++) {
        ...
        else if(result == RangeResult::Yes) {
    ...
    return yesCount > 0 ? RangeResult::Yes : RangeResult::No;
}
...
RangeResult RangeTag::CompareConstants(Operand* opA, Operand* opB, 
    ...
    return RangeResult::No; // Unreachable.
}
