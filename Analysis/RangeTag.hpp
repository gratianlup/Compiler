...
struct Bound {
    ...
    static __int64 GetMinimum() {
        return IntArithmetic::GetMin(IRIntegerKind::Int64, true);
    }
    ...
    static __int64 GetMaximum() {
        return IntArithmetic::GetMax(IRIntegerKind::Int64, true);
    }
...
private:
    ...
    RangeResult ResultFromBool(bool value) {
        return value ? RangeResult::Yes : RangeResult::No;
    }
    ...
    // Removes all known ranges that contain the specified
    // operand (this is also done for any dominated block).
    void RemoveOperandRanges(Operand* op);
...
public:
    ...
    virtual void DetachedFromParent(TaggedBase* parent) override;

    virtual void InstructionRemoved(Instruction* instr, Block* parent) override;

    virtual void ParameterRemoved(Variable* parameter, Function* parent) override;
