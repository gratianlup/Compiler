...
public:
    ...
    // Performs the specified action on each calling site.
    // bool Predicate(CallSite* callSite)
    template <class Predicate>
    void ForEachCallingSite(Predicate action) {
        for(int i = 0; i < callingFuncts_.Count(); i++) {
    ...
    // Returns 'true' if the function is called only by external functions.
    bool IsCalledOnlyByExternalFunctions() const {
        return IsCalledByExternalFunctions() &&
               (CallingSitesCount() == 1);
    }
    ...
    // Returns 'true' if no call site (including the external one)
    // calls functions in this node
    bool IsNotCalledByFunctions() const {
        return callingFuncts_.Count() == 0;
    }
...
public:
    ...
    // Returns 'true' if the associated function is a definition.
    bool IsFunctionDefinition() const {
        return GetFunction()->IsDefinition();
    }

    // Returns 'true' if the associated function is a declaration.
    bool IsFunctionDeclaration() const {
        return GetFunction()->IsDeclaration();
    }

    // Returns 'true' if this node is the external node.
    virtual bool IsExternalNode() const override {
        return false;
    }

    // Returns 'true' if this node is the unknown node.
    virtual bool IsUnknownNode() const override {
        return false;
    }
    ...
    virtual int CalledFunctionsCount() const override;

    // Returns 'true' if any call site calls an external function.
    virtual bool CallsExternalFunctions() const override;

    // Returns 'true' if any call site calls an unknown function.
    virtual bool CallsUnknownFunctions() const override;

    virtual bool IsNodeGroup() const override {
        return false;
    }
};
...
class CallSite {
private:
    StaticList<CallEdge, 3> calledFuncts_; // The called functions.
...
public:
    ...
    // Returns the number of functions called by this call site.
    ...
    void ReplaceCalledFunction(CallNodeBase* previousNode, 
                               CallNodeBase* newNode) {
	...
	// Returns 'true' if any node group is called.
	bool CallsNodeGroup() const {
		for(int i = 0; i < calledFuncts_.Count(); i++) {
			auto callNodeBase = calledFuncts_[i].Callee();

			if(callNodeBase->IsNodeGroup()) {
				return true;
			}
		}
    ...
    // Performs the specified action on each called function.
    // bool Predicate(Function* function)
    template <class Predicate>
    void ForEachCalledFunction(Predicate action) {
        for(int i = 0; i < calledFuncts_.Count(); i++) {
			auto callNodeBase = calledFuncts_[i].Callee();
			auto callNode = static_cast<CallNode*>(callNodeBase);
			DebugValidator::IsFalse(callNode->IsNodeGroup());

			if(callNode->IsExternalNode()) {
				continue;
			}

            if(action(callNode->GetFunction()) == false) {
                // The user aborted.
                return;
            }
        }
    ...
    // Removes a call to the node that represents the specified function reference.
    ...
    // Removes a call to the node that represents the specified function.
    void RemoveCalledFunction(Function* function) {
        DebugValidator::IsNotNull(function);
        RemoveCalledFunction(function->GetReference());
    }

    // Removes a call to the External node.
    void RemoveCallToExternal() {
        RemoveCalledNode(ExternalCallNode::GetExternalNode());
    }

    // Removes a call to the Unknown node.
    void RemoveCallToUnknown() {
        RemoveCalledNode(UnknownCallNode::GetUnknownNode());
    }
...
public:
    ...
    virtual bool Visit(CallNode* node, CallGraph* callGraph) {
        return true;
    }

    // Called when a 'CallNodeGroup' is encountered.
    virtual bool Visit(CallNodeGroup* nodeGroup, CallGraph* callGraph) {
        return true;
    }
};
...
class CallGraph {
private:
    // Visitor used by the reverse-postorder algorithm
    // to collect all call graph nodes in postorder.
    ...
    public:
        ...
        virtual bool Visit(CallNodeGroup* nodeGroup, CallGraph* callGraph) override {
            nodes_.Add(nodeGroup);
            return true;
        }

        List<CallNodeBase*>& GetNodeList() {
            return nodes_;
        }
    };
    ...
    typedef Dictionary<CallSite*, CallSite*> CallSiteDict;
    ...
    CallNodeMap nodesInGroups_;      // The nodes that are part of a node group.
    CallSiteDict alwaysWithUnknown_; // The call sites for which it is not safe
    bool rootNodesComputed_;         // to remove the call to the Unknown node.
    ...
    bool VisitNode(CallNodeBase* node, CallNodeVisitor* visitor) {
        if(node->IsNodeGroup()) {
            return visitor->Visit(static_cast<CallNodeGroup*>(node), this);
        }
        else {
            auto callNode = static_cast<CallNode*>(node);

            if((callNode->IsExternalNode() || callNode->IsUnknownNode())  == false) {
                return visitor->Visit(callNode, this);
            }
    ...
    // Performs a breadth-first search starting with the specified node.
    void BreadthFirstSearchImpl(CallNodeList& startNodes, CallNodeVisitor* visitor);

    // Performs a postorder traversal starting with the specified node.
...
public:
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
    // Returns the list with all call nodes (does not include groups)..
    CallNodeList& GetCallNodes() {
        return nodes_;
    }

    // Returns the list with all call node groups.
    CallNodeGroupList& GetCallNodeGroups() {
        return groupNodes_;
    }

    // Returns the list with all call sites.
    CallSiteList& GetCallSites() {
        return callSites_;
    }

    // Removes the specified call site from the graph.
    // Note that the object is destroyed and memory freed.
    void RemoveCallSite(CallSite* callSite) {
        callSites_.Remove(callSite);
        callToCallSite_.Remove(callSite->GetCallId());
        delete callSite;
    }
    ...
    // Returns 'true' if the call to the 'unknown' node
    // can be removed by further analyses.
    bool CanUnknownBeRemoved(CallSite* callSite) const {
        return alwaysWithUnknown_.ContainsKey(callSite) == false;
    }

    // Adds the specified call site to the list of call sites
    // for which is not allowed to remove the call to the Unknown node.
    void AddAlwaysWithUnknown(CallSite* callSite) {
        DebugValidator::IsNotNull(callSite);
        DebugValidator::IsTrue(callSite->CallsUnknownFunctions());
        alwaysWithUnknown_.Add(callSite, callSite);
    }

    // Performs the specified action on each call node in the unit.
    // bool Predicate(CallNodeBase* node)
    template <class Predicate>
    void ForEachCallNode(Predicate action) {
        for(int i = 0; i < nodes_.Count(); i++) {
            if(action(nodes_[i]) == false) {
                return;
            }
    ...
    // Performs the specified action on each call node group in the unit.
    // bool Predicate(CallNodeBase* node)
    template <class Predicate>
    void ForEachCallNodeGroup(Predicate action) {
        for(int i = 0; i < groupNodes_.Count(); i++) {
            if(action(groupNodes_[i]) == false) {
                return;
            }
        }
    }
    ...
    // optionally considering only nodes called by the External node.
    // 'GetRootNode' can be used afterwards to retrieve the found nodes.
    int FindRoots(bool onlyCalledByExternal = false, bool recompute = false);
    ...
    // Returns 'true' if any root node has been found.
    bool HasRootNodes() const {
        return rootNodes_.Count() > 0;
    }

    // Returns 'true' if no root node has been found.
    bool HasNoRootNodes() const {
        return rootNodes_.Count() == 0;
    }

    // Returns 'true' if a single root node has been found.
    bool HasUniqueRootNode() const {
        return rootNodes_.Count() == 1;
    }

    bool HasNoUniqueRootNode() const {
        return rootNodes_.Count() > 1;
    }
    ...
    // Returns 'true' if the specified node is part
    // of a node group (i.e calling it might lead to recursion).
    bool IsNodeInGroup(CallNodeBase* node) {
        DebugValidator::IsNotNull(node);
        return nodesInGroups_.ContainsKey(node);
    }

    // Marks the specified node being part of a node group.
    void MarkNodeInGroup(CallNodeBase* node) {
        DebugValidator::IsNotNull(node);
        
        if(nodesInGroups_.ContainsKey(node) == false) {
            nodesInGroups_.Add(node, true);
        }
    }
    ...
    // The following methods are synonyms for 'PostorderTraversal'.
    void ReverseInvocationTraversal(CallNodeBase* startNode, 
                                    CallNodeVisitor* visitor) {
        PostorderTraversal(startNode, visitor);
    }

    bool ReverseInvocationTraversalFromRoots(CallNodeVisitor* visitor) {
        return PostorderTraversalFromRoots(visitor);
    }

    bool ReverseInvocationTraversalFromUniqueRoot(CallNodeVisitor* visitor) {
        return PostorderTraversalFromUniqueRoot(visitor);
    }

    // Performs a reverse-postorder traversal of the call graph
    // starting with the specified node.
    void ReversePostorderTraversal(CallNodeBase* startNode, 
                                   CallNodeVisitor* visitor);

    // Performs a reverse-postorder traversal of the call graph
    // starting with each of the root nodes.
    bool ReversePostorderTraversalFromRoots(CallNodeVisitor* visitor);

    // Performs a reverse-postorder traversal of the call graph
    // starting with the unique root node. If such a node
    // doesn't exist no search is performed and 'false' is returned.
    bool ReversePostorderTraversalFromUniqueRoot(CallNodeVisitor* visitor);

    // The following methods are synonyms for 'ReversePostorderTraversal'.
    void InvocationTraversal(CallNodeBase* startNode, 
                             CallNodeVisitor* visitor) {
        ReversePostorderTraversal(startNode, visitor);
    }

    bool InvocationTraversalFromRoots(CallNodeVisitor* visitor) {
        return ReversePostorderTraversalFromRoots(visitor);
    }

    bool InvocationTraversalFromUniqueRoot(CallNodeVisitor* visitor) {
        return ReversePostorderTraversalFromUniqueRoot(visitor);
    }
};
