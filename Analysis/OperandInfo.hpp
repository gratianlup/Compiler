...
#include "ParameterConstantsTag.hpp"
...
enum class ProbabilityType {
    Always, // The value is statically known.
    Percent // The operand might change at runtime, but it's known
...
struct ValueProbability {
    ...
    ValueProbability(Constant* value, ProbabilityType type = ProbabilityType::Always,
    ...
    float GetProbability() const {
        if(Type == ProbabilityType::Always) {
            return 1.0f;
        }
    ...
    bool operator< (const ValueProbability& other) const {
        if(Type != other.Type) {
            return Type == ProbabilityType::Always ? -1 : 1;
        }
        else if(Type == ProbabilityType::Percent) {
...
private:
    ...
    static const int MAXIMUM_INITIALIZER_SIZE = 32;
    static const int MAXIMUM_INITIALIZER_DEPTH = 4;
	...
	void EstimateZeroBitsImpl(Operand* op, Mask& bits, int depth);
    void EstimateZeroBitsParameter(Parameter* parameter, Mask& bits);
    ...
    void EstimateZeroBitsGlobal(GlobalVariable* globalVariable, Mask& bits, int depth);
    bool EstimateZeroBitsInitializer(Initializer* initializer, Mask& bits, int depth);
    bool EstimateZeroBitsConstant(Operand* op, Mask& bits, int depth);

    bool EstimateZeroBitsConstantsTag(GlobalConstantsTag* constantsTag,
                                      Mask& bits, int depth);
    ...
    void EstimateOneBitsImpl(Operand* op, Mask& bits, int depth);
    void EstimateOneBitsParameter(Parameter* parameter, Mask& bits, bool isNegative);
    ...
    void EstimateOneBitsGlobal(GlobalVariable* globalVariable, Mask& bits, int depth);
    bool EstimateOneBitsInitializer(Initializer* initializer, Mask& bits, int depth);
    bool EstimateOneBitsConstant(Operand* op, Mask& bits, int depth);

    bool EstimateOneBitsConstantsTag(GlobalConstantsTag* constantsTag,
                                     Mask& bits, int depth);

    bool TryTakeOneBitsFromCache(Operand* op, Mask& bits);
    bool IsOperandNegative(Operand* op, IntConstant*& intConst);

    // Methods for approximating the values of global variables.
    bool GetMaximumGlobalValue(GlobalVariable* globalVariable, __int64& maxValue);
    bool GetMaximumInitializer(Initializer* initializer, __int64& maxValue, int depth);

    bool GetMinimumGlobalValue(GlobalVariable* globalVariable, __int64& minValue);
    bool GetMinimumInitializer(Initializer* initializer, __int64& minValue, int depth);

    bool IsPositiveGlobalValue(GlobalVariable* globalVariable);
    bool IsPositiveInitializer(Initializer* initializer, int depth);

    bool IsNegativeGlobalValue(GlobalVariable* globalVariable);
    bool IsNegativeInitializer(Initializer* initializer, int depth);

    bool IsZeroGlobalValue(GlobalVariable* globalVariable);
    bool IsZeroInitializer(Initializer* initializer, int depth);

    bool IsNotZeroGlobalValue(GlobalVariable* globalVariable);
    bool IsNotZeroInitializer(Initializer* initializer, int depth);

    RangeResult AreNotEqualParameters(Operand* a, Operand* b);
    RangeResult AreNotEqualParameters(Parameter* parameterA, Parameter* parameterB);
    ParameterConstantsTag* GetParameterConstants(Parameter* parameter);
    ...
    // Creates a tag (if not already created) that caches the known bits.
    KnownBitsTag* AttachBitsTag(Temporary* temp, Mask knownBits);
...
public:	
    ...
    // Returns 'RangeResult::Yes' if, based on the known ranges,
    // it can be determined that the operands are definitely equal,
    // or 'RangeResult::No' if the operands are definitely not equal.
    ...
    // or 'RangeResult::No' if the operands are definitely equal.
    // Should be used only for integer and pointer operands.
    ...
    // or 'RangeResult::No' if 'a' is definitely greater than 'b'.
    // Should be used only for integer and pointer operands.
    ...
    // or 'RangeResult::No' if 'a' is definitely smaller than 'b'.
    // Should be used only for integer and pointer operands.
