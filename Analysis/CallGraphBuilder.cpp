...
bool CallGraphBuilder::IdentifyTargets(Operand* op, UnknownCall* unknownCall, 
    ...
    if(auto functionRef = op->As<FunctionReference>()) {
        ...
        // considered at a previous step.
        if(targetList == nullptr) {
            unknownCall->Targets.Add(UnknownTarget(functionRef, SINGLE_FUNCTION));
        }
        else targetList->Add(UnknownTarget(functionRef, SINGLE_FUNCTION));
        ...
        auto symbolTable = parameterVar->ParentTable();
        ...
        // Create the target.
        auto functionRef = function->GetReference();
        UnknownTarget unknownTarget(functionRef, parameterIndex);
        ...
        // Handle 'load', 'phi', 'index' and 'field'.
...
bool CallGraphBuilder::IdentifyTargets(Instruction* instr, UnknownCall* unknownCall, 
    ...
    else if(auto fieldInstr = instr->As<FieldInstr>()) {
        // Global function pointer records are often used
        // especially for programs targeted to multiple operating systems.
        if(auto variableRef = fieldInstr->BaseOp()->As<VariableReference>()) {
            __int64 index = fieldInstr->GetFieldIndex();
...
void CallGraphBuilder::CollectTargetsFromStore(StoreInstr* instr) {
    ...
    auto pointerType = instr->DestinationOp()->GetType()->As<PointerType>();

    if(pointerType->PointeeType()->IsPointer() == false) {
        // We're interested only in stores to pointers.
        return;
    }
    ...
    for(int i = 0; i < targetList.Count(); i++) {
        if(targetList[i].HasNonConstantIndex()) {
            ...
            MarkTargetUnknown(targetList[i]);
        }
    }
    ...
    if(IdentifyTargets(instr->SourceOp(), nullptr, &sourceTargetList) == false) {
        // No unknown target could be found, add the Unknown node
        // to the targets as a safety measure.
        DebugValidator::Unreachable();
        sourceTargetList.Add(UnknownTarget(nullptr, UNKNOWN_TARGET));
    }
    ...
    for(int i = 0; i < instr->ArgumentCount(); i++) {
        ...
        if(argument->IsPointer()) {
            ...
            if(callTarget == nullptr) {
                ...
                else if(IdentifyTargets(argument, nullptr, &argumentTargetList) == false) {
                    // No argument target could be found, add the Unknown node
                    // to the targets as a safety measure.
                    AddUnknownToParameters(callTargetList, i);
                    return;
                }
                ...
                if(IdentifyTargets(argument, nullptr, &argumentTargetList) == false) {
                    // No argument target could be found, add the Unknown node
                    // to the targets as a safety measure.
                    AddUnknownToParameter(callTarget, i);
                    return;
                }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void CallGraphBuilder::AddUnknownToParameter(FunctionReference* callTarget,
                                             __int64 argumentIndex) {
    // The call target is known, mark the fact that any call that uses
    // the parameter at position 'argumentIndex' also calls Unknown.
    UnknownTarget destTarget(callTarget, argumentIndex);
    AddNewTarget(destTarget);
    unknownTargets_.Add(destTarget, true);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void CallGraphBuilder::AddUnknownToParameters(UnknownTargetList& callTargetList,
                                              __int64 argumentIndex) {
    // The call target is not known and it depends on one or more
    // unknown targets. We create a special call binding that will be 
    // solved when all/some targets are known.
    UnknownTarget unknownTarget(nullptr, UNKNOWN_TARGET);

    for(int i = 0; i < callTargetList.Count(); i++) {
        CreateCallBinding(callTargetList[i], argumentIndex, unknownTarget);
    }
}
...
void CallGraphBuilder::CollectTargetsFromReturn(ReturnInstr* instr) {
    ...
    if(auto returnedFunctionRef = instr->ReturnedOp()->As<FunctionReference>()) {
        ...
        if(IdentifyTargets(instr->ReturnedOp(), nullptr, &returnedTargetList) == false) {
            // No unknown target could be found, add the Unknown node
            // to the targets as a safety measure.
            MarkTargetUnknown(returnedTarget);
            return;
        }
...
bool CallGraphBuilder::ProcessCallBinding(UnknownTarget sourceTarget) {
    ...
    for(int functIdx = 0; functIdx < functions.Count(); functIdx++) {
        for(int bindingIdx = 0; bindingIdx < callBinding.Arguments.Count(); bindingIdx++) {
            // Create the target for the formal parameter.
            UnknownTarget newTarget(functions[functIdx], callBinding.Arguments[bindingIdx]);
            AddNewTarget(newTarget);

            for(int paramIdx = 0; paramIdx < callBinding.Parameters.Count(); paramIdx++) {
                // Connect the formal parameter to its source.
                // If the parameter represents the Unknown node
                // mark 'newTarget' as being able to call it.
                if(callBinding.Parameters[paramIdx].IsUnknownTarget()) {
                    MarkTargetUnknown(newTarget);
                }
                else if(CreateBinding(callBinding.Parameters[paramIdx], newTarget)) {
                    bindingAded = true;
                }
            }
        }
...
void CallGraphBuilder::CompleteCallGraph() {
    ...
    for(int i = 0; i < unknownCalls_.Count(); i++) {
        ...
        for(int j = 0; j < unknownCall.Targets.Count(); j++) {
            ...
            if(unknownTargets_[unknownTarget]) {
        ...
        if(hasAddressTakenTarget || hasAmbiguousTarget || (functionCount == 0)) {
            ...
            // If not all targets could be figure out we should
            // not attempt to remove the call to the Unknown node.
            // If the call was added because one of the target had
            // its address taken we may be able with further analysis to prove
            // that the address it's actually not taken and remove the call.
            if(hasAmbiguousTarget || (functionCount == 0)) {
                callGraph_->AddAlwaysWithUnknown(callSite);
            }
        }
    }
}
...
bool CallGraphBuilder::CompleteUsingContext(UnknownTarget returnedTarget, 
    ...
    // Add only the functions associated with each argument
    ...
    for(int i = 0; i < paramReturn.ReturnedParameters.Count(); i++) {
        ...
        auto argument = instr->GetArgument((int)argumentIndex);

        if(auto argFunctionRef = argument->As<FunctionReference>()) {
            AddCall(callSite, argFunctionRef);
        }
        else {
            UnknownTargetList argumentTargetList;
...
bool CallGraphBuilder::AddNewTarget(UnknownTarget target, UnknownCall* unknownCall) {
    ...
    if(unknownTargets_.ContainsKey(target) == false) {
        unknownTargets_.Add(target, false);
    }
