...
Operand* Peephole::HandleOr(Operand* opA, Operand* opB, Block* block, 
	...
	if(auto result = folder_.FoldBinary(Opcode::Or, opA, opB, block)) {
		return LOG(result);
	}
	...
	if(auto result = HandleAssociative(Opcode::Or, opA, opB, block)) {
		return result;
	}

	// Try to simplify using distributivity rules.
	if(auto result = HandleDistributive(Opcode::Or, opA, opB, block)) {
		return result;
	}
	...
	if(Match<AndInstr>(MatchAny(&a), MatchIC(&C1, block))(opA) && 
       MatchIC(&C2, block)(opB)) {
		if(IA::And(C1, C2) == 0) {
			auto constantOp = folder_.FoldBinary(Opcode::Or, C1, C2, block);
	...
	if(auto result = HandleOperationOnQuest(Opcode::Or, opA, opB)) {
		return result;
	}

	// Check if we can simplify by applying the operation
	// one each incoming value of a 'phi' instruction.
	if(auto result = HandleInstructionOnPhi(Opcode::Or, opA, opB)) {
		return result;
	}
...
Operand* Peephole::HandleOrCmpCmp(Operand* opA, Operand* opB, Block* block) {
	...
	return LOG(CreateCompare(orOp2, GetZeroInt(a), OrderType::Equal, 
...
Operand* Peephole::HandleOrIntCmpConst(CmpInstrBase* leftCmp, CmpInstrBase* rightCmp,
	...
	case OrderType::Equal: {
		...
		case OrderType::Less: {   // (a == 8) | (a < 9)  -> a < 9
			return LOG(rightCmp->ResultOp());
		}
		case OrderType::Equal: {  // (a == 8) | (a == 9) -> (a - 8) < 2
			...
			return LOG(CreateCompare(subOp, twoConst, OrderType::Less,
									 leftCmp->ResultOp()->GetType(), true /* isUcmp */));
		}
		case OrderType::Greater: { // (a == 8) | (a >= 9) -> a >= 8
			// This transformation is safe only if the difference between
			// the two constants is 1.
			if(IsLeftRightMinusOne(leftConst, rightConst) == false) {
				return nullptr;
			...
			return LOG(CreateCompare(leftCmp->LeftOp(), leftCmp->RightOp(),
									 OrderType::GreaterOrEqual, leftCmp->ResultOp()->GetType(),
									 leftCmp->IsUcmp()));
		}
		}
		break;
	...
	case OrderType::NotEqual: {
		switch(rightCmp->Order()) {
		case OrderType::NotEqual:  // (a != 8) | (a != 9) -> true
		case OrderType::Less: {    // (a != 8) | (a < 9) -> true
			return LOG(GetBool(true, leftCmp->ResultOp()));
		}
		case OrderType::Equal:     // (a != 8) | (a == 10) -> a != 8
		case OrderType::Greater: { // (a != 8) | (a > 10)  -> a != 8
	...
	case OrderType::Greater: {
		switch(rightCmp->Order()) {
		case OrderType::Equal: {   // (a > 8) | (a == 9) -> a > 8
			return LOG(leftCmp->ResultOp());
		}
		case OrderType::Greater: { // (a > 8) | (a > 9)  -> a > 8
			// This is true only if both comparisons are of the same type.
		...
		case OrderType::Less: {     // (a > 8) | (a < 10) -> true
			// This is true only if both comparisons are of the same type.
			if(leftCmp->IsUcmp() == leftCmp->IsUcmp()) {
				return LOG(GetBool(true, leftCmp->ResultOp()));
			}
