...
private:
    ...
    bool AreFieldsDifferent(FieldInstr* fieldInstrA, FieldInstr* fieldInstrB,
                            Operand* baseOpA, Operand* baseOpB, int level = 8);

    // Returns the ancestor record.
    FieldInstr* GetAncestor(FieldInstr* fieldInstrA, FieldInstr* fieldInstrB);
    ...
    // (multiple levels of 'addr'/'field'/'index' instructions are allowed).
	...
	// Returns 'true' if the memory initialized by a 'setMemory'/'copyMemory' intrinsic
	// is overwritten before it is used by another memory intrinsic.
	bool IsMemoryIntrinsicDead(CallInstr* callInstr, Intrinsic* intrinsic);

	bool IsDestinationOverwritten(Operand* baseDestOp, __int64 baseDestOffset,
								  Operand* lengthOp, CallInstr* callInstr, 
								  Intrinsic* intrinsic);

	bool IsLargetOrEqual(Operand* opA, Operand* opB);

	// Returns 'true' if the 'load' source operand may alias 
	// the specified address operand.
	bool MayLoadReadFromAddress(LoadInstr* instr, Operand* addressOp);
    ...
    // Returns the base operand, if this is an 'addr', 'index' or 'field' instruction.
	...
	void LogCallRemoved(CallInstr* instr);
