...
void BlockCloner::CloneMultipleBlocks(BlockList& blockList, Function* destFunct,
									  OperandMap* variablesParamsMap, Block* entryBlock) {
	...
	// Copy the operand mappings if specified. Usually used
	// when cloning the blocks to another function.
	if(variablesParamsMap) {
		CopyOperanMappings(variablesParamsMap);
	}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Block* BlockCloner::CloneSingleBlock(Block* block) {
	DebugValidator::IsNotNull(block);
	BlockList blockList;

	blockList.Add(block);
	CloneMultipleBlocks(blockList);
	return blockMap_[block];
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BlockCloner::CloneFunctionBody(Function* sourceFunct, Function* destFunct,
									OperandMap* variablesParamsMap) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void BlockCloner::CopyOperanMappings(OperandMap* opMap) {
	opMap->ForEach([this](OperandMap::TPair& pair) -> bool {
		operandMap_.Add(pair.Key, pair.Value);
		return true;
	});
}
...
void BlockCloner::ProcessInstructions(Block* clonedBlock) {
    for(auto instr = clonedBlock->FirstInstruction(); instr;
        ...
        else ReplaceSourceOperands(instr);
    }
}
...
void BlockCloner::ProcessPhiInstruction(PhiInstr* instr) {
	...
	if(entryBlock_) {
		if((blockMap_.ContainsKey(entryBlock_) == false) ||
		   (instr->ParentBlock() == blockMap_[entryBlock_])) {
            return;
        }
    }
