...
Operand* Peephole::HandleAdd(Operand* opA, Operand* opB, Block* block,
	...
	if(auto result = folder_.FoldBinary(Opcode::Add, opA, opB, block)) {
        return result;
	}
    ...
    if(auto result = HandleAssociative(Opcode::Add, opA, opB, block)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Add, opA, opB, block)) {
        return result;
    }
    ...
    if(auto result = HandleOperationOnQuest(Opcode::Add, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Add, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleAddSimple(Operand* opA, Operand* opB, Block* block,
    ...
    if(Match<SubInstr>(MatchAny(&a), MatchIC(&C1))(opA) && MatchIC(&C2)(opB)) {
        auto constantOp = folder_.FoldBinary(Opcode::Sub, C2, C1, block);
    ...
    if((Match<MulInstr>(MatchAny(&a), MatchIC(&C))(opB) && (opA == a)) ||
       (Match<MulInstr>(MatchAny(&a), MatchIC(&C))(opA) && (opB == a))) {
		...
		auto constantOp = folder_.FoldBinary(Opcode::Add, C, oneConst, block);
...
Operand* Peephole::HandleAddComplex(Operand* opA, Operand* opB, Block* block) {
	...
	if(IsLogicalNot(opA, &a) && MatchIC(&C, block)(opB)) {
		auto oneConst = irGen_.GetIntConst(a->GetType(), 1);
		auto constantOp = folder_.FoldBinary(Opcode::Sub, C, oneConst, block);
    ...
    if((Match<AddInstr>(MatchAny(&a), MatchIC(&C, block))(opA) && (b = opB)) ||
       ((a = opA) && Match<AddInstr>(MatchAny(&b), MatchIC(&C, block))(opB))) {
        // The 'add' should have a single user, else we will 
        // increase the register pressure. The other operand should
        // not already be a constant, else an infinite loop appears
        // that just brings one of the constant to the right.
        auto addInstrOp = b == opB ? opA : opB;
        auto otherOp = b == opB ? opB : opA;

        if((otherOp->IsConstant() == false) && addInstrOp->HasSingleUser()) {
	...
	if((MatchConversion<ZextInstr>(MatchIC(&C1, block))(opA) && 
	   ...
	   (MatchConversion<SextInstr>(MatchIC(&C1, block))(opA) && 
		...
		auto constantOp = folder_.FoldBinary(Opcode::Add, C2, oneConst, block);
	   ...
	   (MatchConversion<ZextInstr>(MatchIC(&C1, block))(opA) && 
		...
		if((C1->GetType() == C2->GetType()) &&
		   (opInfo_.CanAddOverflow(C1, C2) == false)) {
			...
			auto constantOp = folder_.FoldBinary(Opcode::Add, C1, C2, block);
			auto temp = GetTemporary(opA);
    ...
    // (a ^ b) + b -> a, if 'b' is only the sign bit
    // (the xor inverts it, but the add inverts it back like in
    //  0xxxxx...x ^ 100000...0) + 100000...0 = 
    //  1xxxxx...x + 100000...0 = 0xxxxx...x).
    if(opB->DefiningInstrIs<XorInstr>()) {
        // Canonicalize '(a ^ b) + b' to 'b + (a ^ b)'.
        std::swap(opA, opB);
    }

    if(Match<XorInstr>(MatchAny(&a), MatchOp(opB))(opA) && 
       IsOnlySignBit(opB, block)) {
        return LOG(a);
    }
    else if(Match<XorInstr>(MatchOp(opB), MatchAny(&a))(opA) && 
            IsOnlySignBit(opB, block)) {
            return LOG(a);
    }
...
Operand* Peephole::HandleSub(Operand* opA, Operand* opB, Block* block) {
	...
	if(auto result = folder_.FoldBinary(Opcode::Sub, opA, opB, block)) {
        return result;
	}
    ...
    // Try to simplify using associativity/commutativity rules.
    if(auto result = HandleAssociative(Opcode::Sub, opA, opB, block)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Sub, opA, opB, block)) {
        return result;
    }
    ...
    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Sub, opA, opB)) {
        return result;
    }

    // Check if we are adding at least one 'quest' instruction result.
    return HandleOperationOnQuest(Opcode::Sub, opA, opB);
}
...
Operand* Peephole::HandleSubComplex(Operand* opA, Operand* opB, Block* block) {
	...
	if(MatchInt(0, block)(opA) && 
       Match<DivInstr>(MatchAny(&a), MatchIC(&C, block))(opB)) {
		auto constantOp = folder_.FoldBinary(Opcode::Sub, GetZeroInt(opA), C, block);
		auto temp = GetTemporary(opA);
	...
	if(Match<MulInstr>(MatchAny(&a), MatchIC(&C, block))(opB) &&
       (a == opA)) {
		auto constantOp = folder_.FoldBinary(Opcode::Sub, GetOneInt(opA), C, block);
		auto temp = GetTemporary(opA);
		irGen_.GetMul(a, constantOp, temp);
		return LOG(temp);
	}
	...
	if(Match<SubInstr>(MatchIC(&C1, block), MatchAny(&a))(opA) && 
       MatchIC(&C2, block)(opB)) {
		auto constantOp = folder_.FoldBinary(Opcode::Sub, C1, C2, block);
		auto subOp = GetTemporary(opA);
	...
	if(Match<ShlInstr>(MatchAny(&a), MatchIC(&C, block))(opB) &&
       (a == opA)) {
		...
		auto constantOp = folder_.FoldBinary(Opcode::Sub, oneConst,
										  folder_.FoldBinary(Opcode::Shl, 
	...
	if((Match<MulInstr>(MatchAny(&a), MatchIC(&C, block))(opB) && 
       ...
       (a == opA))) {
		auto constantOp = folder_.FoldBinary(Opcode::Sub, GetOneInt(opA), 
                                             C, block);
		auto mulOp = GetTemporary(opA);
	...
	if(Match<MulInstr>(MatchAny(&a), MatchIC(&C, block))(opA) && 
       (a == opB)) {
		auto constantOp = folder_.FoldBinary(Opcode::Sub, C, 
                                             GetOneInt(opA), block);
		auto temp = GetTemporary(opA);
		irGen_.GetMul(a, constantOp, temp);
		return LOG(temp);
	...
	if((MatchIC(&C1, block)(opA) && 
	   ...
	   (MatchIC(&C1, block)(opA) && 
        MatchConversion<SextInstr>(MatchIC(&C2, block))(opB))) {
		if(C2->Value() == 1) {
			return LOG(folder_.FoldBinary(Opcode::Sub, C1, GetOneInt(opB), block));
		}
		else return LOG(C1);
	}
	...
	if(((MatchConversion<SextInstr>(MatchIC(&C1, block))(opA) && 
		...
		if(C1->Value() == 1) {
			return LOG(folder_.FoldBinary(Opcode::Sub, GetZeroInt(opB), C2, block));
		}
		else return LOG(folder_.FoldBinary(Opcode::Sub, GetOneInt(opB), C2, block));
	}
	...
	if(MatchIC(&C)(opA) && 
       Match<XorInstr>(MatchAny(&a), MatchInt(-1, block))(opB)) {
		auto constantOp = folder_.FoldBinary(Opcode::Add, C, 
                                             GetOneInt(opA), block);
		auto temp = GetTemporary(opA);
		irGen_.GetAdd(a, constantOp, temp);
		return LOG(temp);
    ...
    // 0 - a -> a, if 'a' is the sign bit value (0 or 0x80000000 for 'int32').
    if(IsZeroInt(opA, block) && IsOnlySignBit(opB, block)) {
        return LOG(opB);
    }
...
Operand* Peephole::HandleSubPointers(Operand* opA, Operand* opB) {
    ...
    if(auto addrInstrA = targetOpA->DefiningInstrAs<AddressInstr>()) {
    ...
    else if(auto fieldInstrA = targetOpA->DefiningInstrAs<FieldInstr>()) {
        // ptoi(field ptr1, C1) - ptoi(field ptr2, C2) -> 
        //      (ptoi(ptr1) - ptoi(ptr2)) + (offset(C1) - offset(C2))
        if(auto fieldInstrB = targetOpB->DefiningInstrAs<FieldInstr>()) {
            auto C1 = fieldInstrA->IndexOp()->As<IntConstant>();
            auto C2 = fieldInstrB->IndexOp()->As<IntConstant>();
            DebugValidator::IsNotNull(C1);
            DebugValidator::IsNotNull(C2);

            auto castType = ptoiInstrA->CastType();
            bool sameBase = fieldInstrA->BaseOp() == fieldInstrB->BaseOp();
            Operand* baseDiffOp = nullptr;

            // If we have the same base we don't need to emit the base difference.
            if(sameBase == false) {
                baseDiffOp = CreateBaseDifference(fieldInstrA->BaseOp(), 
                                                  fieldInstrB->BaseOp(), 
                                                  castType);
            }

            // Compute the constant 'C1 * size(*ptr1) - C2 * size(*ptr2)'.
            auto recordTypeA = fieldInstrA->GetRecordType();
            __int64 offsetA = recordTypeA->GetFieldOffset(C1->Value());

            auto recordTypeB = fieldInstrB->GetRecordType();
        ...
        // ptoi(field ptr, C) - ptoi(ptr) ->  offset(C)
        else if(fieldInstrA->BaseOp() == ptoiInstrB->TargetOp()) {
            return LOG(CreateFieldOffset(fieldInstrA, ptoiInstrA->CastType()));
        }
    }
    ...
    else if(auto fieldInstrB = targetOpB->DefiningInstrAs<FieldInstr>()) {
        // ptoi(ptr) - ptoi(field ptr, C) -> -offset(C)
        if(fieldInstrB->BaseOp() == targetOpA) {
            return LOG(CreateFieldOffset(fieldInstrA, ptoiInstrA->CastType(), 
                                         true /* negateOffset */));
        }
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Operand* Peephole::CreateFieldOffset(FieldInstr* fieldInstr, const Type* resultType,
                                     bool negateOffset) {
    auto C = fieldInstr->IndexOp()->As<IntConstant>();
...
Operand* Peephole::HandleFadd(Operand* opA, Operand* opB, FloatMode mode) {
	...
	if(opA->IsFloatingConstant() && opB->IsFloatingConstant()) {
		return LOG(folder_.FoldBinary(Opcode::Fadd, opA, opB, nullptr));
	}

    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Fadd, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Fadd, opA, opB)) {
        return result;
    }

	// ALL the following simplifications are valid only if 
	// the 'FloatMode::Exact' mode is NOT activated for the instruction.
	if(mode == FloatMode::Exact) {
        return nullptr;
    }
	...
	// ALL the following simplifications are valid only if
	// the 'FloatMode::Fast' mode is activated for the instruction.
	if(mode != FloatMode::Fast) {
        return nullptr;
    }
	...
	// Try to simplify using associativity/commutativity rules.
    if(auto result = HandleAssociative(Opcode::Fadd, opA, opB, nullptr)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Fadd, opA, opB, nullptr)) {
        return result;
    }
...
Operand* Peephole::HandleFsub(Operand* opA, Operand* opB, FloatMode mode) {
	...
	// If both operands are constant we can fold the operation.
	if(opA->IsFloatingConstant() && opB->IsFloatingConstant()) {
		return LOG(folder_.FoldBinary(Opcode::Fsub, opA, opB, nullptr));
	}

    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Fsub, opA, opB)) {
        return result;
    }
	...
	// Try to simplify using associativity/commutativity rules.
    if(auto result = HandleAssociative(Opcode::Fsub, opA, opB, nullptr)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Fsub, opA, opB, nullptr)) {
        return result;
    }
