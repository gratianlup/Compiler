...
private:
	...
	struct ConstantRange {
		...
		int Count;          // The number of consecutive values.

		ConstantRange() {}

		ConstantRange(__int64 value, int startIndex, int count = 0) :
	...
	typedef StaticList<int, 4> IndexList;
    ...
    //! ALL THESE SHOULD USE CONTROLS!
    static const int MAX_CONST_ARRAY_SIZE = 64;
    static const int MAX_CONST_NESTED_LEVELS = 3;
    static const bool LOAD_FROM_CONST_STRUCT = true;
    static const int MAX_CONVERSION_DEPTH = 8;
    ...
    bool AreEqual(Operand* a, Operand* b, Block* testBlock) {
        ...
        return opInfo.AreEqual(a, b, testBlock) == RangeResult::Yes;
    }
    ...
    // Returns 'true' if the operand is '-valueOp'.
    bool IsNegatedValue(Operand* op, Operand* valueOp);

    // Returns 'true' if the operand can have only the values 0 or 1.
    bool IsBoolean(Operand* op);
    ...
    // if the operand's type is not 'requiredType'.
    Operand* CreateIntCastIfRequired(Operand* op, const Type* requiredType, 
                                     bool isSigned = true);
    ...
    Operand* PreserveUSO(Operand* sourceOp, Operand* destOp);
    ...
    // Returns 'true' if the specified operand represents the sign bit
    // (all bits except the sign bit are zero; the sign bit
    //  must not necessarily be known).
    bool IsOnlySignBit(Operand* op, Block* testBlock);
    ...
    Operand* CreateFieldOffset(FieldInstr* fieldInstr, const Type* resultType,
    ...
    Operand* HandleIntegerCmpAddSubConst(Operand* nonConstOp, IntConstant* intConst,
    ...
    bool HandleSwitchGlobalConstants(SwitchInstr* switchInstr);
    bool HandleSwitchParameterConstants(SwitchInstr* switchInstr);
    ...
    Operand* HandleAddrPtopField(PtopInstr* ptopInstr, FieldInstr* fieldInstr, 
                                 Operand* baseOp, Operand* indexOp);
    ...
    Operand* HandleAddrField(FieldInstr* fieldInstr, Operand* baseOp, Operand* indexOp);


    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	// Methods for simplifying 'field' instructions.
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    Operand* HandleElement(FieldInstr* instr);
    ...
    Operand* CreateAddressOfElement(Operand* baseOp, __int64 index, 
    ...
    Operand* HandleAssociative(Opcode opcode, Operand* opA, Operand* opB, 
                               Block* block, FloatMode mode = FloatMode::Exact);
    ...
    Operand* HandleQuestionBranchless(QuestionInstr* instr);

    Operand* HandleQuestionBranchlessAdd(CmpInstrBase* cmpInstr, 
                                         ArithmeticInstr* adjustmentInstr,
                                         bool trueOperand, bool isAdd);

    Operand* HandleQuestionAndOr(QuestionInstr* instr);

    Operand* HandleQuestionAndOrTrueZero(QuestionInstr* instr);

    Operand* HandleQuestionAndOrFalseZero(QuestionInstr* instr);

    Operand* HandleQuestionAndOrTrueOne(QuestionInstr* instr);

    Operand* HandleQuestionAndOrFalseOne(QuestionInstr* instr);

    Operand* HandleQuestionOneZero(QuestionInstr* instr);

	Operand* HandleQuestionSetTrue(QuestionInstr* instr);

    ArithmeticInstr* DetectBranchlessOperand(QuestionInstr* instr, bool& trueOperand);
	...
	void AddToCurrentRange(__int64 value, __int64 requiredValue, int index,
