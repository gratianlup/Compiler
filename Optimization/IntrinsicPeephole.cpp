...
Operand* Peephole::HandleIntrinsic(CallInstr* instr, Intrinsic* intrinsic) {
    if(auto memIntr = intrinsic->As<MemoryIntrinsic>()) {
        ...
        switch(bitInstr->GetBitwiseKind()) {
            case BitwiseIntrinsicKind::Rotate: {
                HandleRotateIntrinsic(instr);
            }
            case BitwiseIntrinsicKind::ByteSwap: {
...
Operand* Peephole::HandleCopyMemoryIntr(CallInstr* instr) {
    ...
    auto sizeConst = AsIntConstant(CopyMemoryIntr::GetLength(instr),
                                   instr->ParentBlock());
    if((sizeConst == nullptr) || (sizeConst->Value() > 8)) {
        return nullptr;
    }
    
	int size = (int)sizeConst->Value();
...
Operand* Peephole::CreateSetMemoryValue(Operand* valueOp, int size,
    ...
    if(auto intConst = valueOp->As<IntConstant>()) {
        ...
        Operand* t[8];
        int count = 0;
        int currentSize = intConst->GetType()->Size();
        int endSize = storeType->As<IntegerType>()->Size();
        ...
        while(currentSize < endSize) {
            auto constantOp = irGen_.GetIntConst(storeType, 8 * currentSize);
...
Operand* Peephole::HandleSqrtIntrinsic(CallInstr* instr) {
    ...
    if((mathIntrA == nullptr) || (((mathIntrA->GetMathKind() == MathIntrinsicKind::Sqrt) ||
       (mathIntrA->GetMathKind() == MathIntrinsicKind::Sqrtf)) == false)) {
        return nullptr;
    }
...
Operand* Peephole::HandleFmulIntrinsics(Operand* opA, Operand* opB, FloatMode mode) {
	if(auto callInstrA = opA->DefiningInstrAs<CallInstr>()) {
        auto mathIntrA = callInstrA->GetIntrinsicAs<MathIntrinsic>();

        if(auto callInstrB = opB->DefiningInstrAs<CallInstr>()) {
    ...
    else if(auto callInstr = opB->DefiningInstrAs<CallInstr>()) {
        // operand * intrinsic
...
Operand* Peephole::HandleFmulIntrIntr(CallInstr* instrA, CallInstr* instrB, 
    ...
    switch(mathIntrA->GetMathKind()) {
        case MathIntrinsicKind::Pow: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Pow) {
                // pow * pow
        ...
        case MathIntrinsicKind::Sqrt:
        case MathIntrinsicKind::Sqrtf: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Sqrt) {
                // sqrt(a) * sqrt(a) -> a
        ...
        case MathIntrinsicKind::Exp: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Exp) {
                // exp(a) * exp(b) -> exp(a + b)
        ...
        case MathIntrinsicKind::Tan: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Cos) {
                // tan(a) * cos(a) -> sin(a)
                if(instrA->GetArgument(0) == instrB->GetArgument(0)) {
...
Operand* Peephole::HandleFdivIntrIntr(CallInstr* instrA, CallInstr* instrB, 
    ...
    switch(mathIntrA->GetMathKind()) {
        case MathIntrinsicKind::Cos: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Sin) {
            ...
            else if(mathIntrB->GetMathKind() == MathIntrinsicKind::Cos) {
                // cos(a) / cos(a) -> 1.0
                if(instrA->GetArgument(0) == instrB->GetArgument(0)) {
        ...
        case MathIntrinsicKind::Sin: {
            if(mathIntrB->GetMathKind() == MathIntrinsicKind::Tan) {
                // sin(a) / tan(a) -> cos(a)
                if(instrA->GetArgument(0) == instrB->GetArgument(0)) {
            ...
            else if(mathIntrB->GetMathKind() == MathIntrinsicKind::Sin) {
                // sin(a) / sin(a) -> 1.0
                if(instrA->GetArgument(0) == instrB->GetArgument(0)) {
                    return LOG(GetOneFloat(instrA->GetArgument(0)));
                }
        ...
        case MathIntrinsicKind::Tan: {
            ...
            else if(mathIntrB->GetMathKind() == MathIntrinsicKind::Tan) {
                // tan(a) / tan(a) -> 1.0
                if(instrA->GetArgument(0) == instrB->GetArgument(0)) {
                    return LOG(GetOneFloat(instrA->GetArgument(0)));
                }
...
Operand* Peephole::HandleMinMaxIntrCmp(CallInstr* instrA, CallInstr* instrB, 
    ...
    if(mathIntrB->IsMin() && SameMinMaxOperands(instrA, instrB)) {
        ...
        if((order == OrderType::Less) || (order == OrderType::NotEqual) ||
        ...
        if(order == OrderType::LessOrEqual) {
            ...
            if(instrA->ResultOp()->HasSingleUser() &&
               instrB->ResultOp()->HasSingleUser()) {
                return LOG(CreateCompare(instrA->GetArgument(0),
                                         ...
                                         OrderType::NotEqual, resultType, isUnsigned));
            }
        }

        // min(a, b) >  max(a, b) -> false
        if(order == OrderType::Greater) {
            return LOG(GetBool(false, resultType));
        }
        ...
        if(order == OrderType::Equal) {
            return LOG(CreateCompare(instrA->GetArgument(0),
                                     instrB->GetArgument(1),
                                     OrderType::Equal, resultType, isUnsigned));
        }

        // min(a, b) != max(a, b) -> a != b
        if(order == OrderType::NotEqual) {
            return LOG(CreateCompare(instrA->GetArgument(0),
                                     instrB->GetArgument(1),
                                     OrderType::NotEqual, resultType, isUnsigned));
        }
...
Operand* Peephole::HandleMinIntrConstCmp(CallInstr* instrA, IntConstant* intConst,
    ...
    if(IA::IsPositive(intConst)) {
        if((order == OrderType::Equal) && intConst->IsZero()) {
            // min(a, 0) == 0 -> a >= 0
            return LOG(CreateCompare(minOpA, intConst, OrderType::GreaterOrEqual, 
                                     resultType, isUnsigned));
        }
        else if((order == OrderType::Equal) || (order == OrderType::Greater)) {
            // min(a, 0) == 4 -> false
            // min(a, 0) >  0 -> false
            // min(a, 0) >  4 -> false
        ...
        // min(a, 0) == -1 -> a == -1
        return LOG(CreateCompare(minOpA, intConst, OrderType::Equal,
                                 minOpA->GetType(), isUnsigned));
    }
    else if(order == OrderType::Greater) {
        // min(a, 0) >  -1 -> a > -1
        return LOG(CreateCompare(minOpA, intConst, OrderType::Greater,
                                 minOpA->GetType(), isUnsigned));
    }
...
Operand* Peephole::HandleMinIntrOpCmp(CallInstr* instrA, Operand* opB,
    ...
    if(order == OrderType::Equal) {
        if(AreEqual(instrA->GetArgument(0), opB, block)) {
            return LOG(CreateCompare(opB, instrA->GetArgument(1),
                                     OrderType::LessOrEqual, resultType, isUnsigned));
        }
    ...
    if(order == OrderType::NotEqual) {
        if(AreEqual(instrA->GetArgument(0), opB, block)) {
            return LOG(CreateCompare(opB, instrA->GetArgument(1),
                                     OrderType::Greater, resultType, isUnsigned));
        }
        else if(AreEqual(instrA->GetArgument(1), opB, block)) {
            return LOG(CreateCompare(opB, instrA->GetArgument(0),
                                     OrderType::Greater, resultType, isUnsigned));
    ...
    // min(a, b) </<= a -> b </<= a
    // min(a, b) </<= b -> a </<= b
    if((order == OrderType::Less) || (order == OrderType::LessOrEqual)) {
        if(AreEqual(instrA->GetArgument(0), opB, block)) {
...
Operand* Peephole::HandleMaxIntrConstCmp(CallInstr* instrA, IntConstant* intConst,
    ...
    if(IA::IsPositive(intConst)) {
        if(order == OrderType::Equal) {
            if(intConst->IsZero()) {
                // max(a, 0) == 0 -> a <= 0
                return LOG(CreateCompare(maxOpA, intConst, OrderType::LessOrEqual, 
                                         resultType, isUnsigned));
            }
                        
            // max(a, 0) == 5 -> a == 5
            return LOG(CreateCompare(maxOpA, intConst, OrderType::Equal, 
                                     resultType, isUnsigned));
        }
        else if(order == OrderType::Greater) {
            // max(a, 0) > 0 -> a > 0
            // max(a, 0) > 5 -> a > 5
            return LOG(CreateCompare(maxOpA, intConst, OrderType::Greater, 
                                     resultType, isUnsigned));
        }
    }
    else if(order == OrderType::Equal) {
...
Operand* Peephole::HandleMaxIntrOpCmp(CallInstr* instrA, Operand* opB,
    ...
    if(order == OrderType::NotEqual) {
        if(AreEqual(instrA->GetArgument(0), opB, block)) {
            return LOG(CreateCompare(opB, instrA->GetArgument(1),
                                     OrderType::Less, resultType, isUnsigned));
        }
        else if(AreEqual(instrA->GetArgument(1), opB, block)) {
            return LOG(CreateCompare(opB, instrA->GetArgument(0),
                                     OrderType::Less, resultType, isUnsigned));
    ...
    // max(a, b) >/>= a -> b >/>= a
    // max(a, b) >/>= b -> a >/>= b
    if((order == OrderType::Greater) || (order == OrderType::GreaterOrEqual)) {
        if(AreEqual(instrA->GetArgument(0), opB, block)) {
            return LOG(CreateCompare(instrA->GetArgument(1), opB,
                                     order, resultType, isUnsigned));
        }
...
Operand* Peephole::HandleAbsIntrOpCmp(CallInstr* instrA, Operand* opB,
    ...
    // abs(a) > a -> a < 0
    // abs(a) != a -> a < 0
    if((absOp == opB) && ((order == OrderType::Greater) || (order == OrderType::NotEqual))) {
        return LOG(CreateCompare(absOp, GetZeroInt(absOp), OrderType::Less,
                                 resultType, isUnsigned));
    }

    // abs(a) == a -> a >= 0
    if((absOp == opB) && (order == OrderType::Equal)) {
        return LOG(CreateCompare(absOp, GetZeroInt(absOp), 
                                 OrderType::GreaterOrEqual, resultType, isUnsigned));
    }
...
Operand* Peephole::HandleSqrtIntrConstCmp(Operand* a, FloatConstant* C, OrderType order, 
    ...
    if((order == OrderType::Less) && FA::IsNegative(C)) {
        return LOG(GetBool(false, resultType));
    }

    // sqrt(a) > C -> true, if 'C < 0'
    if((order == OrderType::Greater) && FA::IsNegative(C)) {
        return LOG(GetBool(true, resultType));
    }

    // sqrt(a) > C -> false, if 'C == +INF'
    if((order == OrderType::Greater) && FA::IsPositiveInfinity(C)) {
        return LOG(GetBool(false, resultType));
    }
    ...
    if(order == OrderType::Greater) {
        ...
        return LOG(CreateCompare(a, constantOp, OrderType::Greater, resultType, false));
    }

    // sqrt(a) < C -> true, if 'C == +INF'
    if((order == OrderType::Less) && FA::IsPositiveInfinity(C)) {
        return LOG(GetBool(true, resultType));
    }
    ...
    if(order == OrderType::Less) {
        double value = FA::Fmul(C, 2.0);
        auto constantOp = irGen_.GetFloatingConst(C->GetType(), value);
        return LOG(CreateCompare(a, constantOp, OrderType::Less, resultType, false));
    }
