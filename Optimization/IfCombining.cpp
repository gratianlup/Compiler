...
void IfCombining::Execute(Function* function) {
    ...
    for(auto block = function->LastBlock(); block; block = block->PreviousBlock()) {
        if(processed.IsSet(block->Id()) || block->IsEmpty()) {
			continue;
		}
        else processed.SetBit(block->Id());

		bool optimized = DetectQuestion(block)           ||
						 DetectOrChain(block, processed) ||
						 CombineIfWithAndOr(block);
    }
}
...
bool IfCombining::TryExtractQuestion(PhiInstr* instr) {
    ...
    auto op1Block = instr->GetOperandBlock(0);
    auto op2Block = instr->GetOperandBlock(1);
...
void IfCombining::GenerateQuestion(PhiInstr* instr, Block* hoistingBlock, 
    ...
    if(instr->GetOperandBlockReference(0) == ifInstr->TrueTargetOp()) {
        trueOp = instr->GetOperand(0);
        falseOp = instr->GetOperand(1);
    }
    else if(instr->GetOperandBlockReference(1) == ifInstr->TrueTargetOp()) {
        trueOp = instr->GetOperand(1);
        falseOp = instr->GetOperand(0);
    }
    else if(instr->GetOperandBlockReference(0) == ifInstr->FalseTargetOp()) {
        trueOp = instr->GetOperand(1);
        falseOp = instr->GetOperand(0);
    }
    else if(instr->GetOperandBlockReference(1) == ifInstr->FalseTargetOp()) {
        trueOp = instr->GetOperand(0);
        falseOp = instr->GetOperand(1);
    }
...
bool IfCombining::CanInstructionsBeHoisted(Operand* dependentOp, Block* toBlock,
    ...
    if(auto temp = dependentOp->As<Temporary>()) {
        ...
        VisitedDictionary visited;
        HoistingList worklist;
        int cost = 0;

        // If the start instruction is already in 'toBlock' we're done.
        // This can happen for 'if-then' constructs.
        auto startInstr = temp->DefiningInstruction();
        auto sourceBlock = startInstr->ParentBlock();

        if(sourceBlock == toBlock) {
            return true;
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool IfCombining::IsUsedOnlyByHoisted(Instruction* instr, VisitedDictionary& visited,
                                      Block* sourceBlock) {
    ...
    while(userEnum.IsValid()) {
        ...
        if((user->ParentBlock() == sourceBlock) &&
           (visited.ContainsKey(user) == false)) {
            // An exception is for operands of an 'if' or 'ret'
            // (they are not going to be hoisted, but they should not
            //  prevent the hoisting of the other instructions).
            if(auto ifInstr = sourceBlock->BranchInstruction()->As<IfInstr>()) {
                if(ifInstr->ConditionOp()->DefiningInstruction() == instr) {
                    continue;
                }
            }
            else if(auto retInstr = sourceBlock->BranchInstruction()->As<ReturnInstr>()) {
                if(retInstr->IsVoid() || 
                   retInstr->ReturnedOp()->DefiningInstruction() == instr) {
                    continue;
                }
            }
...
void IfCombining::AddOperandsToWorklist(Instruction* instr, HoistingList& worklist,
                                        VisitedDictionary& visited, Block* sourceBlock) {
    // Add the operands to the worklist, if needed.
...
bool IfCombining::IsNonTrapping(Operand* addressOp) {
    ...
    if(auto fieldInstr = addressOp->DefiningInstrAs<FieldInstr>()) {
        return fieldInstr->BaseOp()->IsVariableReference();
    }
...
bool IfCombining::IsTrappingInDominator(Operand* addressOp, Block* startBlock) {
    ...
    while(domBlock) {
        if(trappingOps_.ContainsKey(domBlock) == false) {
            ...
            trappingOps_.Add(domBlock, TrappingDictionary());
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void IfCombining::ComputeTrappingOperands(Block* block, TrappingDictionary& dict) {
...
int IfCombining::ComputeHoistingLimit(Block* hoistingBlock, Block* sourceBlock,
    ...
    //   we are not penalized much because we leave the branches.
    int limit = isIfThenElse ? HOISTING_LIMIT_IF_THEN_ELSE : 
                               HOISTING_LIMIT;
...
bool IfCombining::IsCheapPhi(PhiInstr* instr) {
    ...
    // are constants of references. This encourages further simplifications
	// of the 'quest' instructions that result after the transformation.
    int cheapOps = 0;
...
int IfCombining::EstimateInstructionCost(Instruction* instr) {
	//! TODO: This should use the TargetInfo interface.
    ...
    switch(instr->GetOpcode()) {
        ...
        case Opcode::Address:
        case Opcode::Index: return 1 + (instr->GetSourceOp(1)->IsConstant() ? 0 : 2);
...
bool IfCombining::CombineIfWithAndOr(Block* block) {
    ...
    auto predTrueTarget = predIfInstr->TrueTargetBlock();
    auto trueTarget = ifInstr->TrueTargetBlock();
    ...
    if(predTrueTarget && (predTrueTarget == trueTarget)) {
        ...
        auto predFalseTarget = predIfInstr->FalseTargetBlock();
        auto falseTarget = ifInstr->FalseTargetBlock();
    ...
    //    that are not incoming from it's predecessor.
...
bool IfCombining::CombineIfBlocks(Block* block, Block* predecessorBlock, IfInstr* ifInstr,
    ...
    if(conditionOp->IsConstant() == false) {
        ...
        if(isAnd) {
            instr = LogicalInstr::GetLogical(Opcode::And, predCondOp, 
                                             conditionOp, combOp);
        }
        else instr = LogicalInstr::GetLogical(Opcode::Or, predCondOp, 
                                              conditionOp, combOp);
...
void IfCombining::MakeCompatible(Operand*& opA, Operand*& opB, 
    ...
    if((defInstrA == nullptr) || (defInstrA->IsComparison() == false)) {
        if(opA->GetType() != opB->GetType()) {
            ...
            auto instrA = CmpInstr::GetCmp(OrderType::NotEqual, opA, 
    ...
    if((defInstrB == nullptr) || (defInstrB->IsComparison() == false)) {
        if(opA->GetType() != opB->GetType()) {
            ...
            auto instrB = CmpInstr::GetCmp(OrderType::NotEqual, opB, 
...
bool IfCombining::DetectOrChain(Block* block, SparseBitVector& processed) {
    ...
    if(IsEqualityWithConstantBlock(block, cmpInstr, ifInstr) != FailureReason::None) {
        return false;
    }
    ...
    while(valid && !stopped && block->HasSinglePredecessor()) {
        ...
        auto result = IsEqualityWithConstantBlock(predecessorBlock, cmpInstr, 
                                                  ifInstr, cmpOp);
        bool addCandidate = (result == FailureReason::None) || 
                            (result == FailureReason::TooManyInstructions);
        ...
        if((result != FailureReason::NoIf) &&
           (prevIfInstr->TrueTargetOp() == ifInstr->TrueTargetOp())) {
            if(HasIncomingOperand(prevIfInstr->TrueTargetBlock(), block)) {
        ...
        if(result == FailureReason::TooManyInstructions) {
            stopped = true;
        }
        else if((result != FailureReason::None) &&
                (result != FailureReason::NoIf)) {
            valid = false;
        }
    }
...
Operand* IfCombining::FlatternOrChain(Block* block, SparseBitVector& processed,
    ...
    for(int i = last - 1; i >= 0; i--) {
        ...
        auto orInstr = LogicalInstr::GetLogical(Opcode::Or,  result->GetDestinationOp(), 
    ...
    if(auto definingInstr = ifInstr->ConditionOp()->DefiningInstruction()) {
        if(definingInstr->IsCmp() || definingInstr->IsUcmp()) {
            ...
            if(cmpOp && (cmpInstr->LeftOp() != cmpOp)) {
                return FailureReason::InvalidIf;
            }
            ...
            if(validCmp && cmpInstr->RightOp()->IsIntConstant()) {
                ...
                return block->InstructionCount() == 2 ? FailureReason::None :
                                                        FailureReason::TooManyInstructions;
            }
        }
    }
...
bool IfCombining::IsValidToMergeBlocks(Block* toBlock, Block* fromBlockA, 
    ...
    toBlock->ForEachPhiInstruction([&](PhiInstr* phiInstr) -> bool {
		auto opA = phiInstr->GetOperandFromBlock(fromBlockA);
        auto opB = phiInstr->GetOperandFromBlock(fromBlockB);
...
bool IfCombining::ProfitableToHoistToBlock(Block* fromBlock, Block* hoistingBlock,
    ...
    // used by the 'if' in the hoisting block depends. To the existing ones
    // the ones from 'hoistingList' will be added after the transformation.
    // Don't allow too many, else the code could actually get slower. 
...
int IfCombining::AdjustLimit(int limit, Block* fromBlock, Block* toBlock) {
    ...
    return hoistingLimitAdjustment_.Adjust(limit, fromBlock, toBlock);
}
...
Block* IfCombining::GetSingleSuccessor(Block* block, bool stopOnGoto) {
    ...
    if(auto gotoInstr = block->BranchInstruction()->As<GotoInstr>()) {
        ...
        else return gotoInstr->TargetBlock();
    }
        ...
        if(auto intConst = ifInstr->ConditionOp()->As<IntConstant>()) {
            if(intConst->IsZero()) {
                ...
                return GetSingleSuccessor(ifInstr->FalseTargetBlock(), true);
            }
            else return GetSingleSuccessor(ifInstr->TrueTargetBlock(), true);
        }
    }
    ...
    if(block->HasSinglePredecessor()) {
        ...
        if(ifInstr) {
            ...
            if(intConst->IsZero()) {
                ...
                if(ifInstr->TrueTargetBlock() == block) {
                    return GetSinglePredecessor(other);
                }
            }
            else {
                // The block is not reachable from the the parent of the 'if'.
                if(ifInstr->FalseTargetBlock() == block) {
                    return GetSinglePredecessor(other);
                }
            }
        }
...
Block* IfCombining::GetTrueSuccessor(IfInstr* instr) {
    return GetSingleSuccessor(instr->TrueTargetBlock(), true);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
Block* IfCombining::GetFalseSuccessor(IfInstr* instr) {
    return GetSingleSuccessor(instr->FalseTargetBlock(), true);
}
