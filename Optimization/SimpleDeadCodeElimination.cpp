...
void SimpleDeadCodeElimination::CollectStoreInstructions(Variable* variable, 
    ...
    function->ForEachInstruction([&stores, variable, function, this]
        ...
        if(auto storeInstr = instr->As<StoreInstr>()) {
            ...
            // Collect the 'index'/'field' instruction indices used as the base.
...
void SimpleDeadCodeElimination::AddIndexes(Operand* op, LoadStoreInfo& info, 
    ...
    if(auto variableRef = op->As<VariableReference>()) {
        ...
        info.IndexList.Add(fieldInstr->GetFieldIndex());
        AddIndexes(fieldInstr->BaseOp(), info, variable, isStore);
        return;
    }
...
bool SimpleDeadCodeElimination::AnalyzeDeadCopyCall(CallInstr* callInstr, 
	...
	if((intrinsic == nullptr) ||
       (intrinsic->IsMemoryIntrinsic() == false)) {
        return false;
    }

    // Analyze the destination operand, and for 'copyMemory' the source also.
    if(auto destVariable = GetBaseVariable(callInstr->GetArgument(0))) {
        if(copySetInfo.ContainsKey(destVariable)) {
            CopySetInfo& info = copySetInfo[destVariable];
            info.CopySetReferences++;
            info.CopySetCalls.Add(callInstr);
        }
    }

    if(auto sourceVariable = GetBaseVariable(callInstr->GetArgument(1))) {
        if(copySetInfo.ContainsKey(sourceVariable)) {
            CopySetInfo& info = copySetInfo[sourceVariable];
            info.CopySetReferences++;
        }
    }
