...
// TODO: udiv a, (quest c, C1, C2) -> quest c, (ushr a, C1), (ushr a, C2)
// div a, b -> udiv a, b if a, b > 0
...
Operand* Peephole::HandleMul(Operand* opA, Operand* opB, Block* block,
	...
	if(auto result = folder_.FoldBinary(Opcode::Mul, opA, opB, block)) {
        return LOG(result);
	}
    ...
    if(auto result = HandleAssociative(Opcode::Mul, opA, opB, block)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Mul, opA, opB, block)) {
        return result;
    }
    ...
    if(auto result = HandleOperationOnQuest(Opcode::Mul, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Mul, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleMulSimple(Operand* opA, Operand* opB, Block* block,
    ...
    // (a + a) * C -> a * (2 * C)
    if(auto addInstr = opA->DefiningInstrAs<AddInstr>()) {
        if(opA->HasSingleUser() && opB->IsIntConstant() &&
           (addInstr->LeftOp() == addInstr->RightOp())) {
            auto intConst = opB->As<IntConstant>();
            auto intKind = intConst->GetType()->GetSubtype();

            __int64 value = IA::Mul(intConst->Value(), 2, intKind);
            auto constantOp = irGen_.GetIntConst(opA->GetType(), value);
		
            auto temp = GetTemporary(opA);
	        irGen_.GetMul(addInstr->LeftOp(), constantOp, temp);
	        return LOG(temp);
        }
    }
...
Operand* Peephole::HandleMulComplex(Operand* opA, Operand* opB, Block* block) {
    ...
    // The multiplication of the results of two comparisons
    // can be replaced by an equivalent 'and' operation.
    // This can be done also if the operands are marked as being boolean.
    if(IsBoolean(opA) && IsBoolean(opB)) {
        auto andOp = GetTemporary(opA);
		irGen_.GetAnd(opA, opB, andOp);
...
Operand* Peephole::HandleFmul(Operand* opA, Operand* opB, FloatMode mode) {
	...
	if(opA->IsFloatingConstant() && opB->IsFloatingConstant()) {
		return LOG(folder_.FoldBinary(Opcode::Fmul, opA, opB, nullptr));
	}

    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Fmul, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Fmul, opA, opB)) {
        return result;
    }

	// ALL the following simplifications are valid only if 
	// the 'FloatMode::Exact' mode is NOT activated for the instruction.
	if(mode == FloatMode::Exact) return nullptr;
	...
	// Try to simplify using associativity/commutativity rules.
    if(auto result = HandleAssociative(Opcode::Fmul, opA, opB, nullptr, mode)) {
        return result;
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Fmul, opA, opB, nullptr, mode)) {
        return result;
    }
...
Operand* Peephole::HandleDiv(Operand* opA, Operand* opB, Block* block,
    ...
    if(auto result = folder_.FoldBinary(Opcode::Div, opA, opB, block)) {
        return LOG(result);
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Div, opA, opB, block)) {
        return result;
    }
		...
		if(auto subInstrB = opB->DefiningInstrAs<SubInstr>()) {
			if(IsZeroInt(subInstrA->LeftOp(), block) &&
			   ...
			   subInstrB->HasUndefinedOverflow()) {
				auto rightA = subInstrA->RightOp();
				auto rightB = subInstrB->RightOp();

				auto divOp = GetTemporary(opA);
				irGen_.GetDiv(rightA, rightB, divOp);
				return LOG(divOp);
			}
		}
	}
    ...
    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Div, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Div, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleUdiv(Operand* opA, Operand* opB, Block* block) {
    ...
    // Try to constant-fold.
    if(auto result = folder_.FoldBinary(Opcode::Udiv, opA, opB, block)) {
        return LOG(result);
    }

    // Try to simplify using distributivity rules.
    if(auto result = HandleDistributive(Opcode::Udiv, opA, opB, block)) {
        return result;
    }
    ...
    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Udiv, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Udiv, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleDivCommon(Operand* opA, Operand* opB, 
    ...
    // (a * b) / a -> b
    // (a * b) / -b -> -a
    // (a * b) / -a -> -b, if signed overflow is undefined.
    if(Match<MulInstr>(MatchAny(&a), MatchAny(&b))(opA) &&
       isDiv && opA->DefiningInstruction()->HasUndefinedOverflow()) {
        if(opB == a) {
            return b;
        }
        else if (opB == b) {
        ...
        else if(isDiv && IsNegatedValue(opB, b)) {
            return NegateInt(a);
        }
        else if(isDiv && IsNegatedValue(opB, a)) {
            return NegateInt(b);
        }
    }
    ...
    // -(a * b) / b -> -a
    // -(a * b) / a -> -b, if signed overflow is undefined.
    if(Match<SubInstr>(MatchIC(0), Match<MulInstr>(MatchAny(&a), MatchAny(&b)))(opA) &&
       isDiv && opA->DefiningInstruction()->HasUndefinedOverflow()) {
        if(opB == a) {
            return NegateInt(b);
        }
...
Operand* Peephole::HandleMod(Operand* opA, Operand* opB, Block* block) {
    ...
    // Try to constant-fold.
    if(auto result = folder_.FoldBinary(Opcode::Mod, opA, opB, block)) {
        return LOG(result);
    }
    ...
    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Mod, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Mod, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleUmod(Operand* opA, Operand* opB, Block* block) {
    ...
    // Try to constant-fold.
    if(auto result = folder_.FoldBinary(Opcode::Umod, opA, opB, block)) {
        return LOG(result);
    }
    ...
    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Umod, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Umod, opA, opB)) {
        return result;
    }
...
Operand* Peephole::HandleFdiv(Operand* opA, Operand* opB, FloatMode mode) {
	...
	// If both operands are constant we can fold the operation.
	if(opA->IsFloatingConstant() && opB->IsFloatingConstant()) {
		return LOG(folder_.FoldBinary(Opcode::Fdiv, opA, opB, nullptr));
	}

    // Check if we are adding at least one 'quest' instruction result.
    if(auto result = HandleOperationOnQuest(Opcode::Fdiv, opA, opB)) {
        return result;
    }

    // Check if we can simplify by applying the operation
    // one each incoming value of a 'phi' instruction.
    if(auto result = HandleInstructionOnPhi(Opcode::Fdiv, opA, opB)) {
        return result;
    }
