...
public:
	...
	// Replaces each 'phi' instruction in 'block' with the operand
	// incoming from from the specified block.
	static void ReplacePhisWithIncomingFrom(Block* block, Block* incomingBlock);

	// Removes all 'phi' instruction operands incoming from the specified block.
	static void RemoveIncomingFrom(Block* block, Block* incomingBlock);
