...
void CFGCleaner::Execute(Function* function, IRDominatorTree* domTree) {
    ...
    while(changed) {
        ...
        for(int i = 0; i < list.Count(); i++) {
            ...
            changed |= EliminateRedundantBranch(block) ||
                       EliminateEmptyBlock(block)      ||
                       MergeWithPredecessor(block)     ||
                       HoistBranch(block);
        }
...
bool CFGCleaner::EliminateRedundantBranch(Block* block) {
    ...
    if(auto switchInstr = block->BranchInstruction()->As<SwitchInstr>()) {
        ...
        if(switchInstr->CaseCount() == 1) {
            ...
            auto cmpInstr = CmpInstr::GetCmp(OrderType::Equal, switchInstr->ConditionOp(),
                                             constantOp, temp);
...
bool CFGCleaner::MergeWithPredecessor(Block* block) {
    ...
    if(block->PredecessorCount() == 1) {
        ...
        // Any 'phi' instruction has a single incoming operand
        // (from the predecessor) and can be eliminated.
        if(block->HasPhi()) {
            BlockUtils::ReplacePhisWithIncomingFrom(block, predecessorBlock);
        }
...
bool CFGCleaner::EliminateEmptyBlock(Block* block) {
    ...
    if(block->HasOnlyBranching() == false) {
        if(block->HasPhi()) {
    ...
    auto newTarget = gotoInstr->TargetBlock();
...
bool CFGCleaner::EliminatePhiBlock(Block* block) {
    ...
    if(block->FirstNonPhiInstruction() != gotoInstr) {
    }
        return false;
            ...
            if(incomingPhiInstr->ParentBlock() == block) {
                ...
                for(int i = 0; i < incomingPhiInstr->OperandCount(); i++) {
                    phiInstr->AddOperand(incomingPhiInstr->GetOperand(i),
                                         incomingPhiInstr->GetOperandBlockReference(i));
                }
...
bool CFGCleaner::HoistBranch(Block* block) {
    ...
    if((gotoInstr == nullptr) ||
	   (gotoInstr->TargetBlock()->HasOnlyBranching() == false)) {
        return false;
    }
	
    // We support only 'if'.
	auto target = gotoInstr->TargetBlock();
...
bool CFGCleaner::ConstantFoldBranch(Block* block) {
    ...
    if(branchingInstr->IsGoto() || branchingInstr->IsReturn()) {
        return false;
    }

	if(auto targetBlock = folder_.FoldBranching(branchingInstr)) {
    ...
    auto phiInstr = switchInstr->ConditionOp()->DefiningInstrAs<PhiInstr>();
	...
	for(int i = caseList.Count() - 1; i >= 0; i--) {
		if(phiOps.ContainsKey(caseList[i].Value) == false) {
            ...
            auto caseTargetBlock = switchInstr->GetCase(i).Target;
            RemoveIncomingFromBlock(caseTargetBlock->Target(), block);
...
void CFGCleaner::BlockSimplified(Block* block, int type) {
	...
	Log::Warning(text);
    if(function && *function->Name() == "readMasterJournal") 
    {
