...
private:
    ...
    typedef Dictionary<Operand*, SparseBitVector> OperandToBitsDict;
	typedef Dictionary<Block*, BitVector> BlockToBitsDict;
	typedef Dictionary<Operand*, int> OperandToIdDict;
    typedef Dictionary<Operand*, bool> VisitedDictionary;
    typedef StaticList<CopySetInfo, 32> CopyInfoList;
	typedef Analysis::TypeInfo TI;

    Function* funct_;
    CopyInfoList infoList_;      // The candidate operations.
    OperandToBitsDict destToId_; // Maps the copy to the operation.
    BlockToBitsDict copySets_;
    BlockToBitsDict killSets_;
    BlockToBitsDict inSets_;
    BlockToBitsDict outSets_;
    ...
    // Replaces operands that represent copies with the original ones.
    void ReplaceWithOriginal(CallInstr* instr, OperandToIdDict& availableCopies,
                             BitVector& killSet);
    ...
    void MarkUsedCopies(Operand* op, List<int>& copyCount, VisitedDictionary& visited,
