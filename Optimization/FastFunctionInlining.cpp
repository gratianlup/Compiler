...
Function* FastFunctionInlining::GetCalledFunction(CallInstr* instr) {
    ...
    if((calledFunct == nullptr)               ||
	   (calledFunct->IsDefinition() == false) ||
	   calledFunct->IsVarargs()) {
        return nullptr;
    }

    // Don't inline recursive functions here, because
    // Tail Recursion Elimination usually does a better job.
    if(calledFunct == instr->ParentFunction()) {
        return nullptr;
    }

    // Check if the language rules allow the function to be inlined.
    if(auto languageInfo = GetLanguageInfo()) {
        if(languageInfo->CanBeInlined(calledFunct) == false) {
            return nullptr;
        }
    }
...
bool FastFunctionInlining::ShouldInline(Function* caller, Function* callee, 
    ...
    if(callee->Inline() == InlineType::Always) {
        return true;
    }
    else if(callee->Inline() == InlineType::Never) {
        return false;
    }
...
void FastFunctionInlining::EstimateLoopBlocks(Function* function, 
    ...
    while(block) {
        ...
        auto endBlock = gotoInstr->TargetBlock();

        if(endBlock == block) {
            // This is an infinite loop, exit it.
            block = block->PreviousBlock();
            continue;
        }
