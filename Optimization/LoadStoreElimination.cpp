...
bool LoadStoreElimination::IsAddressUsedByCall(CallInstr* callInstr, Operand* addressOp) {
    ...
    // If the function doesn't depend/modify the global state
    // of the application it means that only the pointer parameters
    // could be read/written by calling it.
    auto calledFunction = callInstr->GetCalledFunction();
    
    if(calledFunction && calledFunction->IsNoState() &&
       (calledFunction->IsVarargs() == false)) {
        for(int i = 0; i < callInstr->ArgumentCount(); i++) {
            auto argument = callInstr->GetArgument(i);

            // We're interested only in pointers. If the pointer
            // is neither read or written it doesn't matter that
            // it aliases the analyzed location.
            if(argument->IsPointer()) {
                ...
                // Check for completely different objects.
                auto baseOpA = GetBaseOperand(addressOp);
                auto baseOpB = GetBaseOperand(addressOp);

                if((baseOpA == nullptr) ||
                   (baseOpB == nullptr) ||
                   (baseOpA->IsVariableReference() == false) ||
                   (baseOpB->IsVariableReference() == false) ||
                   (baseOpA == baseOpB)) {
                    return true;
                }
            }
...
bool LoadStoreElimination::AreReferencesDifferent(Operand* opA, Operand* opB) {
    ...
    if(IsAddressNotTaken(opA, &variableRefA)) {
       if(IsAddressNotTaken(opB, &variableRefB)) {
           ...
           if(variableRefB->IsAddressNotTaken() &&
        ...
        if(variableRefA = WithoutPointerCasts(opA)->As<VariableReference>()) {
           if(variableRefA->IsAddressNotTaken() &&
    ...
    if(opA->DefiningInstrIs<IndexInstr>() ||
       opB->DefiningInstrIs<IndexInstr>()) {
        return false;
           ...
           (opA->DefiningInstruction()->IsSameKind(opB->DefiningInstruction()) == false)) {
        return false;
    }
    ...
    if(baseOpA && baseOpB) {
        ...
        if(baseOpA == baseOpB) {
            ...
            if(auto variableRefB = baseOpB->As<VariableReference>()) {
                auto globalA = variableRefA->GetGlobalVariable();
                auto globalB = variableRefB->GetGlobalVariable();
        ...
        if(valid) {
            ...
            else return AreFieldsDifferent(addrInstrA->As<FieldInstr>(),
                                           addrInstrB->As<FieldInstr>(),
                                           baseOpA, baseOpB);
        }
    }
        ...
        if(baseOp == otherOp) {
            ...
            if(indexOp == nullptr) {
                return false; // 'field' not valid here.
            }
...
bool LoadStoreElimination::AreArrayRecordRefsDifferent(Operand* opA, Operand* opB,
    ...
    else if(auto fieldInstrA = instrA->As<FieldInstr>()) {
        auto fieldInstrB = instrB->As<FieldInstr>();
        return AreFieldsDifferent(fieldInstrA, fieldInstrB, 
                                  baseOpA, baseOpB, level);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool LoadStoreElimination::AreFieldsDifferent(FieldInstr* fieldInstrA, 
                                              FieldInstr* fieldInstrB,
                                              Operand* baseOpA, Operand* baseOpB, 
                                              int level) {
    if(fieldInstrA->GetRecordType() == fieldInstrB->GetRecordType()) {
        // Test based on the offset in the record, because we may have 
        // an "union", where all fields start at offset 0.
        auto fieldA = fieldInstrA->GetSelectedField();
        auto fieldB = fieldInstrB->GetSelectedField();
        ...
        if(fieldA.FieldOffset != fieldB.FieldOffset) {
            ...
            return AreArrayRecordRefsDifferent(fieldInstrA->BaseOp(), 
                                               fieldInstrB->BaseOp(),
                                               baseOpA, baseOpB, level - 1);
        }
    }
    ...
    // }                        }
    // 'field p, 0' and 'field (field p, 1), 0' are independent.
    if(auto ancestorA = GetAncestor(fieldInstrA, fieldInstrB)) {
        return AreFieldsDifferent(ancestorA, fieldInstrB, 
                                  baseOpA, baseOpB, level);
    }
    else if(auto ancestorB = GetAncestor(fieldInstrB, fieldInstrA)) {
        return AreFieldsDifferent(ancestorB, fieldInstrA,
                                  baseOpA, baseOpB, level);
    }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
FieldInstr* LoadStoreElimination::GetAncestor(FieldInstr* fieldInstrA, 
                                                FieldInstr* fieldInstrB) {
    // Test if 'fieldInstrB' is an ancestor for 'fieldInstrA'.
    while(fieldInstrA) {
        if(fieldInstrA->BaseOp() == fieldInstrB->BaseOp()) {
            return fieldInstrA;
        }
        else fieldInstrA = fieldInstrA->BaseOp()->DefiningInstrAs<FieldInstr>();
    }
...
Operand* LoadStoreElimination::ExtractFromSetMemory(CallInstr* instr, Operand* addressOp) {
    // To be able to eliminate a 'load' from a location that is target 
	// by a 'setMemory' we need to have complete overlapping:
    ...
    if((setBaseOp != loadBaseOp) || (setOffset > loadOffset)) {
        return nullptr;
    }
...
bool LoadStoreElimination::IsSetMemoryOfSize(CallInstr* instr, __int64 setOffset, 
    ...
    if(auto intConst = lengthOp->As<IntConstant>()) {
        return (setOffset + intConst->Value()) >= (loadOffset + loadSize);
    }
    ...
    return opInfo.IsGreaterOrEqual(lengthOp, requiredConst) == RangeResult::Yes;
}
...
bool LoadStoreElimination::IsSafeToSpeculateLoad(LoadInstr* loadInstr, Block* toBlock) {
    ...
    if((GetSafetyInfo()->IsSafeToReorderLoad(loadInstr) == false) ||
       (GetSafetyInfo()->DefinitionDominatesBlock(sourceOp, toBlock) == false)) {
        return false;
    }
...
bool LoadStoreElimination::IsAddressNotTaken(Operand* op, VariableReference** variable) {
    if(auto variableRef = op->As<VariableReference>()) {
        return variableRef->IsAddressNotTaken();
    }
...
Operand* LoadStoreElimination::GetBaseOperand(Operand* op) {
    ...
    if(auto instr = op->DefiningInstruction()) {
        if(instr->IsAddress() || instr->IsIndex() || instr->IsField()) {
...
void LoadStoreElimination::EliminateRedundantLoads(Block* block) {
    ...
    while(instr) {
        if(auto loadInstr = instr->As<LoadInstr>()) {
            ...
            if(GetSafetyInfo()->IsSafeToEliminateLoad(loadInstr) == false) {
...
void LoadStoreElimination::EliminateDeadStores(Block* block) {
    // Scan each instruction in the block and try to find 
    // 'store' instructions and memory intrinsics that are overwritten.
    Instruction* instr = block->FirstInstruction();

    while(instr) {
        if(auto storeInstr = instr->As<StoreInstr>()) {
            // If the 'store' is marked as 'volatile' we're not allowed to remove it!
            if(GetSafetyInfo()->IsSafeToEliminateStore(storeInstr) == false) {
                instr = instr->NextInstruction();
                continue;
            }
			...
			if(auto intrinsic = callInstr->GetIntrinsic()) {
				if(intrinsic->IsMemoryIntrinsic() &&
				   IsMemoryIntrinsicDead(callInstr, intrinsic)) {
					instr = instr->NextInstruction();
					LogCallRemoved(callInstr);
					Remove(callInstr);
					continue;
				}
			}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool LoadStoreElimination::IsMemoryIntrinsicDead(CallInstr* callInstr,
												 Intrinsic* intrinsic) {
	DebugValidator::IsNotNull(callInstr);
	DebugValidator::IsNotNull(intrinsic);

	if((intrinsic->Is<SetMemoryIntr>() || 
	    intrinsic->Is<CopyMemoryIntr>()) == false) {
		return false;
	}

	Operand* destOp = intrinsic->Is<SetMemoryIntr>() ? 
					  SetMemoryIntr::GetDestination(callInstr) :
					  CopyMemoryIntr::GetDestination(callInstr);
	Operand* lengthOp = intrinsic->Is<SetMemoryIntr>() ? 
						SetMemoryIntr::GetLength(callInstr) :
						CopyMemoryIntr::GetLength(callInstr);
	__int64 baseDestOffset;
	Operand* baseDestOp = ExtractBaseAndOffset(destOp, baseDestOffset);

	// Scan the instructions that follow the intrinsic call
	// and look for another memory intrinsic that writes completely
	// over the current one. If an instruction is found that might read/write
	// to/from the destination or source operands exit.
	auto instr = callInstr->NextInstruction();

	while(instr) {
		if(auto loadInstr = instr->As<LoadInstr>()) {
			// The 'load' source should not alias the destination operand.
			if(MayLoadReadFromAddress(loadInstr, destOp)) {
				return false;
			}
		}
		else if(auto callInstr = instr->As<CallInstr>()) {
			// If the called function is also a memory intrinsic
			// check if it overwrites the destination operand.
			// Otherwise the function should not alias the destination operand.
			if(auto intrinsic = callInstr->GetIntrinsic()) {
				if(intrinsic->IsMemoryIntrinsic() &&
				   IsDestinationOverwritten(baseDestOp, baseDestOffset, lengthOp, 
											callInstr, intrinsic)) {
					// Found a 'setMemory'/'copyMemory' call that completely
					// overwrites the current one!
					return true;
				}
			}

			if(CallMayReadFromAddress(callInstr, destOp)) {
				return false;
			}
		}
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool LoadStoreElimination::MayLoadReadFromAddress(LoadInstr* instr, Operand* addressOp) {
	return AreReferencesDifferent(instr->SourceOp(), addressOp) == false;
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool LoadStoreElimination::IsDestinationOverwritten(Operand* baseDestOp, __int64 baseDestOffset,
													Operand* lengthOp, CallInstr* callInstr, 
													Intrinsic* intrinsic) {
	DebugValidator::IsNotNull(callInstr);
	DebugValidator::IsNotNull(intrinsic);
	...
	Operand* otherDestOp = intrinsic->Is<SetMemoryIntr>() ?
						   SetMemoryIntr::GetDestination(callInstr) :
						   CopyMemoryIntr::GetDestination(callInstr);
	__int64 otherBaseDestOffset;
	Operand* otherBaseDestOp = ExtractBaseAndOffset(otherDestOp, otherBaseDestOffset);

	//! TODO: Allow region that starts at diff offset
	// but still covers the original region completely.
	if((otherBaseDestOp != baseDestOp) ||
	   (otherBaseDestOffset != baseDestOffset)) {
		return false;
	}

	Operand* otherLengthOp = intrinsic->Is<SetMemoryIntr>() ? 
							 SetMemoryIntr::GetLength(callInstr) :
							 CopyMemoryIntr::GetLength(callInstr);
	return IsLargetOrEqual(otherLengthOp, lengthOp);
}

// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool LoadStoreElimination::IsLargetOrEqual(Operand* opA, Operand* opB) {
	if(opA == opB) {
		return true;
	}
	else if(opA->IsIntConstant() && opB->IsIntConstant()) {
	...
	OperandInfo opInfo(funct_->ParentUnit(), GetTarget());
	return opInfo.IsGreaterOrEqual(opA, opB) == RangeResult::Yes;
}
...
Operand* LoadStoreElimination::ExtractBaseAndOffset(Operand* destOp, __int64& offset) {
    ...
    // This looks through 'index', 'field' and 'addr' instructions,
    ...
    while(destOp->HasDefiningInstruction()) {
        ...
        else if(auto fieldInstr = destOp->DefiningInstrAs<FieldInstr>()) {
            // The index is always a constant.
            auto recordType = fieldInstr->GetRecordType();
            offset += recordType->GetFieldOffset(fieldInstr->GetFieldIndex());
            destOp = fieldInstr->BaseOp();
        }
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
void LoadStoreElimination::LogCallRemoved(CallInstr* instr) {
#if 0
	auto block = instr->ParentBlock();
	auto function = block->ParentFunction();
	string blockName = block && block->HasName() ? *block->Name() : "UNTITLED";
	string functionName = function && function->HasName() ? *function->Name() : "UNTITLED";
	string text = IRPrinter(instr).ToString();
	Log::Warning("Call removed from " + functionName + ":" + blockName + " - " + text);
#endif
}
