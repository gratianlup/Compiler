...
void Inliner::InlineCall(CallInstr* instr) {
    ...
    if(instr->HasDestinationOp()) {
        if(returnedOp) {
            ...
            // A return value doesn't exist if the inlined function
            // contains only an infinite loop.
            auto& constants = callee->ParentUnit()->Constants();
            auto type = instr->GetDestinationOp()->GetType();
            
            instr->GetDestinationOp()->ReplaceWith(constants.GetUndefined(type));
        }
    }
...
Operand* Inliner::InlineFunction(Function* source, CallInstr::ArgumentList* argList,
    ...
    // 'firstBlock' already contains a 'goto', but 'exitBlock' does not.
    // Note that it is possible the exit block doesn't exist
    // because the inlined function contained only an infinite loop.
...
void Inliner::InlineFunction(Function* source, Function* dest,
    ...
    if(mergePhi) {
        returnedOp = mergePhi->GetDestinationOp();
    }
}
...
void Inliner::ProcessBranching(Block* source, Block* copy, Function* dest) {
    ...
    if(auto gotoInstr = branchingInstr->As<GotoInstr>()) {
        ...
        singleSuccessor = gotoInstr->TargetBlock();
    }
...
void Inliner::ProcessPhiInstructions(Function* dest) {
    ...
    for(int i = 0; i < phiInstrs_.Count(); i++) {
        ...
        for(int j = 0; j < phiInstr->OperandCount(); j++) {
            auto incomingBlock = phiInstr->GetOperandBlock(i);
...
VariableReference* Inliner::CopyVariable(Variable* variable, Function* dest) {
    ...
    if(variable->HasName()) {
        ...
        copyVariable = Variable::GetVariable(variable->GetType(), name, dest);
        // Copy the attributes.
        copyVariable->SetAlignment(variable->Alignment());
        copyVariable->SetDllVisibility(variable->GetDllVisibility());
        copyVariable->SetIsAddresTaken(variable->IsAddressTaken());
    ...
    else copyVariable = Variable::GetVariable(variable->GetType(), nullptr, dest);
...
Block* Inliner::MergeReturnedValues(Function* dest, Block* afterBlock,
                                    PhiInstr*& mergePhi) {
    // The function might not have a return if it contains
    // only an infinite loop, so we just ignore it.
    if(returnInstrs_.IsEmpty()) {
        return nullptr;
    }
