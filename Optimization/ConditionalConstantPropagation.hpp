...
public:
    ...
    explicit ConstantLatticeCell(Constant* constant) :
            LatticeCell(CellType::Value), constant_(constant) {}
...
private:
    ...
    virtual LatticeCell* GetTopCell() override {
        return cellCache_->GetTopCell();
    }

    virtual LatticeCell* GetBottomCell() override {
        return cellCache_->GetBottomCell();
    }

    // Returns a lattice cell that represents the specified constant.
    virtual LatticeCell* GetCellForConstant(Constant* constant) override {
    ...
    virtual LatticeCell* GetCellForParameter(Parameter* parameter) override;

    virtual LatticeCell* GetInitializationCell(Operand* op) override {
        auto cell = Lattice::GetInitializationCell(op);
        if(cell) return cell;
        else return GetTopCell();
    }

    virtual bool AreCellsEqual(LatticeCell* cellA, LatticeCell* cellB) override;

    virtual LatticeCell* MergeCells(LatticeCell* cellA, LatticeCell* cellB) override;

    // Methods for simulating the instructions.
    virtual void Transfer(Instruction* instr) override;
    
    virtual LatticeCell* TransferPhi(PhiInstr* instr, bool isOptimistic) override;
    ...
    virtual int AddFeasibleSuccessors(ControlInstr* instr, 
                                      TSuccessorList& successorList) override;
...
public:
    ...
    float GetFoldedInstructionsWeight(bool includeChildren = false) const {
        float value = (float)((counts_.SimpleCount * SIMPLE_WEIGHT) +
                      ...
                      (float)std::max(1, ((std::min(counts_.SimpleCount, 1) * SIMPLE_WEIGHT) +
        ...
        if(includeChildren && (children_.Count() > 0)) {
            for(int i = 0; i < children_.Count(); i++) {
