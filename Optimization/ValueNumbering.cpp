...
bool ValueNumbering::HandleSpecialCases(Instruction*& instr, DominatorNode* blockNode,
    ...
    // and function that don't depend on the global state ('nostate').
...
Operand* ValueNumbering::ProcessLoad(LoadInstr* instr) {
    ...
    AvailableOperand availableOp;

    if(availableMemOps_.TryGetValue(instr->SourceOp(), &availableOp)) {
        // Found an operand, now check the version.
        if(availableOp.Version == requiredVersion) {
            return availableOp.Operand;
        }
    }

    // Insert the loaded operand into the available table,
    // so that it can be reused by subsequent loads.
    availableOp.Operand = instr->ResultOp();
    availableOp.Version = memoryVersion_;
    availableMemOps_.Add(instr->SourceOp(), availableOp);
...
bool ValueNumbering::IsSmallLoopHeaderWithNoStore(LoadInstr* loadInstr) {
    ...
    if(auto gotoInstr = trueBlock->BranchInstruction()->As<GotoInstr>()) {
        loopBody = trueBlock;
        valid = gotoInstr->TargetBlock() == block;
    }
...
Operand* ValueNumbering::ProcessCall(CallInstr* instr, DominatorNode* blockNode,
    ...
    if(ValueNumber::IsCallSafe(instr)) {
        auto previousResult = table->GetOrInsert(instr);

        if(previousResult == nullptr) {
            return nullptr;
        }

        // Even if we find a suitable 'call', we might not be able
        // to use it if it has pointer arguments that point to locations
        // that might have been changed between the two calls.
        auto previousCall = previousResult->DefiningInstruction()->As<CallInstr>();

        if(HasGlobalStateChanged(previousCall, instr) == false) {
...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
bool ValueNumbering::HasGlobalStateChanged(CallInstr* previousCall, 
                                           CallInstr* currentCall) {
    DebugValidator::IsNotNull(previousCall);
    DebugValidator::IsNotNull(currentCall);

    // We need to check if any of the pointer arguments
    // might have changed since the call to 'previousCall'
    // (this also includes the effect of the call itself).
    for(int i = 0; i < previousCall->ArgumentCount(); i++) {
        auto argument = previousCall->GetArgument(i);

        if(argument->IsPointer()) {
            //! TODO: USE ALIAS ANALYSIS
            //! ADD TO LIST OF TRACKED OPERANDS, WALK TO PREVIOUS_CALL
            //! AND CHECK IF ANY STORE/CALL HAS ALIAS WITH ANY TRACKED
            return true;
        }
    }
...
void ValueNumbering::ProcessBranching(DominatorNode* blockNode, DominatorNode* childNode,
    ...
    if(auto ifInstr = blockNode->Block()->BranchInstruction()->As<IfInstr>()) {
        if(auto cmpInstr = ifInstr->ConditionOp()->DefiningInstrAs<CmpInstrBase>()) {
            ...
            if((childNode->Block() == ifInstr->TrueTargetBlock()) &&
            ...
            else if((childNode->Block() == ifInstr->FalseTargetBlock()) &&
                     BlockUtils::DominatesItself(childNode->Block(), block, domTree_)) {
